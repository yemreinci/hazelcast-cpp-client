...
main(int argc, char* args[])
{
    ...
    HZ_LOG(logger_,
           info,
           (boost::format("Soak test is starting with the following parameters: threadCount = %1% "
                          ",  server address = %2%") %
    ...
    for (int i = 0; i < thread_count; i++) {
        tasks.emplace_back(boost::async([&]() {
            HZ_LOG(logger_,
                   info,
                   (boost::format("Thread %1% is started") % std::this_thread::get_id()).str());
            ...
            while (!is_cancelled) {
                ...
                try {
                    if (operation < 30) {
                        ...
                        map
                          ->execute_on_key<std::string, std::string, identified_entry_processor>(
                            key, identified_entry_processor{ std::to_string(distrib(gen)) })
                          .get();
                    ...
                    if (total_count % 10000 == 0) {
                        HZ_LOG(
                          logger_,
                          info,
                          (boost::format("Thread %1% --> Total: %2% {get count: %3% , put count: "
                                         "%4% , values count: %5% , execute_on_key count: %6%") %
                           std::this_thread::get_id() % total_count % get_count % put_count %
                           values_count % execute_on_key_count)
                            .str());
                    }
                } catch (std::exception& e) {
                    HZ_LOG(
                      logger_, warning, (boost::format("Exception occured: %1%") % e.what()).str());
                }
            }
            ...
            HZ_LOG(logger_,
                   info,
                   (boost::format("Thread %1% is finished. --> Total: %2% {get count: %3% , put "
