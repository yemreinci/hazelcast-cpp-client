...
public:
    ...
    {
        return person_map.values<Person>(query::sql_predicate(hz, std::string("name==") + name))
          .get();
    }
    ...
    std::vector<Person> get_with_name_and_age(hazelcast_client& hz,
                                              const std::string& name,
                                              int32_t age,
                                              hazelcast::client::imap& person_map)
    {
        return person_map
          .values<Person>(query::sql_predicate(
            hz, (boost::format("name == %1% AND age == %2%") % name % age).str()))
          .get();
    }
...
query_map_using_paging_predicate()
{
    ...
    auto predicate3 =
      employees->new_paging_predicate<int, employee>(employee_entry_comparator(), (size_t)predSize);
...
query_map_using_different_predicates()
{
    ...
    values = intMap
               ->values<int, query::equal_predicate>(
                 query::equal_predicate(client, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
               .get();
    ...
    values = intMap
               ->values<int, query::equal_predicate>(
                 query::equal_predicate(client, query::query_constants::THIS_ATTRIBUTE_NAME, 8))
               .get();
    ...
    values = intMap
               ->values<int, query::not_equal_predicate>(
                 query::not_equal_predicate(client, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
               .get();

    // this(value) != 8
    values = intMap
               ->values<int, query::not_equal_predicate>(
                 query::not_equal_predicate(client, query::query_constants::THIS_ATTRIBUTE_NAME, 8))
               .get();
    ...
    // value <= 10
    values = intMap
               ->values<int, query::greater_less_predicate>(query::greater_less_predicate(
    ...
    values = intMap
               ->values<int, query::in_predicate>(
                 query::in_predicate(client, query::query_constants::KEY_ATTRIBUTE_NAME, inVals))
               .get();

    // value in {4, 10, 19}
    values = intMap
               ->values<int, query::in_predicate>(
                 query::in_predicate(client, query::query_constants::THIS_ATTRIBUTE_NAME, inVals))
               .get();
    ...
    values =
      intMap
        ->values<int>(query::not_predicate(
          client,
          query::between_predicate(client, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10)))
        .get();
    ...
    auto sql =
      (boost::format("%1% BETWEEN 4 and 7") % query::query_constants::KEY_ATTRIBUTE_NAME).str();
