...
main()
{
    ...
    nearCacheConfig.get_eviction_config()
      .set_eviction_policy(config::LRU)
    ...
    NearCacheSupport::print_near_cache_stats(
      map, "The put(1..100, article) calls have no effect on the empty Near Cache");

    for (int i = 1; i <= 100; i++) {
        map->get<int, std::string>(i).get();
    }
    NearCacheSupport::print_near_cache_stats(map,
                                             "The first get(1..100) calls populate the Near Cache");

    for (int i = 1; i <= 100; i++) {
        map->get<int, std::string>(i).get();
    }
    NearCacheSupport::print_near_cache_stats(
      map, "The second get(1..100) calls are served from the Near Cache");

    map->put<int, std::string>(101, "bar").get();
    NearCacheSupport::print_near_cache_stats(
      map, "The put(101, \"bar\") call has no effect on the populated Near Cache");

    map->get<int, std::string>(101).get();
    NearCacheSupport::print_near_cache_stats(
      map, "The first get(101) call triggers the eviction and population of the Near Cache");
    ...
    map->get<int, std::string>(101).get();
    NearCacheSupport::print_near_cache_stats(
      map, "The second get(101) call is served from the Near Cache");
