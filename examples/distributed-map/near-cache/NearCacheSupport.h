...
public:
    ...
    static void print_near_cache_stats(std::shared_ptr<hazelcast::client::imap>& map)
    {
        ...
        printf("The first article instance was retrieved from the remote instance (Near Cache "
               "misses: %ld).\n",
               (long)stats->get_misses());
        printf("The second and third article instance were retrieved from the local Near Cache "
               "(Near Cache hits: %ld).\n",
               (long)stats->get_hits());
    }
    ...
    static void wait_for_invalidation_events()
    {
        std::this_thread::sleep_for(
          std::chrono::seconds(2 * CACHE_INVALIDATION_MESSAGE_BATCH_FREQUENCY_SECONDS));
    }

    static void wait_for_near_cache_eviction_count(std::shared_ptr<hazelcast::client::imap>& map,
