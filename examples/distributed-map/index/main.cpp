...
main()
{
    ...
    for (int i = 0; i < mapSize; ++i) {
        map
          ->put<std::string, Person>(
            std::string("person-") + std::to_string(i),
            { std::string("myname-") + std::to_string(i), (i % 2 == 0), (i % 100) })
          .get();
    }
    ...
    auto entries = map
                     ->entry_set<std::string, Person>(
                       hazelcast::client::query::sql_predicate(hz, "name == 'myname-30'"))
                     .get();
