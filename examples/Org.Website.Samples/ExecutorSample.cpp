...
// The Java server side example MessagePrinter implementation looks like the following (Please
// observe that Java class should implement the Callable and the IdentifiedDataSerializable
// interfaces):
//
...
struct hz_serializer<MessagePrinter> : identified_data_serializer
{
    ...
    static int32_t get_class_id() noexcept { return 555; }

    static void write_data(const MessagePrinter& object,
...
public:
    ...
    {
        std::cout << "The execution of the task is completed successfully and server returned:"
                  << *response << std::endl;
    }
...
main()
{
    ...
    auto result_future =
      ex->submit<MessagePrinter, std::string>(MessagePrinter{ "message to any node" });
    ...
    ex->execute_on_member<MessagePrinter>(
      MessagePrinter{ "message to very first member of the cluster" }, firstMember);
    // Submit the MessagePrinter Runnable to all Hazelcast Cluster Members
    ex->execute_on_all_members<MessagePrinter>(
      MessagePrinter{ "message to all members in the cluster" });
    // Submit the MessagePrinter Runnable to the Hazelcast Cluster Member owning the key called
    // "key"
    ...
    ex->submit<MessagePrinter, std::string>(
      MessagePrinter{ "Message when using the member selector" }, MyMemberSelector());
