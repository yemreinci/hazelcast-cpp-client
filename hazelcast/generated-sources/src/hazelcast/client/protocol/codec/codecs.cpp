...
void
client_addpartitionlostlistener_handler::handle(ClientMessage& msg)
{
    auto messageType = msg.get_message_type();
    if (messageType == 1538) {
        auto* initial_frame = reinterpret_cast<ClientMessage::frame_header_t*>(
...
void
client_adddistributedobjectlistener_handler::handle(ClientMessage& msg)
{
    auto messageType = msg.get_message_type();
    if (messageType == 2306) {
        auto* initial_frame = reinterpret_cast<ClientMessage::frame_header_t*>(
...
ClientMessage
map_unlock_encode(const std::string& name, const data& key, int64_t thread_id, int64_t reference_id)
{
    size_t initial_frame_size =
      ClientMessage::REQUEST_HEADER_LEN + ClientMessage::INT64_SIZE + ClientMessage::INT64_SIZE;
    ClientMessage msg(initial_frame_size);
...
void
map_addentrylistenertokeywithpredicate_handler::handle(ClientMessage& msg)
{
    auto messageType = msg.get_message_type();
...
void
map_addpartitionlostlistener_handler::handle(ClientMessage& msg)
{
    auto messageType = msg.get_message_type();
...
ClientMessage
topic_publishall_encode(const std::string& name, const std::vector<data>& messages)
{
    size_t initial_frame_size = ClientMessage::REQUEST_HEADER_LEN;
    ClientMessage msg(initial_frame_size);
    msg.set_retryable(false);
...
{
    size_t initial_frame_size =
      ClientMessage::REQUEST_HEADER_LEN + ClientMessage::INT32_SIZE + ClientMessage::INT32_SIZE;
    ClientMessage msg(initial_frame_size);
    msg.set_retryable(true);
...
ClientMessage
atomicref_contains_encode(const raft_group_id& group_id, const std::string& name, const data* value)
{
    size_t initial_frame_size = ClientMessage::REQUEST_HEADER_LEN;
    ClientMessage msg(initial_frame_size);
    msg.set_retryable(true);
...
void
replicatedmap_addentrylistenertokeywithpredicate_handler::handle(ClientMessage& msg)
{
    auto messageType = msg.get_message_type();
