...
protected:
    ...
    {
    public:
        ThreeSecondDelayCompleteOperation(
        ...
        virtual const std::string get_name() const { return "ThreeSecondDelayCompleteOperation"; }
...
remote::Member
HazelcastServerFactory::start_server()
{
...
bool
HazelcastServerFactory::shutdown_server(const remote::Member& member)
{
    return remoteController->shutdownMember(cluster_id_, member.uuid);
}

bool
HazelcastServerFactory::terminate_server(const remote::Member& member)
{
    return remoteController->terminateMember(cluster_id_, member.uuid);
}

const std::string&
HazelcastServerFactory::get_server_address()
{
    return server_address_;
}

std::string
HazelcastServerFactory::read_from_xml_file(const std::string& xml_file_path)
{
...
const std::string&
HazelcastServerFactory::get_cluster_id() const
{
    return cluster_id_;
}
...
TEST_F(ClientExpirationListenerTest, notified_afterExpirationOfEntries)
{
    ...
    expirationListener.on_expired(
      [&expirationEventArrivalCount](entry_event&&) { expirationEventArrivalCount.count_down(); });
...
TEST_F(ClientListenerFunctionObjectsTest, lambda)
{
    ...
    auto id = map
                ->add_entry_listener(entry_listener().on_added([&called](entry_event&& e) {
...
TEST_F(ClientListenerFunctionObjectsTest, functor)
{
    ...
    auto id = map->add_entry_listener(entry_listener().on_added(std::move(handler)), true).get();
...
TEST_F(ClientListenerFunctionObjectsTest, staticFunction)
{
    ...
    auto id =
      map->add_entry_listener(entry_listener().on_added(&OnEntryAdded::handler), true).get();
...
public:
    ...
    int get_actual_key() const { return actual_key_; }
...
public:
    ...
    static client_config create_near_cached_object_map_client_config()
    {
        ...
        config.add_near_cache_config(
          config::near_cache_config(ONE_SECOND_MAP_NAME, config::OBJECT));

        return config;
    }

    ClientMapTest()
      : client_(hazelcast_client(GetParam()()))
...
protected:
                                           ...
                                           boost::latch* evict_latch = nullptr)
    {
        return entry_listener()
          .on_added([&add_latch](entry_event&&) { add_latch.count_down(); })
          .on_removed([&remove_latch](entry_event&&) { remove_latch.count_down(); })
          .on_updated([&update_latch](entry_event&&) { update_latch.count_down(); })
          .on_expired([&expiry_latch](entry_event&&) { expiry_latch.count_down(); })
    ...
    void validate_expiry_invalidations(std::shared_ptr<imap> map, std::function<void()> f)
    {
        ...
        auto id =
          map->add_entry_listener(make_countdown_listener(dummy, dummy, dummy, expiry), false)
            .get();
...
TEST_P(ClientMapTest, testIssue537)
{
    ...
    listener.on_added([&latch1](entry_event&&) { latch1.count_down(); })
...
TEST_P(ClientMapTest, testRemoveAll)
{
    ...
    imap_
      ->remove_all(
        query::equal_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, "key5"))
      .get();
    ...
    imap_
      ->remove_all(
        query::like_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value%"))
      .get();
...
TEST_P(ClientMapTest, testGetAllPutAll)
{
    ...
    std::unordered_map<std::string, std::string> m2 =
      imap_->get_all<std::string, std::string>(tempSet).get();
...
TEST_P(ClientMapTest, testPutTtl)
{
    validate_expiry_invalidations(imap_, [=]() {
        imap_->put<std::string, std::string>("key1", "value1", std::chrono::seconds(1)).get();
    });
}
...
TEST_P(ClientMapTest, testPutConfigTtl)
{
    ...
    validate_expiry_invalidations(
      map, [=]() { map->put<std::string, std::string>("key1", "value1").get(); });
}
...
TEST_P(ClientMapTest, testPutIfAbsent)
{
    ...
    ASSERT_EQ("value1",
              (imap_->put_if_absent<std::string, std::string>("key1", "value3").get().value()));
}

TEST_P(ClientMapTest, testPutIfAbsentTtl)
{
    ASSERT_FALSE(
      (imap_->put_if_absent<std::string, std::string>("key1", "value1", std::chrono::seconds(1))
         .get()
         .has_value()));
    ASSERT_EQ(
      "value1",
      (imap_->put_if_absent<std::string, std::string>("key1", "value3", std::chrono::seconds(1))
         .get()
         .value()));
    ...
    ASSERT_EQ(
      "value3",
      (imap_->put_if_absent<std::string, std::string>("key1", "value4", std::chrono::seconds(1))
         .get()
         .value()));
}
...
{
    validate_expiry_invalidations(
      imap_, [=]() { imap_->set("key1", "value1", std::chrono::seconds(1)).get(); });
}
...
TEST_P(ClientMapTest, testJsonValues)
{
    ...
    for (int i = 0; i < numItems; ++i) {
        imap_
          ->put("key_" + std::to_string(i),
                hazelcast_json_value("{ \"value\"=\"value_" + std::to_string(i) + "\"}"))
          .get();
    }
...
TEST_P(ClientMapTest, DISABLED_testJsonValuesWithpaging_predicate)
{
    ...
    for (int i = 0; i < numItems; ++i) {
        imap_
          ->put<std::string, hazelcast_json_value>(
            "key_" + std::to_string(i),
            hazelcast_json_value("{ \"value\"=\"value_" + std::to_string(i) + "\"}"))
          .get();
    }
    auto predicate =
      imap_->new_paging_predicate<std::string, hazelcast_json_value>((size_t)predSize);
...
TEST_P(ClientMapTest, testValuesWithPredicate)
{
    ...
    values = int_map_
               ->values<int>(
                 query::equal_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
               .get();
    ...
    values = int_map_
               ->values<int>(
                 query::equal_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 8))
               .get();
    ASSERT_EQ(1, (int)values.size());
    ...
    // key == numItems
    values = int_map_
               ->values<int>(query::equal_predicate(
                 client_, query::query_constants::KEY_ATTRIBUTE_NAME, numItems))
               .get();
    ...
    values = int_map_
               ->values<int>(
                 query::not_equal_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
               .get();
    ...
    // this(value) != 8
    values = int_map_
               ->values<int>(query::not_equal_predicate(
                 client_, query::query_constants::THIS_ATTRIBUTE_NAME, 8))
               .get();
    ASSERT_EQ(numItems - 1, (int)values.size());
    std::sort(values.begin(), values.end());
    for (int i = 0; i < numItems - 1; ++i) {
    ...
    // 5 <= key <= 10
    values = int_map_
               ->values<int>(query::between_predicate(
                 client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10))
               .get();
    std::sort(values.begin(), values.end());
    ...
    values = int_map_
               ->values<int>(query::between_predicate(
                 client_, query::query_constants::KEY_ATTRIBUTE_NAME, 20, 30))
               .get();
    ASSERT_EQ(0, (int)values.size());
    ...
    values = int_map_
               ->values<int>(query::greater_less_predicate(
                 client_, query::query_constants::THIS_ATTRIBUTE_NAME, 10, true, true))
               .get();
    ASSERT_EQ(6, (int)values.size());
    std::sort(values.begin(), values.end());
    for (int i = 0; i < 6; ++i) {
    ...
    values = int_map_
               ->values<int>(query::greater_less_predicate(
                 client_, query::query_constants::THIS_ATTRIBUTE_NAME, 15, true, false))
               .get();
    ...
    values = int_map_
               ->values<int>(query::greater_less_predicate(
                 client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, false, false))
               .get();
    ...
    // key in {4, 10, 19}
    values = int_map_
               ->values<int>(query::in_predicate(
                 client_, query::query_constants::KEY_ATTRIBUTE_NAME, 4, 10, 19))
               .get();
    ...
    values = int_map_
               ->values<int>(query::in_predicate(
                 client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19))
               .get();
    ...
    values =
      int_map_->values<int>(query::instance_of_predicate(client_, "java.lang.Integer")).get();
    ...
    // not_predicate
    // !(5 <= key <= 10)
    query::not_predicate notPredicate(
      client_,
    ...
    values =
      int_map_
        ->values<int>(query::and_predicate(
          client_,
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10),
          query::in_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19)))
        .get();
    ASSERT_EQ(1, (int)values.size());
    std::sort(values.begin(), values.end());
    ...
    std::vector<std::string> strValues =
      imap_
        ->values<std::string>(
          query::like_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value1"))
        .get();
    ...
    strValues = imap_
                  ->values<std::string>(query::ilike_predicate(
                    client_, query::query_constants::THIS_ATTRIBUTE_NAME, "%VALue%1%"))
                  .get();
    ...
    strValues = imap_
                  ->values<std::string>(query::ilike_predicate(
                    client_, query::query_constants::THIS_ATTRIBUTE_NAME, "%VAL%2%"))
                  .get();
    ...
    hazelcast::util::hz_snprintf(
      sql, 50, "%s BETWEEN 4 and 7", query::query_constants::KEY_ATTRIBUTE_NAME);
    ...
    strValues = imap_
                  ->values<std::string>(query::regex_predicate(
                    client_, query::query_constants::THIS_ATTRIBUTE_NAME, ".*value.*2.*"))
                  .get();
    ASSERT_EQ(2, (int)strValues.size());
    std::sort(strValues.begin(), strValues.end());
    ASSERT_EQ("myvalue_22_test", strValues[0]);
...
TEST_P(ClientMapTest, testValuesWithpaging_predicate)
{
    ...
    query::greater_less_predicate lessThanTenPredicate(
      client_, query::query_constants::THIS_ATTRIBUTE_NAME, 9, false, true);
    ...
    predSize = 2;
    auto predicate3 =
      int_map_->new_paging_predicate<int, employee>(EmployeeEntryComparator(), (size_t)predSize);
...
TEST_P(ClientMapTest, testKeySetWithPredicate)
{
    ...
    // key == numItems
    keys = int_map_
             ->key_set<int>(query::equal_predicate(
               client_, query::query_constants::KEY_ATTRIBUTE_NAME, numItems))
             .get();
    ...
    keys = int_map_
             ->key_set<int>(
               query::not_equal_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 8))
             .get();
    ASSERT_EQ(numItems - 1, (int)keys.size());
    std::sort(keys.begin(), keys.end());
    for (int i = 0; i < numItems - 1; ++i) {
    ...
    // 20 <= key <=30
    keys = int_map_
             ->key_set<int>(query::between_predicate(
               client_, query::query_constants::KEY_ATTRIBUTE_NAME, 20, 30))
             .get();
    ASSERT_EQ(0, (int)keys.size());
    ...
    // value <= 10
    keys = int_map_
             ->key_set<int>(query::greater_less_predicate(
               client_, query::query_constants::THIS_ATTRIBUTE_NAME, 10, true, true))
             .get();
    ASSERT_EQ(6, (int)keys.size());
    std::sort(keys.begin(), keys.end());
    ...
    keys = int_map_
             ->key_set<int>(
               query::in_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 4, 10, 19))
             .get();
    ...
    keys =
      int_map_
        ->key_set<int>(query::and_predicate(
          client_,
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10),
          query::in_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19)))
        .get();
    ...
    keys =
      int_map_
        ->key_set<int>(query::or_predicate(
          client_,
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10),
          query::in_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19)))
        .get();
    ...
    strKeys = imap_
                ->key_set<std::string>(query::ilike_predicate(
                  client_, query::query_constants::THIS_ATTRIBUTE_NAME, "%VALue%1%"))
                .get();
    ...
    strKeys = imap_
                ->key_set<std::string>(query::regex_predicate(
                  client_, query::query_constants::THIS_ATTRIBUTE_NAME, ".*value.*2.*"))
                .get();
    ASSERT_EQ(2, (int)strKeys.size());
    std::sort(strKeys.begin(), strKeys.end());
...
TEST_P(ClientMapTest, testKeySetWithpaging_predicate)
{
    ...
    predSize = 2;
    auto predicate3 =
      int_map_->new_paging_predicate<int, employee>(EmployeeEntryKeyComparator(), (size_t)predSize);
    std::vector<int> result = employees_->key_set<int>(predicate3).get();
    // since keyset result only returns keys from the server, no ordering based on the value but
    // ordered based on the keys
    ASSERT_EQ(2, (int)result.size());
...
TEST_P(ClientMapTest, testEntrySetWithPredicate)
{
    ...
    // key == numItems
    entries = int_map_
                ->entry_set<int, int>(query::equal_predicate(
                  client_, query::query_constants::KEY_ATTRIBUTE_NAME, numItems))
                .get();
    ...
    // key != 5
    entries = int_map_
                ->entry_set<int, int>(query::not_equal_predicate(
                  client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5))
                .get();
    ...
    // 5 <= key <= 10
    entries = int_map_
                ->entry_set<int, int>(query::between_predicate(
                  client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10))
                .get();
    std::sort(entries.begin(), entries.end());
    ...
    // value <= 10
    entries = int_map_
                ->entry_set<int, int>(query::greater_less_predicate(
                  client_, query::query_constants::THIS_ATTRIBUTE_NAME, 10, true, true))
                .get();
    ASSERT_EQ(6, (int)entries.size());
    std::sort(entries.begin(), entries.end());
    ...
    // key in {4, 10, 19}
    entries = int_map_
                ->entry_set<int, int>(query::in_predicate(
                  client_, query::query_constants::KEY_ATTRIBUTE_NAME, 4, 10, 19))
                .get();
    ...
    // value instanceof Integer
    entries =
      int_map_->entry_set<int, int>(query::instance_of_predicate(client_, "java.lang.Integer"))
        .get();
    ...
    entries =
      int_map_->entry_set<int, int>(query::instance_of_predicate(client_, "java.lang.String"))
        .get();
    ASSERT_EQ(0, (int)entries.size());
    ...
    entries =
      int_map_
        ->entry_set<int, int>(query::and_predicate(
          client_,
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10),
          query::in_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19)))
        .get();
    ...
    entries =
      int_map_
        ->entry_set<int, int>(query::or_predicate(
          client_,
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 5, 10),
          query::in_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, 4, 10, 19)))
        .get();
    ...
    imap_->put<std::string, std::string>("key_111_test", "myvalue_111_test").get();
    expectedStrEntries[12] =
      std::pair<std::string, std::string>("key_111_test", "myvalue_111_test");
    imap_->put<std::string, std::string>("key_22_test", "myvalue_22_test").get();
    expectedStrEntries[13] = std::pair<std::string, std::string>("key_22_test", "myvalue_22_test");

    // like_predicate
    // value LIKE "value1" : {"value1"}
    std::vector<std::pair<std::string, std::string>> strEntries =
      imap_
        ->entry_set<std::string, std::string>(
          query::like_predicate(client_, query::query_constants::THIS_ATTRIBUTE_NAME, "value1"))
        .get();
    ...
    // value ILIKE "%VALue%1%" : {"key_111_test", "key1", "key10", "key11"}
    strEntries = imap_
                   ->entry_set<std::string, std::string>(query::ilike_predicate(
                     client_, query::query_constants::THIS_ATTRIBUTE_NAME, "%VALue%1%"))
                   .get();
    ...
    // value matches the regex ".*value.*2.*" : {key_22_test, value2}
    strEntries = imap_
                   ->entry_set<std::string, std::string>(query::regex_predicate(
                     client_, query::query_constants::THIS_ATTRIBUTE_NAME, ".*value.*2.*"))
                   .get();
    ASSERT_EQ(2, (int)strEntries.size());
    std::sort(strEntries.begin(), strEntries.end());
...
TEST_P(ClientMapTest, testEntrySetWithpaging_predicate)
{
    ...
    auto predicate3 =
      int_map_->new_paging_predicate<int, employee>(EmployeeEntryComparator(), (size_t)predSize);
    std::vector<std::pair<int, employee>> result =
      employees_->entry_set<int, employee>(predicate3).get();
    ASSERT_EQ(2, (int)result.size());
...
TEST_P(ClientMapTest, testListenerWithPortableKey)
{
    ...
    entry_listener listener;
    listener.on_added([&countDownLatch, &atomicInteger](entry_event&& event) {
        ++atomicInteger;
...
TEST_P(ClientMapTest, testListener)
{
    ...
    auto listener1 = make_countdown_listener(latch1Add, latch1Remove, dummy, dummy);
    auto listener2 = make_countdown_listener(latch2Add, latch2Remove, dummy, dummy);
...
TEST_P(ClientMapTest, testListenerWithtrue_predicate)
{
    ...
    auto listenerId =
      int_map_->add_entry_listener(std::move(listener), query::true_predicate(client_), false)
        .get();
...
TEST_P(ClientMapTest, testListenerWithfalse_predicate)
{
    ...
    auto listenerId =
      int_map_->add_entry_listener(std::move(listener), query::false_predicate(client_), false)
        .get();
...
TEST_P(ClientMapTest, testListenerWithequal_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener),
          query::equal_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 3),
          true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithnot_equal_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener),
          query::not_equal_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 3),
          true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithgreater_less_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(std::move(listener),
                             query::greater_less_predicate(
                               client_, query::query_constants::KEY_ATTRIBUTE_NAME, 2, true, true),
                             false)
        .get();
...
TEST_P(ClientMapTest, testListenerWithbetween_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener),
          query::between_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, 1, 2),
          true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithsql_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(std::move(listener), query::sql_predicate(client_, "__key < 2"), true)
        .get();
...
TEST_P(ClientMapTest, testListenerWithRegExPredicate)
{
    ...
    auto listenerId =
      imap_
        ->add_entry_listener(
          std::move(listener),
          query::regex_predicate(client_, query::query_constants::KEY_ATTRIBUTE_NAME, ".*met.*"),
          true)
        .get();

    imap_->put<std::string, std::string>("ilkay", "yasar").get();
    imap_->put<std::string, std::string>("mehmet", "demir").get();
    imap_->put<std::string, std::string>("metin", "ozen", std::chrono::seconds(1))
      .get(); // evict after 1 second
    ...
    ASSERT_FALSE(
      (imap_->get<std::string, std::string>("metin").get().has_value())); // trigger eviction
...
TEST_P(ClientMapTest, testListenerWithinstance_of_predicate)
{
    ...
    auto listenerId =
      int_map_
        ->add_entry_listener(
          std::move(listener), query::instance_of_predicate(client_, "java.lang.Integer"), false)
        .get();
...
TEST_P(ClientMapTest, testListenerWithnot_predicate)
{
    ...
    query::not_predicate notPredicate(
      client_,
      query::greater_less_predicate(
        client_, query::query_constants::KEY_ATTRIBUTE_NAME, 3, true, false));
...
TEST_P(ClientMapTest, testListenerWithand_predicate)
{
    ...
    // key < 3
    query::greater_less_predicate greaterLessPred(
      client_, query::query_constants::KEY_ATTRIBUTE_NAME, 3, false, true);
    // value == 1
...
TEST_P(ClientMapTest, testClearEvent)
{
    ...
    clearListener.on_map_cleared([&latch1](map_event&&) { latch1.count_down(); });
...
TEST_P(ClientMapTest, testEvictAllEvent)
{
    ...
    evictListener.on_map_evicted([&latch1](map_event&& event) { latch1.count_down(); });
...
{
    imap_->put_transient<std::string, std::string>("ali", "veli", std::chrono::milliseconds(1100))
      .get();
    imap_->flush().get();
    ASSERT_EQ(1, imap_->size().get());
...
TEST_P(ClientMapTest, testExecuteOnNonExistentKey)
{
    ...
    boost::optional<int> result =
      employees_->execute_on_key<int, int, EntryMultiplier>(17, processor).get();
...
TEST_P(ClientMapTest, testExecuteOnKeys)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_keys<int, int, EntryMultiplier>(keys, processor).get();
...
TEST_P(ClientMapTest, testExecuteOnEntries)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor).get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithtrue_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(processor, query::true_predicate(client_))
        .get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithfalse_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(processor, query::false_predicate(client_))
        .get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithand_predicate)
{
    ...
    query::and_predicate andPredicate(
      client_,
      query::between_predicate(client_, "a", 25, 35),
      query::not_predicate(client_, query::equal_predicate(client_, "a", 35)));

    EntryMultiplier processor(4);

    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, andPredicate).get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithor_predicate)
{
    ...
    /* age == 21 OR age > 25 */
    query::or_predicate orPredicate(client_,
                                    query::equal_predicate(client_, "a", 21),
                                    query::greater_less_predicate(client_, "a", 25, false, false));

    EntryMultiplier processor(4);

    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, orPredicate).get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithequal_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(processor,
                                                        query::equal_predicate(client_, "a", 25))
        .get();
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor, query::equal_predicate(client_, "a", 10))
               .get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithgreater_less_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(
          processor, query::greater_less_predicate(client_, "a", 25, false, true))
        .get(); // <25 matching
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor, query::greater_less_predicate(client_, "a", 25, true, true))
               .get(); // <=25 matching
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor, query::greater_less_predicate(client_, "a", 25, false, false))
               .get(); // >25 matching
    ...
    result = employees_
               ->execute_on_entries<int, int, EntryMultiplier>(
                 processor, query::greater_less_predicate(client_, "a", 25, true, false))
               .get(); // >=25 matching
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithin_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, predicate).get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithinstance_of_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_
        ->execute_on_entries<int, int, EntryMultiplier>(
          processor, query::instance_of_predicate(client_, "com.hazelcast.client.test.Employee"))
        .get();
...
TEST_P(ClientMapTest, testExecuteOnEntriesWithnot_predicate)
{
    ...
    std::unordered_map<int, boost::optional<int>> result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, notPredicate).get();
    ...
    result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, notfalse_predicate)
        .get();
    ...
    query::not_predicate notbetween_predicate(client_,
                                              query::between_predicate(client_, "a", 25, 35));
    result =
      employees_->execute_on_entries<int, int, EntryMultiplier>(processor, notbetween_predicate)
        .get();
...
TEST_P(ClientMapTest, testQueryOverJsonObject)
{
    ...
    std::vector<hazelcast_json_value> result =
      map
        ->values<hazelcast_json_value>(
          query::greater_less_predicate(client_, "age", 6, false, true))
        .get();
...
{
    static int get_factory_id() { return 666; }

    static int get_class_id() { return 3; }

    static void write_data(const test::ClientMapTest::EntryMultiplier& object,
...
{
public:
    static int32_t get_factory_id() { return 666; }

    static int32_t get_class_id() { return 9; }
...
struct hz_serializer<test::ClientMapTest::MapGetInterceptor> : public identified_data_serializer
{
    ...
    static int get_class_id() { return 6; }

    static void write_data(const test::ClientMapTest::MapGetInterceptor& object,
                           object_data_output& writer)
    {
...
namespace std {
template<>
struct hash<hazelcast::client::test::PartitionAwareInt>
{
