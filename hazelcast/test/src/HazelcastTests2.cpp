...
protected:
    ...
    {
        hazelcast::util::BlockingConcurrentQueue<int>* q =
          (hazelcast::util::BlockingConcurrentQueue<int>*)args.arg0;
...
const std::string UTFUtilTest::INVALID_UTF_STRING_INSUFFICIENT_BYTES =
  "a \xc3\xa9 \xe5\x92 \xf6\xa7\x93\xb5";

TEST_F(UTFUtilTest, validUTF8)
{
    ASSERT_GT(hazelcast::util::UTFUtil::is_valid_ut_f8(VALID_UTF_STRING), 0);
}

TEST_F(UTFUtilTest, invalidUTF8)
{
    ASSERT_EQ(-1, hazelcast::util::UTFUtil::is_valid_ut_f8(INVALID_UTF_STRING_INSUFFICIENT_BYTES));
}

TEST_F(UTFUtilTest, readValidUTF8)
{
    serialization::pimpl::data_input<std::string> in(VALID_UTF_STRING);
    std::string utfBuffer;
    int numberOfUtfChars = hazelcast::util::UTFUtil::is_valid_ut_f8(VALID_UTF_STRING);
...
{
    serialization::pimpl::data_input<std::string> in(INVALID_UTF_STRING_INSUFFICIENT_BYTES);
    std::string utfBuffer;
    for (int i = 0; i < 5; ++i) {
        byte c = in.read<byte>();
...
protected:
    ...
    public:
        ConcurrentQueueTask(hazelcast::util::ConcurrentQueue<int>& q,
                            ...
                            boost::latch& start_remove_latch,
                            int removal_value)
          : q_(q)
          , start_latch_(start_latch)
          , start_remove_latch_(start_remove_latch)
          , removal_value_(removal_value)
        {}
        ...
        virtual const std::string get_name() const { return "ConcurrentQueueTask"; }
...
TEST_F(ClientUtilTest, testStringUtilTimeToString)
{
    std::string timeString =
      hazelcast::util::StringUtil::time_to_string(std::chrono::steady_clock::now());
...
{
    ASSERT_EQ("never",
              hazelcast::util::StringUtil::time_to_string(std::chrono::steady_clock::time_point()));
}
...
TEST_F(ClientConfigTest, testSetCredentials)
{
    ...
    clientConfig.set_credentials(
      std::make_shared<security::username_password_credentials>(name, password));
    ...
    auto user_pass_credential =
      std::static_pointer_cast<security::username_password_credentials>(credentials);
...
TEST_F(ConfiguredBehaviourTest, testAsyncStartTrueNoCluster)
{
    ...
    ASSERT_THROW((client.get_map(random_map_name())), exception::hazelcast_client_offline);
...
TEST_F(ConfiguredBehaviourTest, testAsyncStartTrue)
{
    ...
    client_config_.get_network_config()
      ...
      .set_connection_attempt_limit(INT32_MAX);
    client_config_.set_property("hazelcast.client.shuffle.member.list", "false");
    client_config_.add_listener(
      lifecycle_listener().on_connected([&connectedLatch]() { connectedLatch.try_count_down(); }));
    client_config_.get_connection_strategy_config().set_async_start(true);
...
TEST_F(ConfiguredBehaviourTest, testReconnectModeOFFSingleMember)
{
    ...
    client.add_lifecycle_listener(
      lifecycle_listener().on_shutdown([&shutdownLatch]() { shutdownLatch.try_count_down(); }));
...
TEST_F(ConfiguredBehaviourTest, testReconnectModeASYNCSingleMemberStartLate)
{
    ...
    client_config_.add_listener(lifecycle_listener().on_connected(
      [&initialConnectionLatch]() { initialConnectionLatch.try_count_down(); }));
    client_config_.get_connection_strategy_config().set_reconnect_mode(
      config::client_connection_strategy_config::ASYNC);
    hazelcast_client client(std::move(client_config_));
...
TEST_F(PortableVersionTest, test_nestedPortable_versionedSerializer)
{
    ...
    // cached class definition of Child and the class definition from data coming from ss1 should be
    // compatible
  ...
  : public identified_data_serializer
{
    ...
    static void write_data(const test::PartitionAwareTest::SimplePartitionAwareObject& object,
...
TEST_F(ClientSerializationTest, testDifferentVersions)
{
    ...
    serialization::pimpl::data data =
      serializationService.to_data<TestNamedPortable>(TestNamedPortable{ "portable-v1", 111 });

    serialization::pimpl::data data2 =
      serializationService2.to_data<TestNamedPortableV2>(TestNamedPortableV2{ "portable-v2", 123 });
...
TEST_F(ClientSerializationTest, testBasicFunctionality)
{
    ...
    TestMainPortable main{
        (byte)113, false,           'x',
        -500,      56789,           -50992225,
        900.5678f, -897543.3678909, "this is main portable object created for testing!",
        inner
    };
...
TEST_F(ClientSerializationTest, testPrimitiveArrays)
{
    ...
    const std::string stringArray[] = { "ali",
                                        "veli",
    ...
    ASSERT_EQ(
      stringVector,
      to_data_and_back_to_object<std::vector<std::string>>(serializationService, stringVector));
}
...
TEST_F(ClientSerializationTest, testWriteObjectWithPortable)
{
    ...
    ObjectCarryingPortable<TestNamedPortable> objectCarryingPortable{ TestNamedPortable{ "name",
                                                                                         2 } };
    serialization::pimpl::data data =
      ss.to_data<ObjectCarryingPortable<TestNamedPortable>>(&objectCarryingPortable);
...
TEST_F(ClientSerializationTest, testWriteObjectWithIdentifiedDataSerializable)
{
    ...
    ObjectCarryingPortable<TestDataSerializable> objectCarryingPortable{ TestDataSerializable{
      2, 'c' } };
    serialization::pimpl::data data =
      ss.to_data<ObjectCarryingPortable<TestDataSerializable>>(&objectCarryingPortable);
...
TEST_F(ClientSerializationTest, testWriteObjectWithCustomXSerializable)
{
    serialization_config serializationConfig;
    serialization::pimpl::SerializationService ss(serializationConfig);
    ObjectCarryingPortable<TestCustomXSerializable> objectCarryingPortable{ TestCustomXSerializable{
      131321 } };
    serialization::pimpl::data data =
      ss.to_data<ObjectCarryingPortable<TestCustomXSerializable>>(&objectCarryingPortable);
...
TEST_F(ClientSerializationTest, testWriteObjectWithCustomPersonSerializable)
{
    ...
    ObjectCarryingPortable<TestCustomPerson> objectCarryingPortable{ TestCustomPerson{
      "TestCustomPerson" } };
    serialization::pimpl::data data =
      ss.to_data<ObjectCarryingPortable<TestCustomPerson>>(&objectCarryingPortable);
...
TEST_F(ClientSerializationTest, object_data_input_output)
{
    ...
    std::vector<float> ff = { 0.6543f, -3.56f, 45.67f };
    std::vector<double> dd = { 456.456, 789.789, 321.321 };
    std::vector<std::string> stringVector = { "ali",
                                              "veli",
                                              "イロハニホヘト チリヌルヲ ワカヨタレソ ツネナラム" };
    ...
    serialization::object_data_input in(out.to_byte_array(),
                                        0,
...
TEST_F(ClientSerializationTest, testGetUTF8CharCount)
{
    ...
    int strLen = hazelcast::util::Bits::read_int_b(byteArray, 0);
    ASSERT_EQ(7, strLen);
}
...
protected:
    ...
    {
        auto nearCacheConfig = create_near_cache_config(DEFAULT_NEAR_CACHE_NAME, in_memory_format);
    ...
    std::unique_ptr<hazelcast::client::internal::nearcache::impl::
                      NearCacheRecordStore<serialization::pimpl::data, serialization::pimpl::data>>
    create_near_cache_record_store(config::near_cache_config& near_cache_config,
                                   config::in_memory_format in_memory_format)
    {
        std::unique_ptr<hazelcast::client::internal::nearcache::impl::NearCacheRecordStore<
            ...
            case config::BINARY:
                recordStore = std::unique_ptr<
                  hazelcast::client::internal::nearcache::impl::
                    NearCacheRecordStore<serialization::pimpl::data, serialization::pimpl::data>>(
                  new hazelcast::client::internal::nearcache::impl::store::NearCacheDataRecordStore<
                    serialization::pimpl::data,
                    serialization::pimpl::data,
                    serialization::pimpl::data>(DEFAULT_NEAR_CACHE_NAME, near_cache_config, *ss_));
                break;
            case config::OBJECT:
                recordStore = std::unique_ptr<
                  hazelcast::client::internal::nearcache::impl::
    ...
    config::near_cache_config create_near_cache_config(const char* name,
                                                       config::in_memory_format in_memory_format)
    {
...
{
    create_near_cache_with_max_size_policy(GetParam(), config::eviction_config::ENTRY_COUNT, 1000);
}
