...
{
    const T* get_partition_key() const override { return &partition_key; }
...
struct TaskSerializerNoOpRead : public TaskSerializerFactory
{
    ...
    static T read_data(object_data_input& in) { return T{}; }
};
  ...
  : public TaskSerializerNoOpRead<test::executor::tasks::AppendCallable>
{
    ...
    static void write_data(const test::executor::tasks::AppendCallable& object,
...
struct hz_serializer<test::executor::tasks::CancellationAwareTask> : public TaskSerializerFactory
{
    ...
    static void write_data(const test::executor::tasks::CancellationAwareTask& object,
                           object_data_output& out)
    {
...
template<typename T>
struct hz_serializer<test::executor::tasks::MapPutPartitionAwareCallable<T>>
  : public TaskSerializerFactory
{
    static int32_t get_class_id()
    {
        return static_cast<int32_t>(
          test::executor::tasks::task_ids::MAP_PUTPARTITIONAWARE_CALLABLE);
    }

    static void write_data(const test::executor::tasks::MapPutPartitionAwareCallable<T>& object,
                           object_data_output& out)
    {
  ...
  : public TaskSerializerBase<test::executor::tasks::TaskWithUnserializableResponse>
{
    ...
    {
        return static_cast<int32_t>(
          test::executor::tasks::task_ids::TASK_WITH_UNSERIALIZABLE_RESPONSE);
    }
};
  ...
  : public TaskSerializerNoOpRead<test::executor::tasks::SerializedCounterCallable>
{
    ...
    static void write_data(const test::executor::tasks::SerializedCounterCallable& object,
                           object_data_output& out)
    {
