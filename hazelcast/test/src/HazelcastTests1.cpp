...
protected:
    ...
    static void SetUpTestCase() { instance = new HazelcastServer(*g_srvFactory); }
    ...
    static Response get_client_stats_from_server()
    {
        ...
        Response response;
        remoteController->executeOnController(
          response, g_srvFactory->get_cluster_id(), script, Lang::PYTHON);
        return response;
    }
    ...
    std::string get_client_local_address(hazelcast_client& client)
    {
        ...
        localAddressString << localSocketAddress->get_host() << ":"
    ...
    void produce_some_stats(std::shared_ptr<imap>& map)
    {
        ...
        ASSERT_EQ_EVENTUALLY(invalidationRequests + 1,
...
{
    client_config clientConfig;
    clientConfig.set_property(client_properties::STATISTICS_ENABLED, "false")
      .set_property(client_properties::STATISTICS_PERIOD_SECONDS, "1");
...
TEST_F(ClientStatisticsTest, testClientStatisticsContent)
{
    ...
    ASSERT_TRUE_EVENTUALLY((statsFromServer = get_client_stats_from_server()).success &&
                           !statsFromServer.result.empty());
    ASSERT_TRUE(statsFromServer.success);
    ...
    ASSERT_NE(std::string::npos,
              statsFromServer.result.find(std::string("nc.") + mapName + "." +
...
TEST_F(ClientStatisticsTest, testStatisticsCollectionNonDefaultPeriod)
{
    ...
    ASSERT_NO_THROW((connectionTimeStat = std::stoll(statsMap["clusterConnectionTimestamp"])))
      << "connectionTimeStat value is not in correct (" << stats << ")";

    ASSERT_EQ(1U, statsMap.count("clientAddress"))
      << "clientAddress stat should exist (" << stats << ")";
    std::string expectedClientAddress = get_client_local_address(*client);
    ASSERT_EQ(expectedClientAddress, statsMap["clientAddress"]);

    ASSERT_EQ(1U, statsMap.count("clientVersion"))
      << "clientVersion stat should exist (" << stats << ")";
    ASSERT_EQ(HAZELCAST_VERSION, statsMap["clientVersion"]);

    // time measured by us after client connection should be greater than the connection time
    // reported by the statistics
    ASSERT_GE(clientConnectionTime, connectionTimeStat)
      << "connectionTimeStat was " << connectionTimeStat << ", clientConnectionTime was "
      << clientConnectionTime << "(" << stats << ")";

    std::string mapHitsKey = "nc." + get_test_name() + ".hits";
    ASSERT_EQ(0U, statsMap.count(mapHitsKey))
      << mapHitsKey << " stat should not exist (" << stats << ")";
...
int32_t
hz_serializer<test::rb::StartsWithStringFilter>::get_factory_id()
{
    return 666;
}

int32_t
hz_serializer<test::rb::StartsWithStringFilter>::get_class_id()
{
    return 14;
}

void
hz_serializer<test::rb::StartsWithStringFilter>::write_data(
  const test::rb::StartsWithStringFilter& object,
...
TEST_F(RingbufferTest, headSequence)
{
    ...
    ASSERT_EQ(client2_ringbuffer_->head_sequence().get(),
              client_ringbuffer_->head_sequence().get());
}
...
TEST_F(RingbufferTest, tailSequence)
{
    ...
    ASSERT_EQ(client2_ringbuffer_->tail_sequence().get(),
              client_ringbuffer_->tail_sequence().get());
}
...
TEST_F(RingbufferTest, remainingCapacity)
{
    ...
    ASSERT_EQ(client2_ringbuffer_->remaining_capacity().get(),
              client_ringbuffer_->remaining_capacity().get());
}
...
// checks if the max count works. So if more results are available than needed, the surplus results
// should not be read.
...
namespace test {
std::string
ClientTestSupportBase::get_ca_file_path()
{
    return "hazelcast/test/resources/cpp_client.crt";
}

hazelcast::client::client_config
ClientTestSupportBase::get_config(bool ssl_enabled, bool smart)
{
    client_config clientConfig;
    clientConfig.get_network_config().add_address(
      address(g_srvFactory->get_server_address(), 5701));
    if (ssl_enabled) {
        clientConfig.set_cluster_name(get_ssl_cluster_name());
        clientConfig.get_network_config().get_ssl_config().set_enabled(true).add_verify_file(
          get_ca_file_path());
    }
...
hazelcast_client
ClientTestSupportBase::get_new_client()
{
    return hazelcast_client(get_config());
}

const std::string
ClientTestSupportBase::get_ssl_file_path()
{
    return "hazelcast/test/resources/hazelcast-ssl.xml";
}

std::string
ClientTestSupportBase::random_map_name()
{
    return random_string();
}

std::string
ClientTestSupportBase::random_string()
{
...
void
ClientTestSupportBase::sleep_seconds(int32_t seconds)
{
...
boost::uuids::uuid
ClientTestSupportBase::generate_key_owned_by(spi::ClientContext& context, const member& member)
{
...
const std::string
ClientTestSupportBase::get_ssl_cluster_name()
{
    return "ssl-dev";
}
} // namespace test
...
namespace util {
StartedThread::StartedThread(const std::string& name,
                             ...
                             void* arg3)
  : name_(name)
  , logger_(std::make_shared<logger>("StartedThread",
                                     "StartedThread",
                                     logger::level::info,
...
void
StartedThread::init(void(func)(ThreadArgs&), void* arg0, void* arg1, void* arg2, void* arg3)
{
...
protected:
#ifdef HZ_BUILD_WITH_SSL
    std::vector<hazelcast::client::internal::socket::SSLSocket::CipherInfo> get_ciphers(
      client_config config)
    {
        hazelcast_client client(std::move(config));
        spi::ClientContext context(client);
        std::vector<std::shared_ptr<connection::Connection>> conns =
          context.get_connection_manager().get_active_connections();
        EXPECT_GT(conns.size(), (size_t)0);
        std::shared_ptr<connection::Connection> aConnection = conns[0];
        hazelcast::client::internal::socket::SSLSocket& socket =
          (hazelcast::client::internal::socket::SSLSocket&)aConnection->get_socket();
...
TEST_F(ClientConnectionTest, testTcpSocketTimeoutToOutsideNetwork)
{
    ...
    config.get_network_config()
      ...
      .set_connection_timeout(std::chrono::seconds(2))
...
TEST_F(ClientConnectionTest, testSslSocketTimeoutToOutsideNetwork)
{
    ...
    config.set_cluster_name(get_ssl_cluster_name())
      ...
      .get_ssl_config()
      .set_enabled(true)
      .add_verify_file(get_ca_file_path());
    ASSERT_THROW(hazelcast_client client(std::move(config)), exception::illegal_state);
}
...
TEST_F(ClientConnectionTest, testExcludedCipher)
{
    ...
    config.get_network_config().get_ssl_config().set_cipher_list(std::string("HIGH:!") +
                                                                 unsupportedCipher);

    auto newCiphers = get_ciphers(std::move(config));

    for (std::vector<hazelcast::client::internal::socket::SSLSocket::CipherInfo>::const_iterator
           it = newCiphers.begin();
         it != newCiphers.end();
         ++it) {
...
public:
    ClusterTest()
      : ssl_factory_(g_srvFactory->get_server_address(), get_ssl_file_path())
    {}
...
protected:
                                                ...
                                                boost::latch& shutdown)
    {
        return lifecycle_listener()
          .on_starting([&starting]() { starting.count_down(); })
          .on_started([&started]() { started.count_down(); })
          .on_connected([&connected]() { connected.count_down(); })
          .on_disconnected([&disconnected]() { disconnected.count_down(); })
          .on_shutting_down([&shutting_down]() { shutting_down.count_down(); })
          .on_shutdown([&shutdown]() { shutdown.count_down(); });
    }
...
TEST_P(ClusterTest, testAllClientStates)
{
    ...
    auto listener = make_all_states_listener(startingLatch,
                                             startedLatch,
                                             connectedLatch,
                                             disconnectedLatch,
                                             shuttingDownLatch,
...
TEST_P(ClusterTest, testConnectionAttemptPeriod)
{
    ...
    clientConfig.get_network_config()
      .set_connection_attempt_period(std::chrono::milliseconds(900))
      .set_connection_timeout(std::chrono::seconds(2))
      .set_connection_attempt_limit(2);
...
#else
INSTANTIATE_TEST_SUITE_P(All, ClusterTest, ::testing::Values([]() {
...
socket_interceptor
make_socket_interceptor(boost::latch& l)
{
    return socket_interceptor().on_connect(
      [&l](const hazelcast::client::hz_socket& connected_sock) {
...
TEST_F(SocketOptionsTest, testConfiguration)
{
    ...
    clientConfig.get_network_config()
      ...
      .set_linger_seconds(5)
      .set_buffer_size_in_bytes(bufferSize);

    hazelcast_client client(std::move(clientConfig));

    config::socket_options& socketOptions =
      client.get_client_config().get_network_config().get_socket_options();
...
{
    std::shared_ptr<pn_counter> pnCounter =
      client->get_pn_counter(testing::UnitTest::GetInstance()->current_test_info()->name());
...
protected:
    ...
    void terminate_member(const member& address, HazelcastServer& server1, HazelcastServer& server2)
    {
...
protected:
    membership_listener make_membership_listener(boost::latch& joined, boost::latch& left)
    {
        return membership_listener()
          .on_joined([&joined](const membership_event&) { joined.count_down(); })
          .on_left([&left](const membership_event&) { left.count_down(); });
    }
...
TEST_P(SimpleListenerTest, testDeregisterListener)
{
    ...
    ASSERT_FALSE(
      map->remove_entry_listener(spi::ClientContext(hazelcastClient).random_uuid()).get());
...
INSTANTIATE_TEST_SUITE_P(All,
                         ...
                         ::testing::Values([]() { return ClientTestSupportBase::get_config(); },
                                           []() {
                                               return ClientTestSupportBase::get_config(false,
                                                                                        false);
                                           }));
} // namespace test
} // namespace client
...
protected:
    ...
    {
        ASSERT_TRUE(client);
        flake_id_generator_ = client->get_flake_id_generator(
...
TEST_F(ClientTxnMapTest, testPutGet)
{
    ...
    ASSERT_EQ("value1",
              (client_.get_map(name)->get<std::string, std::string>("key1").get().value()));
}
...
TEST_F(ClientTxnMapTest, testReplace)
{
    ...
    ASSERT_EQ("value1",
              (map->replace<std::string, std::string>("key1", "myNewValue").get().value()));

    context.commit_transaction().get();

    ASSERT_EQ("myNewValue",
              (client_.get_map(name)->get<std::string, std::string>("key1").get().value()));
}
...
//                    try {
//                        final TransactionalMap<String, Integer> txMap =
//                        context.getMap("testTxnGetForUpdate"); txMap->getForUpdate("var");
//                        latch1.count_down();
...
membership_listener
make_member_removed_listener(boost::latch& l)
{
    return membership_listener().on_left([&l](const membership_event&) { l.count_down(); });
}
...
void
test_transactional_offer_poll2_thread(hazelcast::util::ThreadArgs& args)
{
...
TEST_F(DataInputTest, testReadIntegerArray)
{
    std::vector<byte> bytes{
        0x00, 0x00, 0x00, 0x02, 0x12, 0x34, 0x56, 0x78, 0x1A, 0xBC, 0xDE, 0xEF
    };
    serialization::pimpl::data_input<std::vector<byte>> dataInput(bytes);
...
{
    std::vector<byte> bytes{ 0x00, 0x00, 0x00, 0x02, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                             0xDE, 0xEF, 0x11, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8 };
    serialization::pimpl::data_input<std::vector<byte>> dataInput(bytes);
...
{
    hazelcast::util::AddressHolder addressHolder =
      hazelcast::util::AddressUtil::get_address_holder("[fe80::62c5:*:fe05:480a%en0]:8080");
...
TEST_F(BitsTest, testLittleEndian)
{
    ...
    {
        int32_t expected = 0x1A * (int32_t)threeBytesFactor + 0x9A * (int32_t)twoBytesFactor +
                           0xAA * (int32_t)oneByteFactor + 0xBA;
        int32_t actual;
        byte* resBytes = (byte*)&actual;
    ...
    {
        int64_t expected = 0x1A * sevenBytesFactor + 0x2A * sixBytesFactor +
                           0x3A * fiveBytesFactor + 0x4A * fourBytesFactor +
                           0x5A * threeBytesFactor + 0x6A * twoBytesFactor + 0x7A * oneByteFactor +
                           0x8A;
...

    {
        ...
        ASSERT_EQ(0xA1 * threeBytesFactor + 0xDE * twoBytesFactor + 0xBC * oneByteFactor + 0xAB,
                  actual);
    }

    {
        ...
        ASSERT_EQ(0xA1 * sevenBytesFactor + 0x4B * sixBytesFactor + 0x5B * fiveBytesFactor +
                    0x6B * fourBytesFactor + 0x7B * threeBytesFactor + 0x8B * twoBytesFactor +
                    0x9B * oneByteFactor + 0xAB,
                  actual);
    }
}
} // namespace util
...
{
    std::vector<byte> bytes = { 0x00, 0x00, 0x00, 0x02, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC,
                                0xDE, 0xEF, 0x01, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8 };
...
{
    SyncHttpsClient httpsClient(
      "ec2.us-east-1.amazonaws.com",
      "/?Action=DescribeInstances&Version=2014-06-15&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-"
      "Credential=AKIAIU5IAVNR6X75ARYQ%2F20170413%2Fus-east-1%2Fec2%2Faws4_request&X-Amz-Date="
      "20170413T083821Z&X-Amz-Expires=30&X-Amz-Signature="
      "dff261333170c81ecb21f3a0d5820147233197a32c&X-Amz-SignedHeaders=host");
    try {
