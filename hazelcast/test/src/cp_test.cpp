...
protected:
    ...
    {
        if (std::string(testing::UnitTest::GetInstance()->current_test_suite()->name()) ==
            "basic_sessionless_semaphore_test") {
            factory = new HazelcastServerFactory(
              "hazelcast/test/resources/hazelcast-cp-sessionless-semaphore.xml");
        } else {
...
template<typename T>
HazelcastServerFactory* cp_test<T>::factory = nullptr;
template<typename T>
HazelcastServer* cp_test<T>::server1 = nullptr;
template<typename T>
HazelcastServer* cp_test<T>::server2 = nullptr;
template<typename T>
HazelcastServer* cp_test<T>::server3 = nullptr;
...
TEST_F(basic_atomic_long_test, test_use_after_destroy)
{
    ...
    ASSERT_THROW(cp_structure_->increment_and_get().get(), exception::distributed_object_destroyed);
}
...
TEST_F(basic_latch_test, test_wait_until)
{
    ...
    ASSERT_EQ(
      ...
      cp_structure_->wait_until(std::chrono::steady_clock::now() + std::chrono::seconds(120))
        .get());
}
...
protected:
    ...
    void test_when_session_closed(std::function<void()> f)
    {
        ...
        auto session_id =
          spi::ClientContext(*client_).get_proxy_session_manager().get_session(group_id);
...
TEST_F(basic_lock_test, test_lock_unlock_then_lock)
{
    ...
    auto f = std::async([=]() { return cp_structure_->lock_and_get_fence().get(); });
...
TEST_F(basic_lock_test, test_lock_auto_release_on_client_shutdown)
{
    ...
    ASSERT_TRUE_EVENTUALLY(
      (remoteController->executeOnController(
         response, factory->get_cluster_id(), script.str().c_str(), Lang::JAVASCRIPT),
...
TEST_F(basic_sessionless_semaphore_test, test_acquire_when_no_permits)
{
    ...
    auto f = std::async([=]() { cp_structure_->acquire().get(); });

    ASSERT_TRUE_ALL_THE_TIME(
      (f.wait_for(std::chrono::seconds::zero()) == std::future_status::timeout &&
       cp_structure_->available_permits().get() == 0),
      3);
...
TEST_F(basic_sessionless_semaphore_test, test_multiple_acquire_when_not_enough_permits)
{
    ...
    ASSERT_TRUE_ALL_THE_TIME(
      (f.wait_for(std::chrono::seconds::zero()) == std::future_status::timeout &&
       cp_structure_->available_permits().get() == number_of_permits),
      3);
...
TEST_F(basic_sessionless_semaphore_test, test_try_acquire_until)
{
    ...
    for (int32_t i = 0; i < number_of_permits; ++i) {
        ...
        ASSERT_TRUE(
          cp_structure_
            ->try_acquire_until(std::chrono::steady_clock::now() + std::chrono::seconds(120))
            .get());
    }
...
{
    ASSERT_FALSE(
      cp_structure_->try_acquire_until(std::chrono::steady_clock::now() + std::chrono::seconds(2))
        .get());
}
...
TEST_F(basic_session_semaphore_test, test_acquire_after_release)
{
    ...
    auto f = std::async([=]() { ASSERT_NO_THROW(cp_structure_->release().get()); });
