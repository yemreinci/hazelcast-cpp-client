...
protected:
    entry_listener make_add_remove_listener(boost::latch& added_latch, boost::latch& removed_latch)
    {
        return entry_listener()
          .on_added([&added_latch](entry_event&&) { added_latch.count_down(); })
          .on_removed([&removed_latch](entry_event&&) { removed_latch.count_down(); });
    }
...
#ifdef HZ_BUILD_WITH_SSL
        sslFactory =
          new HazelcastServerFactory(g_srvFactory->get_server_address(), get_ssl_file_path());
...
protected:
    ...
    void TearDown() override { q->clear().get(); }
...
TEST_F(ClientQueueTest, testListener)
{
    ...
    auto listener =
      item_listener().on_added([&latch1](item_event&& item_event) { latch1.count_down(); });
...
void
test_offer_poll_thread2(hazelcast::util::ThreadArgs& args)
{
...
namespace tasks {
bool
SelectAllMembers::select(const hazelcast::client::member& member) const
{
    return true;
}

bool
SelectNoMembers::select(const hazelcast::client::member& member) const
{
    return false;
}
} // namespace tasks
...
protected:
    ...
    {
        factory =
          new HazelcastServerFactory(g_srvFactory->get_server_address(),
    ...
    {
    public:
        FailingExecutionCallback(const std::shared_ptr<boost::latch>& latch1)
          : latch1_(latch1)
        {}
    ...
    {
    public:
        SuccessfullExecutionCallback(const std::shared_ptr<boost::latch>& latch1)
          : latch1_(latch1)
        {}
    ...
    {
    public:
        explicit ResultSettingExecutionCallback(const std::shared_ptr<boost::latch>& latch1)
          : latch1_(latch1)
        {}
        ...
        boost::optional<T> get_result() { return result_.get(); }
    ...
    public:
        MultiExecutionCompletionCallback(std::string msg,
                                         ...
                                         const std::shared_ptr<boost::latch>& complete_latch)
          : msg_(std::move(msg))
          , response_latch_(std::move(response_latch))
          , complete_latch_(complete_latch)
        {}

        void on_response(const member& member,
                         const boost::optional<std::string>& response) override
        {
        ...
        void on_failure(const member& member, std::exception_ptr exception) override {}
    ...
    public:
        MultiExecutionNullCallback(std::shared_ptr<boost::latch> response_latch,
          ...
          : response_latch_(std::move(response_latch))
          , complete_latch_(std::move(complete_latch))
        {}
...
TEST_F(ClientExecutorServiceTest, testSubmitFailingCallableReasonExceptionCause)
{
    ...
    auto failingFuture =
      service
        ->submit<executor::tasks::FailingCallable, std::string>(executor::tasks::FailingCallable())
        .get_future();
...
TEST_F(ClientExecutorServiceTest, testExecute_withNoMemberSelected)
{
    ...
    ASSERT_THROW(
      service->execute<executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>>(
        executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>{
          mapName, spi::ClientContext(*client).random_uuid() },
        selector),
...
TEST_F(ClientExecutorServiceTest, testCallableSerializedOnce)
{
    ...
    auto future =
      service
        ->submit_to_key_owner<executor::tasks::SerializedCounterCallable, int, std::string>(
...
TEST_F(ClientExecutorServiceTest, testCallableSerializedOnce_submitToAddress)
{
    ...
    auto future = service
                    ->submit_to_member<executor::tasks::SerializedCounterCallable, int>(
...
TEST_F(ClientExecutorServiceTest, testUnserializableResponse_exceptionPropagatesToClient)
{
    ...
    auto future = service
                    ->submit<executor::tasks::TaskWithUnserializableResponse, bool>(
...
TEST_F(ClientExecutorServiceTest, testSubmitCallableToMember)
{
    ...
    auto future =
      service
        ->submit_to_member<executor::tasks::GetMemberUuidTask, boost::uuids::uuid>(task, members[0])
        .get_future();
...
TEST_F(ClientExecutorServiceTest, testSubmitCallableToMembers)
{
    ...
    auto futuresMap =
      service->submit_to_members<executor::tasks::GetMemberUuidTask, boost::uuids::uuid>(task,
                                                                                         members);
...
TEST_F(ClientExecutorServiceTest, testSubmitCallable_withMemberSelector)
{
    ...
    auto f = service->submit<executor::tasks::AppendCallable, std::string>(callable, selectAll)
               .get_future();
...
TEST_F(ClientExecutorServiceTest, submitCallableToAllMembers)
{
    ...
    auto futuresMap =
      service->submit_to_all_members<executor::tasks::AppendCallable, std::string>(callable);
...
TEST_F(ClientExecutorServiceTest, submitCallableToMember_withExecutionCallback)
{
    ...
    executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid> callable(
      testName, spi::ClientContext(*client).random_uuid());

    std::shared_ptr<boost::latch> latch1(new boost::latch(1));
    std::shared_ptr<SuccessfullExecutionCallback> callback(
      new SuccessfullExecutionCallback(latch1));

    std::vector<member> members = client->get_cluster().get_members();
    ASSERT_EQ(numberOfMembers, members.size());

    service->submit_to_member<executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>,
                              boost::uuids::uuid>(callable, members[0], callback);
...
TEST_F(ClientExecutorServiceTest, submitCallableToMember_withMultiExecutionCallback)
{
    ...
    service->submit_to_members<executor::tasks::AppendCallable, std::string>(
      callable, members, callback);
...
TEST_F(ClientExecutorServiceTest, submitCallable_withExecutionCallback)
{
    ...
    service->submit<executor::tasks::AppendCallable, std::string>(
      callable, selector, std::static_pointer_cast<execution_callback<std::string>>(callback));
...
TEST_F(ClientExecutorServiceTest,
       submitCallableWithNullResultToAllMembers_withMultiExecutionCallback)
{
    std::shared_ptr<iexecutor_service> service = client->get_executor_service(get_test_name());
...
TEST_F(ClientExecutorServiceTest, testSubmitCallable)
{
    ...
    auto result =
      service->submit<executor::tasks::AppendCallable, std::string>(callable).get_future();
...
TEST_F(ClientExecutorServiceTest, testSubmitCallable_withExecutionCallback)
{
    ...
    service->submit<executor::tasks::AppendCallable, std::string>(
      callable, std::static_pointer_cast<execution_callback<std::string>>(callback));
...
TEST_F(ClientExecutorServiceTest, submitCallableToKeyOwner)
{
    ...
    auto f = service
               ->submit_to_key_owner<executor::tasks::AppendCallable, std::string, std::string>(
                 callable, "key")
               .get_future();
...
TEST_F(ClientExecutorServiceTest, submitCallableToKeyOwner_withExecutionCallback)
{
    ...
    service->submit_to_key_owner<executor::tasks::AppendCallable, std::string, std::string>(
      callable, "key", std::static_pointer_cast<execution_callback<std::string>>(callback));
...
TEST_F(ClientExecutorServiceTest, submitCallablePartitionAware)
{
    ...
    auto f = service
               ->submit<executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>,
                        boost::uuids::uuid>(callable)
               .get_future();
...
TEST_F(ClientExecutorServiceTest, submitCallablePartitionAware_WithExecutionCallback)
{
    ...
    service->submit<executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>,
                    boost::uuids::uuid>(callable, callback);

    ASSERT_OPEN_EVENTUALLY(*latch1);
    auto value =
      std::static_pointer_cast<ResultSettingExecutionCallback<boost::uuids::uuid>>(callback)
        ->get_result();
    ASSERT_TRUE(value);
...
TEST_F(ClientExecutorServiceTest, testExecute)
{
    ...
    service->execute(executor::tasks::MapPutPartitionAwareCallable<std::string>(testName, "key"));
...
TEST_F(ClientExecutorServiceTest, testExecuteOnKeyOwner)
{
    ...
    service->execute_on_key_owner<executor::tasks::MapPutPartitionAwareCallable<boost::uuids::uuid>,
                                  boost::uuids::uuid>(callable, key);
...
TEST_F(ClientExecutorServiceTest, testExecuteOnMembers)
{
    ...
    ASSERT_TRUE_EVENTUALLY(map->contains_key(members[0].get_uuid()).get() &&
                           map->contains_key(members[1].get_uuid()).get());
}
...
TEST_F(AwsClientTest, testClientAwsMemberNonDefaultPortConfig)
{
    ...
    clientConfig.get_network_config()
      .get_aws_config()
      .set_enabled(true)
      .set_access_key(std::getenv("AWS_ACCESS_KEY_ID"))
...
TEST_F(AwsClientTest, testClientAwsMemberWithSecurityGroupDefaultIamRole)
{
    client_config clientConfig;
    clientConfig.set_property(client_properties::PROP_AWS_MEMBER_PORT, "60000");
    clientConfig.get_network_config().get_aws_config().set_enabled(true).set_security_group_name(
      "launch-wizard-147");
...
TEST_F(DescribeInstancesTest, testDescribeInstancesTagAndNonExistentValueSet)
{
    ...
    awsConfig.set_enabled(true)
      ...
      .set_secret_key(getenv("AWS_SECRET_ACCESS_KEY"))
      .set_tag_key("aws-test-tag")
      .set_tag_value("non-existent-value");
    client::aws::impl::DescribeInstances desc(awsConfig, awsConfig.get_host_header(), get_logger());
    std::unordered_map<std::string, std::string> results = desc.execute();
...
TEST_F(DescribeInstancesTest, testDescribeInstancesNonExistentSecurityGroup)
{
    ...
    awsConfig.set_enabled(true)
      .set_access_key(getenv("AWS_ACCESS_KEY_ID"))
      .set_secret_key(getenv("AWS_SECRET_ACCESS_KEY"))
      .set_security_group_name("non-existent-group");
    client::aws::impl::DescribeInstances desc(awsConfig, awsConfig.get_host_header(), get_logger());
    std::unordered_map<std::string, std::string> results = desc.execute();
    ASSERT_TRUE(results.empty());
}
...
TEST_F(CloudUtilityTest, testUnmarshallResponseXml)
{
    ...
    std::unordered_map<std::string, std::string> results =
      hazelcast::client::aws::utility::cloud_utility::unmarshal_the_response(responseStream,
                                                                             get_logger());
