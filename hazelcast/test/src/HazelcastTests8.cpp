...
protected:
    ...
    std::shared_ptr<imap> get_near_cached_map_from_client(config::near_cache_config config)
    {
...
TEST_F(ClientMapNearCacheTest, testRemoveAllNearCache)
{
    ...
    map
      ->remove_all(query::equal_predicate(*client_, query::query_constants::KEY_ATTRIBUTE_NAME, 20))
      .get();
...
protected:
    ...
    {
        ListenerState(int latch_count, int64_t start_sequence)
          : latch1(latch_count)
          , start_sequence(start_sequence)
          , number_of_messages_received(0)
        {}

        explicit ListenerState(int latch_count)
          : ListenerState(latch_count, -1)
        {}
...
protected:
    ...
    public:
        ...
        void print() const
        {
            std::cerr << "Total = " << total() << ", puts = " << put_count
                      << " , gets = " << get_count << " , removes = " << remove_count << std::endl;
        }

        int total() const { return (int)get_count + (int)put_count + (int)remove_count; }
    };
    ...
    {
    public:
        Task(Stats& stats, std::shared_ptr<imap> map, std::shared_ptr<logger> lg)
          : stats_(stats)
          , map_(map)
          , logger_(std::move(lg))
        {}
        ...
        void run()
        {
            ...
            while (running) {
                ...
                try {
                    ...
                    HZ_LOG(
                      *logger_, warning, std::string("[SimpleMapTest io_exception] ") + e.what());
    ...
    static void start(const HazelcastServer& server)
    {
        ...
        std::cerr << " Remove Percentage: " << (100 - (PUT_PERCENTAGE + GET_PERCENTAGE))
                  << std::endl;
        client_config clientConfig;
        ...
        clientConfig.get_network_config()
          .add_address(address(member.host, member.port))
          .set_connection_attempt_period(std::chrono::seconds(10));

        Stats stats;
        auto lg = std::make_shared<logger>(
          "SimpleMapTest", "SimpleMapTest", logger::level::finest, logger::default_handler);
...
ClientTestSupport::ClientTestSupport()
{
    ...
    test_name_ = out.str();
    logger_ =
      std::make_shared<logger>("Test", test_name_, logger::level::info, logger::default_handler);
}

logger&
ClientTestSupport::get_logger()
{
    return *logger_;
}

const std::string&
ClientTestSupport::get_test_name() const
{
    return test_name_;
}
...
TEST_F(ClientMessageTest, testFragmentedMessageHandling)
{
    ...
    auto datas_opt = handler.msg->get_first_var_sized_field<
