...
protected:
    ...
    void put_all_entries_into_map(std::shared_ptr<replicated_map> map)
    {
        ...
        execute_for_each(
          [=, &mapTest](size_t i) { mapTest[std::string("foo-") + std::to_string(i)] = "bar"; });
    ...
    void put_entries_into_map(const std::shared_ptr<replicated_map>& map)
    {
        execute_for_each([=](size_t i) {
            auto oldEntry =
              map->put<std::string, std::string>(std::string("foo-") + std::to_string(i), "bar")
                .get();
    ...
    {
        execute_for_each([&](size_t i) {
            map->put<std::string, std::string>(std::string("foo-") + std::to_string(i), value)
              .get();
        });
    }
    ...
    static void SetUpTestCase()
    {
        factory = new HazelcastServerFactory(
          g_srvFactory->get_server_address(),
    ...
    static client_config get_client_config_with_near_cache_invalidation_enabled()
    {
        ...
        return std::move(get_config()
                           .set_cluster_name("replicated-map-binary-test")
                           .add_near_cache_config(nearCacheConfig));
    }
...
TEST_F(ClientReplicatedMapTest, testRemove)
{
    ...
    execute_for_each([=](size_t index) {
        auto val =
          map2->remove<std::string, std::string>(std::string("foo-") + std::to_string(index)).get();
        ASSERT_TRUE(val.has_value());
        ASSERT_EQ("bar", val.value());
    });
...
{
    static int32_t get_factory_id() { return 5; }

    static int32_t get_class_id() { return 6; }

    static void write_portable(test::ClientReplicatedMapTest::SamplePortable object,
                               serialization::portable_writer& out)
    {
        out.write<int32_t>("a", object.a);
    }

    static test::ClientReplicatedMapTest::SamplePortable read_portable(
      serialization::portable_reader& reader)
    {
...
protected:
    ...
    entry_listener make_event_counting_listener(ListenerState& state)
    {
        ...
        return entry_listener()
          ...
          .on_map_evicted([&state](map_event&&) { ++state.map_evict_count; })
          .on_map_cleared([&state](map_event&&) { ++state.map_clear_count; });
    }
...
TEST_F(ClientReplicatedMapListenerTest, testListenToKeyForEntryAdded)
{
    ...
    ASSERT_TRUE_EVENTUALLY(state_.keys.size() == 1U && state_.keys.pop() == 1 &&
                           state_.add_count.load() == 1);
}
...
TEST_F(ClientReplicatedMapListenerTest, testListenWithPredicate)
{
    auto replicatedMap = client->get_replicated_map(get_test_name());
    replicatedMap
      ->add_entry_listener(make_event_counting_listener(state_), query::false_predicate(*client))
      .get();
    replicatedMap->put(2, 2).get();
...
public:
    ...
    {
        near_cache_config_ =
          NearCacheTestUtils::create_near_cache_config(GetParam(), get_test_name());
    }
...
protected:
    ...
    public:
        ...
        {
            assert_near_cache_stats(
              stats, expected_owned_entry_count, expected_hits, expected_misses, 0, 0);
        }
    ...
    void create_near_cache_context()
    {
        ...
        near_cache_ =
          near_cache_manager_->get_near_cache<serialization::pimpl::data,
                                              ...
                                              serialization::pimpl::data>(get_test_name());
    ...
    void test_contains_key(bool use_near_cached_map_for_removal)
    {
        ...
        std::shared_ptr<replicated_map>& adapter =
          use_near_cached_map_for_removal ? near_cached_map_ : no_near_cache_map_;
    ...
    bool check_contain_keys()
    {
        return !near_cached_map_->contains_key(1).get() &&
               near_cached_map_->contains_key(2).get() && near_cached_map_->contains_key(3).get() &&
               !near_cached_map_->contains_key(5).get();
    }

    void assert_near_cache_invalidation_requests(monitor::near_cache_stats& stat,
                                                 int64_t invalidation_requests)
    {
        if (near_cache_config_.is_invalidate_on_change() && invalidation_requests > 0) {
            ...
            ASSERT_EQ_EVENTUALLY(invalidation_requests,
    ...
    void populate_map()
    {
        for (int i = 0; i < DEFAULT_RECORD_COUNT; i++) {
            no_near_cache_map_->put<int, std::string>(i, std::string("value-") + std::to_string(i))
              .get();
        }
    ...
    int64_t get_expected_misses_with_local_update_policy()
    {
        if (near_cache_config_.get_local_update_policy() == config::near_cache_config::CACHE) {
            // we expect the first and second get() to be hits, since the value should be already be
            // cached
    ...
    std::shared_ptr<hazelcast::client::internal::nearcache::NearCache<serialization::pimpl::data,
                                                                      serialization::pimpl::data>>
      near_cache_;
 ...
 *
 * This variant uses the {@link NearCacheTestContext#nearCacheAdapter}, so there is no Near Cache
 * invalidation necessary.
 */
 ...
 *
 * This variant uses the noNearCacheMap(client with no near cache), so we need to configure Near
 * Cache invalidation.
 */
...
/**
 * Checks that the Near Cache never returns its internal {@link NearCache#NULL_OBJECT} to the public
 * API.
 */
       ...
       whenEmptyMap_thenPopulatedNearCacheShouldReturnNull_neverNULLOBJECT)
{
    ...
    for (int i = 0; i < DEFAULT_RECORD_COUNT; i++) {
        ...
        if (value.get() != NULL) {
            ...
            ASSERT_EQ(
              (hazelcast::client::internal::nearcache::
                 NearCache<serialization::pimpl::data, serialization::pimpl::data>::NULL_OBJECT),
              near_cache_->get(key))
 ...
 * even if the Near Cache is full and the eviction is disabled (via {@link
 * com.hazelcast.config.EvictionPolicy#NONE}.
 *
 * This variant uses the {@link NearCacheTestContext#nearCacheAdapter}, so there is no Near Cache
 * invalidation necessary.
 */
       ...
       whenCacheIsFull_thenPutOnSameKeyShouldUpdateValue_withUpdateOnNearCacheAdapter)
{
    ...
    NearCacheTestUtils::set_eviction_config(
      near_cache_config_, config::NONE, config::eviction_config::ENTRY_COUNT, size);
...
/**
 * Checks that the Near Cache updates value for keys which are already in the Near Cache,
 * even if the Near Cache is full an the eviction is disabled (via {@link
 * com.hazelcast.config.EvictionPolicy#NONE}.
 *
 * This variant uses the {@link NearCacheTestContext#dataAdapter}, so we need to configure Near
 * Cache invalidation.
 */
TEST_P(BasicClientReplicatedMapNearCacheTest,
...
TEST_P(BasicClientReplicatedMapNearCacheTest, testNearCacheStats)
{
    ...
    populate_near_cache();
    {
        ...
        NearCacheTestUtils::assert_near_cache_stats(
          *stats_, DEFAULT_RECORD_COUNT, DEFAULT_RECORD_COUNT, DEFAULT_RECORD_COUNT);
    }
}
...
{
    NearCacheTestUtils::set_eviction_config(
      near_cache_config_, config::LRU, config::eviction_config::ENTRY_COUNT, DEFAULT_RECORD_COUNT);
    create_no_near_cache_context();
    ...
    populate_map();
    no_near_cache_map_
      ->put(DEFAULT_RECORD_COUNT, std::string("value-") + std::to_string(DEFAULT_RECORD_COUNT))
      .get();
    ...
    ASSERT_GE(evictions, expectedEvictions)
      << "Near Cache eviction count didn't reach the desired value (" << expectedEvictions
      << " vs. " << evictions << ") (" << stats_->to_string();

    SCOPED_TRACE("testNearCacheEviction");

    NearCacheTestUtils::assert_near_cache_stats(
      *stats_, expectedOwnedEntryCount, expectedHits, expectedMisses, expectedEvictions, 0);
}

INSTANTIATE_TEST_SUITE_P(ReplicatedMapNearCacheTest,
                         BasicClientReplicatedMapNearCacheTest,
...
protected:
    ...
    static config::near_cache_config new_near_cache_config() { return config::near_cache_config(); }
      ...
      config::near_cache_config config)
    {
        ...
        client_ =
          std::unique_ptr<hazelcast_client>(new hazelcast_client(std::move(*client_config_)));
    ...
    std::shared_ptr<monitor::near_cache_stats> get_near_cache_stats(replicated_map& rep_map)
    {
        ...
        auto nearCache =
          nearCacheManager->get_near_cache<serialization::pimpl::data,
                                           serialization::pimpl::data,
                                           serialization::pimpl::data>(rep_map.get_name());
...
TEST_F(ClientTopicTest, testTopicListeners)
{
    ...
    auto id = topic_
                ->add_message_listener(topic::listener().on_received(
                  [&latch1](topic::message&&) { latch1.count_down(); }))
                .get();
