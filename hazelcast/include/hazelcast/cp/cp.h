...
{
public:
    session_aware_proxy(const std::string& service_name,
                        const std::string& proxy_name,
                        client::spi::ClientContext* context,
                        const raft_group_id& group_id,
                        const std::string& object_name,
...
private:
    ...
    boost::future<int64_t> alter_data(data& function_data, alter_result_type result_type);
...
public:
    ...
    boost::future<boost::optional<typename std::remove_pointer<T>::type>> alter_and_get(
      const F& function)
    {
        return to_object<typename std::remove_pointer<T>::type>(
    ...
    template<typename T, typename F>
    boost::future<boost::optional<typename std::remove_pointer<T>::type>> get_and_alter(
      const F& function)
    {
        return to_object<typename std::remove_pointer<T>::type>(
...
private:
    ...
    boost::future<boost::optional<data>> invoke_apply(const data function_data,
...
public:
     ...
     *
     * @return true only if the internal counter has reached zero. This function may spuriously
     * return false with very low probability even if the internal counter has reached zero.
     */
     ...
     *
     * Blocks the calling thread until the internal counter reaches 0 or server thread is
     * interrupted (e.g. cluster shutdown). If it is zero already, returns immediately.
     *
 ...
 * stl \counting_semaphore (@see https://en.cppreference.com/w/cpp/thread/counting_semaphore).
 * Semaphores are often used to restrict the number of threads than can access some physical or
 * logical resource. <p> \counting_semaphore is a cluster-wide counting semaphore. Conceptually, it
 * maintains a set of permits. Each \acquire blocks if necessary until a permit is available, and
 * then takes it. Dually, each \release adds a permit, potentially releasing a blocking acquirer.
 * However, no actual permit objects are used; the semaphore just keeps a count of the number
 * available and acts accordingly. <p> Hazelcast's distributed semaphore implementation guarantees
 * that threads invoking any of the \acquire methods are selected to obtain permits in the order of
 * their invocations (first-in-first-out; FIFO). Note that FIFO ordering implies the order which the
 * primary replica of an \counting_semaphore receives these acquire requests. Therefore, it is
 ...
 * in the application.
 * <p>
 * \counting_semaphore works on top of the Raft consensus algorithm. It offers linearizability
 * during crash failures and network partitions. It is CP with respect to the CAP principle. If a
 * network partition occurs, it remains available on at most one side of the partition. <p> It has 2
 * variations: <ul> <li> The default impl is session-aware. In this one, when a caller makes its
 * very first \acquire call, it starts a new CP session with the underlying CP group. Then,
 * liveliness of the caller is tracked via this CP session. When the caller fails, permits acquired
 * by this HazelcastInstance are automatically and safely released. However, the session-aware
 * version comes with a limitation, that is, a HazelcastInstance cannot release permits before
 * acquiring them first. In other words, a Hazelcast client can release only the permits it has
 * acquired earlier. It means, you can acquire a permit from one thread and release it from another
 * thread using the same Hazelcast client, but not different instances of Hazelcast client. You can
 * use the session-aware CP \counting_semaphore impl by disabling server side JDK compatibility
 ...
 * only 1 permit (similar to stl \binary_semaphore). In this case, the semaphore works like a
 * non-reentrant lock. <p> All of the API methods in the new CP \fenced_lock impl offer the
 * exactly-once execution semantics for the session-aware version. For instance, even if a \release
 * call is internally retried because of a crashed Hazelcast member, the permit is released only
 * once. However, this guarantee is not given for the sessionless, a.k.a, JDK-compatible CP
 * \counting_semaphore. For this version, you can tune execution semantics via server side config
...
public:
     ...
     * @param rel_time the maximum time to wait for a permit. The supported resolution is
     * milliseconds.
    ...
    boost::future<bool> try_acquire_until(const std::chrono::time_point<Clock, Duration>& abs_time,
                                          int32_t permits = 1)
    {
        using namespace std::chrono;
        auto now = Clock::now();
...
protected:
    ...
    counting_semaphore(const std::string& proxy_name,
                       client::spi::ClientContext* context,
                       const raft_group_id& group_id,
                       const std::string& object_name,
                       internal::session::proxy_session_manager& session_manager);

    virtual boost::future<bool> try_acquire_for_millis(int32_t permits,
...
{
public:
    sessionless_semaphore(const std::string& proxy_name,
                          client::spi::ClientContext* context,
                          const raft_group_id& group_id,
                          const std::string& object_name,
                          internal::session::proxy_session_manager& session_manager);
...
protected:
    boost::future<bool> try_acquire_for_millis(int32_t permits,
...
{
public:
    session_semaphore(const std::string& proxy_name,
                      client::spi::ClientContext* context,
                      const raft_group_id& group_id,
                      const std::string& object_name,
                      internal::session::proxy_session_manager& session_manager);
...
private:
    ...
    template<typename T,
             typename = typename std::enable_if<std::is_same<atomic_long, T>::value ||
    ...
    std::shared_ptr<fenced_lock> create_fenced_lock(raft_group_id&& group_id,
                                                    const std::string& proxy_name,
                                                    const std::string& object_name);

    template<typename T,
             typename = typename std::enable_if<std::is_same<fenced_lock, T>::value>::type>
    ...
    std::shared_ptr<counting_semaphore> create_semaphore(raft_group_id&& group_id,
                                                         const std::string& proxy_name,
                                                         const std::string& object_name);

    template<typename T,
             typename = typename std::enable_if<std::is_same<counting_semaphore, T>::value>::type>
