...
public:
    ...
    {
        return proxy::ReplicatedMapImpl::add_entry_listener(std::shared_ptr<impl::BaseEventHandler>(
          new EntryEventHandler<protocol::codec::replicatedmap_addentrylistener_handler>(
            get_name(),
     ...
     */
    template<typename K>
    typename std::enable_if<!std::is_base_of<query::predicate, K>::value,
    ...
    add_entry_listener(entry_listener&& listener, const K& key)
    {
        return proxy::ReplicatedMapImpl::add_entry_listener_to_key(
          std::shared_ptr<impl::BaseEventHandler>(
            new EntryEventHandler<protocol::codec::replicatedmap_addentrylistenertokey_handler>(
              get_name(),
              get_context().get_client_cluster_service(),
              get_context().get_serialization_service(),
              std::move(listener),
     ...
     */
    template<typename P>
    typename std::enable_if<std::is_base_of<query::predicate, P>::value,
                            boost::future<boost::uuids::uuid>>::type
     ...
     * Changes on the map are <b>NOT</b> reflected on the collection on the <b>CLIENT</b> or vice
     * versa. The order of the elements is not guaranteed due to the internal asynchronous
     * replication behavior. If a specific order is needed, use
...
private:
    ...
    private:
        void fire_map_wide_event(const boost::optional<data>& key,
                                 const boost::optional<data>& value,
                                 const boost::optional<data>& old_value,
                                 const boost::optional<data>& merging_value,
                                 int32_t event_type,
                                 ...
                                 int32_t number_of_affected_entries)
        {
            ...
            map_event mapEvent(
              std::move(member).value(), mapEventType, instance_name_, number_of_affected_entries);
            listener_.map_cleared_(std::move(mapEvent));
        }

        void fire_entry_event(const boost::optional<data>& key,
                              const boost::optional<data>& value,
                              const boost::optional<data>& old_value,
                              const boost::optional<data>& merging_value,
                              int32_t event_type,
