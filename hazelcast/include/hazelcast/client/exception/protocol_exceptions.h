...
#define DEFINE_EXCEPTION_CLASS(ClassName, errorNo, isRuntime)                                      \
    ...
    {                                                                                              \
    public:                                                                                        \
        ClassName(const std::string& source,                                                       \
        ...
        {}                                                                                         \
        explicit ClassName(const std::string& message)                                             \
          : ClassName("", message)                                                                 \
        {}                                                                                         \
        ClassName()                                                                                \
          : ClassName("", "")                                                                      \
        {}                                                                                         \
        ClassName(const std::string& errorName,                                                    \
                  int32_t errorCode,                                                               \
...
DEFINE_EXCEPTION_CLASS(hazelcast_instance_not_active,
                       protocol::HAZELCAST_INSTANCE_NOT_ACTIVE,
                       true);
...
/**
 * Raised at Ringbuffer::readOne if the provided sequence is smaller then Ringbuffer::headSequence()
 * is passed.
 */
...
DEFINE_EXCEPTION_CLASS(replicated_map_cant_be_created_on_lite_member,
                       protocol::REPLICATED_MAP_CANT_BE_CREATED,
                       true);
...
DEFINE_EXCEPTION_CLASS(indeterminate_operation_state,
                       protocol::INDETERMINATE_OPERATION_STATE,
                       true);

DEFINE_EXCEPTION_CLASS(node_id_out_of_range,
                       protocol::FLAKE_ID_NODE_ID_OUT_OF_RANGE_EXCEPTION,
                       true);
...
DEFINE_EXCEPTION_CLASS(lock_acquire_limit_reached,
                       protocol::LOCK_ACQUIRE_LIMIT_REACHED_EXCEPTION,
                       true);
...
{
public:
    retryable_hazelcast(const std::string& error_name,
                        int32_t error_code,
                        const std::string& source,
                        const std::string& message,
                        const std::string& details,
    ...
    explicit retryable_hazelcast(const std::string& source = "",
                                 const std::string& message = "",
                                 const std::string& details = "",
                                 std::exception_ptr cause = nullptr);
};
...
{
public:
    explicit member_left(const std::string& source = "",
                         const std::string& message = "",
                         const std::string& details = "",
                         std::exception_ptr cause = nullptr);
};
...
{
public:
    explicit consistency_lost(const std::string& source = "",
                              const std::string& message = "",
                              const std::string& details = "",
                              std::exception_ptr cause = nullptr);
};
...
DEFINE_EXCEPTION_CLASS(client_not_allowed_in_cluster,
                       protocol::CLIENT_NOT_ALLOWED_IN_CLUSTER,
                       true);
