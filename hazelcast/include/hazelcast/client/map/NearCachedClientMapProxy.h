...
protected:
    ...
    void on_initialize() override
    {
        ...
        internal::nearcache::NearCacheManager& nearCacheManager =
          this->get_context().get_near_cache_manager();
        ...
        near_cache_ =
          impl::nearcache::InvalidationAwareWrapper<serialization::pimpl::data,
                                                    V>::as_invalidation_aware(near_cache_,
                                                                              partitionCount);
    ...
    boost::future<bool> contains_key_internal(const serialization::pimpl::data& key_data) override
    {
        ...
        if (cached) {
            return boost::make_ready_future(internal::nearcache::NearCache<K, V>::NULL_OBJECT !=
                                            cached);
        }
    ...
    boost::future<bool> remove_internal(const serialization::pimpl::data& key,
                                        const serialization::pimpl::data& value) override
    {
        try {
    ...
    boost::future<protocol::ClientMessage> remove_all_internal(
      const serialization::pimpl::data& predicate_data) override
    {
        try {
    ...
    boost::future<protocol::ClientMessage> delete_internal(
    ...
    boost::future<protocol::ClientMessage> try_put_transient_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      std::chrono::milliseconds ttl) override
    {
    ...
    boost::future<boost::optional<serialization::pimpl::data>> put_if_absent_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      std::chrono::milliseconds ttl) override
    {
    ...
    boost::future<bool> replace_if_same_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
    ...
    boost::future<protocol::ClientMessage> set_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      std::chrono::milliseconds ttl) override
    {
    ...
    boost::future<EntryVector> get_all_internal(
      int partition_id,
      const std::vector<serialization::pimpl::data>& partition_keys) override
    {
        ...
        try {
            ...
            return imap::get_all_internal(partition_id, remainingKeys)
              .then(boost::launch::deferred, [=](boost::future<EntryVector> f) {
                  EntryVector allEntries(result);
                  for (auto& entry : f.get()) {
                      auto key =
                        std::make_shared<serialization::pimpl::data>(std::move(entry.first));
                      auto value =
                        std::make_shared<serialization::pimpl::data>(std::move(entry.second));
                      bool marked = false;
    ...
    boost::future<boost::optional<serialization::pimpl::data>> execute_on_key_internal(
      const serialization::pimpl::data& key_data,
    ...
    boost::future<protocol::ClientMessage> put_all_internal(int partition_id,
                                                            const EntryVector& entries) override
    {
        try {
...
private:
    impl::nearcache::KeyStateMarker* get_key_state_marker()
    {
        return std::static_pointer_cast<
                 impl::nearcache::InvalidationAwareWrapper<serialization::pimpl::data, V>>(
                 near_cache_)
          ->get_key_state_marker();
    }

    void add_near_cache_invalidate_listener(
      std::shared_ptr<client::impl::BaseEventHandler>& handler)
    {
        try {
            invalidation_listener_id_ =
              proxy::ProxyImpl::register_listener(create_near_cache_entry_listener_codec(), handler)
                .get();
    ...
    public:
        ...
        ClientMapAddNearCacheEventHandler(
          const std::shared_ptr<internal::nearcache::NearCache<serialization::pimpl::data, V>>&
            cache)
          : near_cache_(cache)
        {}
        ...
        void handle_imapinvalidation(const boost::optional<data>& key,
    ...
    public:
        ...
        {
            return protocol::codec::map_addnearcacheinvalidationlistener_encode(
              name_, static_cast<int32_t>(listener_flags_), local_only);
        }
        ...
        NearCacheEntryListenerMessageCodec(const std::string& name,
    ...
    {
        for (MARKER_MAP::const_iterator it = markers.begin(); it != markers.end(); ++it) {
            if (it->second) {
    ...
    /*
                    void tryToPutNearCache(const std::shared_ptr<serialization::pimpl::data>
       &key_data, const std::shared_ptr<serialization::pimpl::data> &response) {
