...
public:
    ...
    InvalidationAwareWrapper(std::shared_ptr<internal::nearcache::NearCache<K, V>> cache,
                             int partition_count)
      : near_cache_(cache)
    ...
    void initialize() override { near_cache_->initialize(); }

    const std::string& get_name() const override { return near_cache_->get_name(); }

    std::shared_ptr<V> get(const std::shared_ptr<K>& key) override { return near_cache_->get(key); }
    ...
    */
    ...
    int size() const override { return near_cache_->size(); }

    KeyStateMarker* get_key_state_marker() { return key_state_marker_.get(); }
