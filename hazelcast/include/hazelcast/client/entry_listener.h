...
public:
     ...
     * \param h a `void` function object that is callable with a single parameter of type
     * `EntryEvent &&` \return `*this`
     */
...
private:
    ...
    EntryHandlerType added_ = util::noop<entry_event&&>, removed_ = util::noop<entry_event&&>,
                     updated_ = util::noop<entry_event&&>, evicted_ = util::noop<entry_event&&>,
                     expired_ = util::noop<entry_event&&>, merged_ = util::noop<entry_event&&>;

    using MapHandlerType = std::function<void(map_event&&)>;
    MapHandlerType map_evicted_ = util::noop<map_event&&>, map_cleared_ = util::noop<map_event&&>;

    int32_t flags_ = 0;

    void add_flag(entry_event::type t) { flags_ |= static_cast<int32_t>(t); }
