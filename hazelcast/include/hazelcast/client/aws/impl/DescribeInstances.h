...
{
public:
    DescribeInstances(config::client_aws_config& aws_config,
    ...
    /**
     * Invoke the service to describe the instances, unmarshal the response and return the
     * discovered node map. The map contains mappings from private to public IP and all contained
     * nodes match the filtering rules defined by the {@link #client_aws_config}.
     *
