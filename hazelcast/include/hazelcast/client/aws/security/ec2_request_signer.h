...
{
public:
    ec2_request_signer(const config::client_aws_config& aws_config,
    ...
    std::string get_canonicalized_query_string(
      const std::unordered_map<std::string, std::string>& attributes) const;

private:
    /* Task 1 */
    std::string get_canonicalized_request(
      const std::unordered_map<std::string, std::string>& attributes) const;
    ...
    std::vector<std::string> get_list_of_entries(
    ...
    std::string hmac_sh_a256_hex(const std::vector<unsigned char>& key,
                                 const std::string& msg) const;

    std::string convert_to_hex_string(const unsigned char* buffer, unsigned int len) const;

    unsigned int hmac_sh_a256_bytes(const void* key,
                                    int key_len,
                                    const std::string& msg,
                                    unsigned char* hash) const;

    unsigned int hmac_sh_a256_bytes(const std::string& key,
                                    const std::string& msg,
                                    unsigned char* hash) const;

    unsigned int hmac_sh_a256_bytes(const std::vector<unsigned char>& key,
                                    const std::string& msg,
                                    unsigned char* hash) const;

    unsigned int hmac_sh_a256_bytes(const void* key_buffer,
                                    int key_len,
