...
namespace client {
/*
 * You can use native C++ Client to connect to hazelcast nodes and make almost all operations that a
 * node does. Different from nodes, clients do not hold data.
 *
 * Some of features of C++ Clients are:
 * * Access to distributed data structures like IMap, IQueue, MultiMap, ITopic etc... For complete
 * list see the classes extending DistributedObject
 * * Access to transactional distributed data structures like TransactionalMap, TransactionalQueue
 * etc...
 * * Ability to add cluster listeners to a cluster and entry/item listeners to distributed data
 * structures.
 * @see MembershipListener, IMap#add_entry_listener , IQueue#add_item_listener etc .
 * * C++ Client is smart by default, which means that it knows where the data is and asks directly
 * to correct node. Note that you can turn this feature off ( client_config#setSmart), if you don't
 * want your clients to connect every node.
 *
 * Our C++ client is completely open source and the source code is freely available at
 * https://github.com/hazelcast/hazelcast-cpp-client . Please feel free to contribute. You can join
 * our community at https://groups.google.com/forum/#!forum/hazelcast where you can find answers to
 * your questions.
 */
...
public:
     ...
     * primitive values and are k-ordered (roughly ordered). IDs are in the range from {@code 0} to
     * {@code Long.MAX_VALUE}. <p> The IDs contain timestamp component and a node ID component,
     * which is assigned when the member joins the cluster. This allows the IDs to be ordered and
     * unique without any coordination between members, which makes the generator safe even in
     * split-brain scenario (for caveats,
     ...
     * <p>
     * LifecycleService allows you to shutdown this HazelcastInstance and listen for the lifecycle
     * events.
     *
