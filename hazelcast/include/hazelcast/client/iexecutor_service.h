...
public:
    ...
    public:
        ...
        executor_promise(boost::future<boost::optional<T>>& future,
        ...
        boost::shared_future<boost::optional<T>> get_future() { return shared_future_; }
    ...
    private:
        ...
        bool invoke_cancel_request(bool may_interrupt_if_running)
        {
            ...
            if (partition_id_ > -1) {
                auto request = protocol::codec::executorservice_cancelonpartition_encode(
    ...
    void execute_on_all_members(const HazelcastSerializable& command)
    {
        ...
        for (std::vector<member>::const_iterator it = memberList.begin(); it != memberList.end();
             ++it) {
            submit_to_member<HazelcastSerializable, executor_marker>(command, *it);
        }
    }
     ...
     * map of Member-executor_promise pairs representing pending completion of the task on each
     * member
     *
     * @param task    the task submitted to given members
     * @param members the given members
     * @param <T>     the result type of callable
     * @return map of Member-executor_promise pairs representing pending completion of the task on
     * each member
     */
    template<typename HazelcastSerializable, typename T>
     ...
     * {@link execution_callback<T>::onResponse()} or {@link
     * execution_callback<T>::onFailure(exception::IException)}.
     *
     ...
     */
    template<typename HazelcastSerializable, typename T>
    void submit(const HazelcastSerializable& task,
    ...
    /**
     * Submits a task to randomly selected members. Caller will be notified for the result of the
     * task by
     * {@link execution_callback<T>::onResponse()} or {@link
     * execution_callback<T>::onFailure(exception::IException)}.
     *
    ...
    /**
     * Submits a task to the owner of the specified key. Caller will be notified for the result of
     * the task by
     * {@link execution_callback<T>::onResponse()} or {@link
     * execution_callback<T>::onFailure(exception::IException)}.
     *
     ...
     */
    template<typename HazelcastSerializable, typename T, typename K>
    void submit_to_key_owner(const HazelcastSerializable& task,
                             const K& key,
                             const std::shared_ptr<execution_callback<T>>& callback)
    {
     ...
     */
    template<typename HazelcastSerializable, typename T>
    void submit_to_member(const HazelcastSerializable& task,
                          const member& member,
                          const std::shared_ptr<execution_callback<T>>& callback)
    {
    ...
    /**
     * Submits a task to the specified members. Caller will be notified for the result of the each
     * task by
    ...
    {
        std::shared_ptr<MultiExecutionCallbackWrapper<T>> multiExecutionCallbackWrapper(
          new MultiExecutionCallbackWrapper<T>((int)members.size(), callback));

        for (auto& member : members) {
            std::shared_ptr<ExecutionCallbackWrapper<T>> executionCallback(
    ...
    /**
     * Submits task to the selected members. Caller will be notified for the result of the each task
     * by
     * {@link MultiExecutionCallback#onResponse(Member, Object)}, and when all tasks are completed,
    ...
    /**
     * Submits task to all the cluster members. Caller will be notified for the result of each task
     * by
     * {@link MultiExecutionCallback#onResponse(const Member &, const std::shared_ptr<V> &)}, and
...
private:
    ...
    public:
        MultiExecutionCallbackWrapper(
          int member_size,
          const std::shared_ptr<multi_execution_callback<T>>& multi_execution_callback)
          : multi_execution_callback_(multi_execution_callback)
          , members_(member_size)
        {}
    ...
    public:
        ExecutionCallbackWrapper(
          ...
          member member)
          : multi_execution_callback_wrapper_(multi_execution_callback_wrapper)
    ...
    template<typename T>
    executor_promise<T> submit_to_partition_internal(const serialization::pimpl::data& task_data,
                                                     bool prevent_sync,
                                                     int partition_id)
    {
    ...
    template<typename T>
    void submit_to_partition_internal(const serialization::pimpl::data& task_data,
                                      int partition_id,
                                      const std::shared_ptr<execution_callback<T>>& callback)
    {
        ...
        spi::impl::ClientExecutionServiceImpl* executionService =
          &get_context().get_client_execution_service();
        messageFuture.first.then(
                                 ...
                                 boost::uuids::uuid uuid)
    {
        return invoke_on_partition_owner(
          protocol::codec::executorservice_submittopartition_encode(name_, uuid, task_data),
          partition_id);
    }

    template<typename HazelcastSerializable, typename T, typename K>
    executor_promise<T> submit_to_key_owner_internal(const HazelcastSerializable& task,
                                                     const K& key,
                                                     bool prevent_sync)
    {
    ...
    template<typename HazelcastSerializable, typename T, typename K>
    void submit_to_key_owner_internal(const HazelcastSerializable& task,
                                      const K& key,
                                      const std::shared_ptr<execution_callback<T>>& callback)
    {
        ...
        submit_to_partition_internal<T>(
          to_data<HazelcastSerializable>(task), partitionId, callback);
    }

    template<typename T>
    executor_promise<T> submit_to_random_internal(const serialization::pimpl::data& task_data,
                                                  bool prevent_sync)
    {
    ...
    template<typename HazelcastSerializable, typename T>
    executor_promise<T> submit_to_target_internal(const HazelcastSerializable& task,
                                                  const member& member,
                                                  bool prevent_sync)
    {
        boost::uuids::uuid uuid = context_.random_uuid();
    ...
    template<typename HazelcastSerializable, typename T>
    void submit_to_target_internal(const HazelcastSerializable& task,
                                   const member& member,
                                   const std::shared_ptr<execution_callback<T>>& callback)
    {
        boost::uuids::uuid uuid = context_.random_uuid();
    ...
    invoke_on_target_internal(const HazelcastSerializable& task,
                              ...
                              boost::uuids::uuid uuid)
    {
        return invoke_on_target(protocol::codec::executorservice_submittomember_encode(
      ...
      boost::future<protocol::ClientMessage> future)
    {
        try {
            auto response =
              retrieve_result_from_message<T>(&(get_serialization_service()), std::move(future));
    ...
    check_sync(std::pair<boost::future<protocol::ClientMessage>,
                         std::shared_ptr<spi::impl::ClientInvocation>>& future_pair,
               boost::uuids::uuid uuid,
               int partition_id,
               const member& member,
               bool prevent_sync)
    {
        ...
        if (sync) {
            ...
            objectFuture = future_pair.first.then(
              boost::launch::sync, [=](boost::future<protocol::ClientMessage> f) {
                  return retrieve_result_from_message<T>(serializationService, std::move(f));
              });
        }

        return executor_promise<T>(
          objectFuture, uuid, partition_id, member.get_uuid(), get_context(), future_pair.second);
    }
