 ...
 * based on the partition key. For example, if "Steve" was used, then the following would be on one
 * partition. <ul> <li>a customers IMap with an entry of key "Steve"</li> <li>an orders IMap using a
 * customer key type implementing PartitionAware with key "Steve</li> <li>any queue named
 * "Steve"</li> <li>any PartitionAware object with partition key "Steve"</li>
 * </ul>
 *
 * If you have a {@link com.hazelcast.core.IExecutorService} which needs to deal with a customer and
 * a customer's orders, you can achieve optimal performance by putting them on the same partition.
 * <p/>
 * {@link DistributedObject} also has a notion of the partition key which is of type String
