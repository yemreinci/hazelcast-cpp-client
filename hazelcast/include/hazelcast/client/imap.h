...
public:
    ...
    */
     ...
     * Notice that map interceptor runs on the nodes. Because of that same class should be
     * implemented in java side with same classId and factoryId.
     ...
     * another thread. otherwise it will slow down the system.
     *
    ...
    boost::future<std::vector<K>> key_set(query::paging_predicate<K, V>& predicate)
    {
        ...
        return key_set_for_paging_predicate_data(
                 protocol::codec::holder::paging_predicate_holder::of(predicate,
                                                                      serialization_service_))
          .then([=, &predicate](boost::future<std::pair<std::vector<serialization::pimpl::data>,
    ...
    boost::future<std::vector<std::pair<K, V>>> entry_set(query::paging_predicate<K, V>& predicate)
    {
        ...
        return entry_set_for_paging_predicate_data(
                 protocol::codec::holder::paging_predicate_holder::of(predicate,
                                                                      serialization_service_))
          .then([=, &predicate](boost::future<std::pair<EntryVector, query::anchor_data_list>> f) {
              auto result = f.get();
              predicate.set_anchor_data_list(std::move(result.second));
     ...
     *   imap.addIndex(config::index_config(config::index_config::index_type::SORTED, "age"));  //
     * Sorted index for range queries
     *   imap.addIndex(config::index_config(config::index_config::index_type::HASH, "active"));  //
     * Sorted index for range queries
     * </pre>
     ...
     * <p>
     * Until the index finishes being created, any searches for the attribute will use a full Map
     * scan, thus avoiding using a partially built index and returning incorrect results.
     *
    ...
    boost::future<void> clear() { return to_void_future(proxy::IMapImpl::clear_data()); }
     ...
     * Notice that map EntryProcessor runs on the nodes. Because of that, same class should be
     * implemented in java side with same classId and factoryId.
     *
    ...
    boost::future<boost::optional<ResultType>> execute_on_key(const K& key,
     ...
     */
    template<typename K, typename ResultType, typename EntryProcessor>
    boost::future<boost::optional<ResultType>> submit_to_key(const K& key,
                                                             const EntryProcessor& entry_processor)
    {
        return to_object<ResultType>(
     ...
     */
    template<typename K, typename ResultType, typename EntryProcessor>
    boost::future<std::unordered_map<K, boost::optional<ResultType>>> execute_on_keys(
     ...
     */
    template<typename K, typename ResultType, typename EntryProcessor>
    boost::future<std::unordered_map<K, boost::optional<ResultType>>> execute_on_entries(
      const EntryProcessor& entry_processor)
    {
        return to_object_map<K, ResultType>(
    ...
    {
        return to_object_map<K, ResultType>(
          proxy::IMapImpl::execute_on_entries_data(to_data(entry_processor), to_data(predicate)));
    }
    ...
    query::paging_predicate<K, V> new_paging_predicate(COMPARATOR&& comparator,
                                                       size_t predicate_page_size)
    {
        return query::paging_predicate<K, V>(
          get_serialization_service(), std::move(comparator), predicate_page_size);
    }

    template<typename K, typename V, typename INNER_PREDICATE, typename COMPARATOR>
    query::paging_predicate<K, V> new_paging_predicate(const INNER_PREDICATE& predicate,
                                                       COMPARATOR&& comparator,
                                                       size_t predicate_page_size)
    {
        return query::paging_predicate<K, V>(
          get_serialization_service(), predicate, std::move(comparator), predicate_page_size);
    }
...
protected:
    ...
    virtual boost::future<boost::optional<serialization::pimpl::data>> get_internal(
    ...
    virtual boost::future<bool> remove_internal(const serialization::pimpl::data& key_data,
    ...
    virtual boost::future<bool> try_remove_internal(const serialization::pimpl::data& key_data,
                                                    std::chrono::milliseconds timeout)
    {
    ...
    virtual boost::future<boost::optional<serialization::pimpl::data>> put_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      std::chrono::milliseconds ttl)
    {
    ...
    virtual boost::future<boost::optional<serialization::pimpl::data>> put_if_absent_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data,
      std::chrono::milliseconds ttl)
    {
    ...
    virtual boost::future<boost::optional<serialization::pimpl::data>> replace_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& value_data)
    {
    ...
    virtual boost::future<boost::optional<serialization::pimpl::data>> execute_on_key_internal(
      const serialization::pimpl::data& key_data,
    ...
    boost::future<boost::optional<serialization::pimpl::data>> submit_to_key_internal(
      const serialization::pimpl::data& key_data,
      const serialization::pimpl::data& processor)
    {
    ...
    boost::future<EntryVector> execute_on_keys_internal(const std::unordered_set<K>& keys,
                                                        const EntryProcessor& entry_processor)
    {
        if (keys.empty()) {
        ...
        return proxy::IMapImpl::execute_on_keys_data(keysData,
                                                     to_data<EntryProcessor>(entry_processor));
    }

    virtual boost::future<protocol::ClientMessage> put_all_internal(int partition_id,
      ...
      std::vector<std::pair<K, boost::optional<V>>> entries)
    {
        std::sort(entries.begin(),
                  ...
                  [&](const std::pair<K, boost::optional<V>>& lhs,
                      ...
                      if (0 != result) {
                          // std sort: comparison function object returns ​true if the first
