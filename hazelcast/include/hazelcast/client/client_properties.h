...
/**
 * Client Properties is an internal class. Only use properties documentations as references to be
 * used in client_config#set_property
 *
 */
...
public:
    ...
    /**
     * Client will be sending heartbeat messages to members and this is the timeout. If there is no
     * any message passing between client and member within the given time via this property in
     * milliseconds the connection will be closed.
     *
     ...
     * or {@link client_network_config#redoOperation} is set to true_
     * <p/>
    ...
    /**
     * Client will retry requests which either inherently retryable(idempotent client)
     * or {@link client_network_config#redoOperation} is set to true.
     * <p/>
    ...
    /**
     * The discovery mechanism will discover only IP addresses. You can define the port on which
     * Hazelcast is expected to be running here. This port number is not used by the discovery
     * mechanism itself, it is only returned by the discovery mechanism. The default port is {@link
     ...
     * If the time is already passed, then the exception is delegated to the user. If not, the
     * invocation is retried. Note that, if invocation gets no exception and it is a long running
     * one, then it will not get any exception, no matter how small this timeout is set.
     */
     ...
     * this property is set to true. When it is set to false, the client tries to connect to the
     ...
     * <p/>
     * To prevent the system from overloading, user can apply a constraint on the number of
     * concurrent invocations. If the maximum number of concurrent invocations has been exceeded and
     * a new invocation comes in, then hazelcast will throw hazelcast_overload_exception <p/> By
     * default it is configured as INT32_MAX.
     */
     ...
     * <p/>
     * If an invocation can't be made because there are too many pending invocations, then an
     * exponential backoff is done to give the system time to deal with the backlog of invocations.
     * This property controls how long an invocation is allowed to wait before getting a {@link
     * com.hazelcast.core.hazelcast_overload}. <p/> <p> When set to -1 then
     * <code>hazelcast_overload_exception</code> is thrown immediately without any waiting.
     * </p>
     */
    ...
    /**
     * If an operation has backups, this property specifies how long the invocation will wait for
     * acks from the backup replicas. If acks are not received from some backups, there will not be
     * any rollback on other successful replicas.
     */
    ...
    /**
     * When this configuration is enabled, if an operation has sync backups and acks are not
     * received from backup replicas in time, or the member which owns primary replica of the target
     * partition leaves the cluster, then the invocation fails with \indeterminate_operation_state.
     * However, even if the invocation fails, there will not be any rollback on other successful
     * replicas.
     */
