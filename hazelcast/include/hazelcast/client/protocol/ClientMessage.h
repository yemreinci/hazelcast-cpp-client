...
struct HAZELCAST_API paging_predicate_holder
{
    ...
    static paging_predicate_holder of(const query::paging_predicate<K, V>& p,
...
template<>
struct HAZELCAST_API is_trivial_entry_vector<
  std::vector<std::pair<serialization::pimpl::data, hazelcast::client::serialization::pimpl::data>>>
  : std::false_type
{};
 ...
 * ...... frame-length                 = int32 message-type                 = int32 correlation-id
 * = int64
 *
 * var-sized-param              = string-frame / custom-type-frames / var-sized-param-list-frames /
 * fixed-sized-param-list-frame / map-fixed-to-fixed-frame / map-var-sized-to-var-sized-frames /
 * null-frame
 *
 ...
 * var-sized-param-list-frames  = begin-frame *var-sized-param  end-frame  ; all elements should be
 * same type fixed-sized-param-list-frame = frame-length flags *fixed-sized-param    ; all elements
 * should be same type
 *
 *
 ...
 *
 * flags          = begin-fragment end-fragment final begin-data-structure end-data-structure
 * is-null is-event 9reserved ; reserved for fragmentation begin-fragment = BIT ; reserved for
 * fragmentation end-fragment   = BIT ; set to 1 when this frame is the last frame of the
 * client-message final          = BIT ; set to 1 if this frame represents a null field. is-null =
 * BIT ; set to 1 if this is a begin-frame. begin-frame represents begin of a custom-type or a
 * variable-field-list, 0 otherwise begin          = BIT ; set to 1 if this an end-frame. end-frame
 * represents end of a custom-type or a variable-field-list, 0 otherwise end            = BIT ;
 * Reserved for future usage. reserved       = BIT ; Irrelevant int this context unused         =
 * BIT is-event       = BIT ;
 *
...
public:
    ...
    const std::vector<std::vector<byte>>& get_buffer() const { return data_buffer_; }

    void wrap_for_read();

    inline byte* wr_ptr(size_t requested_bytes) { return wr_ptr(requested_bytes, requested_bytes); }
    ...
    inline byte* peek(size_t requested_bytes)
    {
        ...
        if (offset_ >= data_buffer_[buffer_index_].size()) {
            ...
            if (buffer_index_ == data_buffer_.size()) {
                BOOST_THROW_EXCEPTION(client::exception::hazelcast_serialization(
                  "peek",
                  (boost::format(
                     "Not enough bytes in client message to read. Requested %1% bytes but "
        ...
        if (offset_ + requested_bytes > data_buffer_[buffer_index_].size()) {
            BOOST_THROW_EXCEPTION(client::exception::hazelcast_serialization(
              "peek",
              (boost::format(
                 "Not enough bytes in client message to read. Requested %1% bytes but there "
    ...
    //---------------------- Getters -------------------------------
    template<typename T>
    typename std::enable_if<std::is_same<T, uint8_t>::value, T>::type inline get()
    {
        return *rd_ptr(UINT8_SIZE);
    }

    template<typename T>
    typename std::enable_if<std::is_same<T, int8_t>::value, T>::type inline get()
    {
        return *rd_ptr(UINT8_SIZE);
    }
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, uint16_t>::value, T>::type inline get()
    {
        return boost::endian::load_little_u16(rd_ptr(UINT16_SIZE));
    }

    template<typename T>
    typename std::enable_if<std::is_same<T, int16_t>::value, T>::type inline get()
    {
        return boost::endian::load_little_s16(rd_ptr(INT16_SIZE));
    }

    inline uint32_t get_uint32() { return boost::endian::load_little_u32(rd_ptr(UINT32_SIZE)); }

    template<typename T>
    typename std::enable_if<std::is_same<T, int32_t>::value, T>::type inline get()
    {
        return boost::endian::load_little_s32(rd_ptr(INT32_SIZE));
    }

    template<typename T>
    typename std::enable_if<std::is_same<T, uint64_t>::value, T>::type inline get()
    {
        return boost::endian::load_little_u64(rd_ptr(UINT64_SIZE));
    }

    template<typename T>
    typename std::enable_if<std::is_same<T, int64_t>::value, T>::type inline get()
    {
        return boost::endian::load_little_s64(rd_ptr(INT64_SIZE));
    }

    template<typename T>
    typename std::enable_if<std::is_same<T, std::string>::value, T>::type inline get()
    {
    ...
    get()
    {
        ...
        auto content_length = static_cast<int32_t>(f->frame_len) - SIZE_OF_FRAME_LENGTH_AND_FLAGS;
        size_t item_count =
          content_length / (ClientMessage::get_sizeof<typename T::value_type::first_type>() +
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, address>::value, T>::type inline get()
    {
        // skip begin frame
        rd_ptr(SIZE_OF_FRAME_LENGTH_AND_FLAGS);
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, member>::value, T>::type inline get()
    {
        // skip begin frame
        ...
        rd_ptr(static_cast<int32_t>(f->frame_len) - SIZE_OF_FRAME_LENGTH_AND_FLAGS - UUID_SIZE -
               UINT8_SIZE);
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, boost::uuids::uuid>::value, T>::type inline get()
    {
        if (get<bool>()) {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, codec::StackTraceElement>::value, T>::type inline get()
    {
        // skip begin frame
        rd_ptr(SIZE_OF_FRAME_LENGTH_AND_FLAGS);
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, query::anchor_data_list>::value, T>::type inline get()
    {
        // skip begin frame
        rd_ptr(SIZE_OF_FRAME_LENGTH_AND_FLAGS);
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, map::data_entry_view>::value, T>::type inline get()
    {
        // skip begin frame
        rd_ptr(SIZE_OF_FRAME_LENGTH_AND_FLAGS);
    ...
    template<typename T>
    typename std::enable_if<
      std::is_same<T, typename std::pair<typename T::first_type, typename T::second_type>>::value,
      T>::type inline get()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, cp::raft_group_id>::value, T>::type get();
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, codec::ErrorHolder>::value, T>::type inline get()
    {
        // skip begin frame
        rd_ptr(SIZE_OF_FRAME_LENGTH_AND_FLAGS);
        ...
        codec::ErrorHolder h = {
            error_code, std::move(class_name), std::move(message), std::move(stack_traces)
        };
    ...
    inline void set(uint16_t value) { boost::endian::store_little_u16(wr_ptr(UINT16_SIZE), value); }

    inline void set(int16_t value) { boost::endian::store_little_s16(wr_ptr(INT16_SIZE), value); }

    inline void set(uint32_t value) { boost::endian::store_little_u32(wr_ptr(UINT32_SIZE), value); }

    inline void set(int32_t value) { boost::endian::store_little_s32(wr_ptr(INT32_SIZE), value); }

    inline void set(uint64_t value) { boost::endian::store_little_u64(wr_ptr(UINT64_SIZE), value); }

    inline void set(int64_t value) { boost::endian::store_little_s64(wr_ptr(INT64_SIZE), value); }
    ...
    inline void set(const std::string* value) { set_nullable<std::string>(value); }
    ...
    void inline add_flag(uint16_t flag) { set_header_flags(get_header_flags() | flag); }
...
private:
    ...
    template<typename T>
    typename std::enable_if<(std::is_same<int32_t, T>::value || std::is_same<int64_t, T>::value),
...
template<>
void HAZELCAST_API
ClientMessage::set(const std::vector<std::pair<boost::uuids::uuid, int64_t>>& values,
                   bool is_final);
