...
protected:
      ...
      std::chrono::milliseconds ttl)
    {
        try {
            ...
            auto result =
              invoke_and_get_future<boost::optional<serialization::pimpl::data>>(request, key_data);
    ...
    {
        return register_listener(create_entry_listener_to_key_codec(std::move(key)),
                                 std::move(entry_event_handler));
    }
    ...
    {
        return register_listener(create_entry_listener_with_predicate_codec(std::move(predicate)),
                                 std::move(entry_event_handler));
    }
      ...
      serialization::pimpl::data&& predicate)
    {
        return register_listener(
          create_entry_listener_to_key_with_predicate_codec(std::move(key), std::move(predicate)),
          std::move(entry_event_handler));
    }
          ...
          .then(boost::launch::deferred,
                [=](boost::future<boost::optional<serialization::pimpl::data>> f) {
                    try {
                        ...
                        auto sharedValue =
                          std::make_shared<serialization::pimpl::data>(std::move(*response));
                        if (near_cache_) {
...
private:
    ...
    std::shared_ptr<
      internal::nearcache::NearCache<serialization::pimpl::data, serialization::pimpl::data>>
      near_cache_;
    ...
    public:
        ...
        {
            return protocol::codec::replicatedmap_addnearcacheentrylistener_encode(
        ...
        {
            return protocol::codec::replicatedmap_removeentrylistener_encode(name_,
    ...
    public:
        ...
        protocol::ClientMessage encode_add_request(bool local_only) const override
        {
            return protocol::codec::replicatedmap_addentrylistenerwithpredicate_encode(
    ...
    public:
        ...
        void handle_entry(const boost::optional<data>& key,
            ...
            switch (event_type) {
                ...
                case static_cast<int32_t>(entry_event::type::EVICTED): {
                    near_cache_->invalidate(
                      std::make_shared<serialization::pimpl::data>(std::move(*key)));
                    break;
                }
                ...
                default:
                    BOOST_THROW_EXCEPTION(
                      (exception::exception_builder<exception::illegal_argument>(
    ...
    std::shared_ptr<spi::impl::ListenerMessageCodec> create_entry_listener_codec(
      const std::string name)
    {
    ...
    std::shared_ptr<spi::impl::ListenerMessageCodec> create_entry_listener_to_key_codec(
      serialization::pimpl::data&& key_data)
    {
        return std::shared_ptr<spi::impl::ListenerMessageCodec>(
    ...
    std::shared_ptr<spi::impl::ListenerMessageCodec> create_entry_listener_with_predicate_codec(
      serialization::pimpl::data&& predicate_data)
    {
        return std::shared_ptr<spi::impl::ListenerMessageCodec>(
          new ReplicatedMapAddEntryListenerWithPredicateMessageCodec(name_,
