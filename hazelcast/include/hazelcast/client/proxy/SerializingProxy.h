...
public:
    ...
    {
        return message_future.then(boost::launch::deferred, [](boost::future<T> f) { f.get(); });
    }
...
protected:
    ...
    boost::future<protocol::ClientMessage> invoke_on_partition(protocol::ClientMessage& request,
                                                               int partition_id);
    ...
    typename std::enable_if<
      ...
      boost::optional<std::string>>::type inline to_object(const serialization::pimpl::data& data)
    {
    ...
    {
        return f.then(boost::launch::deferred,
                      [=](boost::future<std::unique_ptr<serialization::pimpl::data>> f) {
                          return to_object<T>(f.get());
                      });
    }
...
protected:
    ...
    template<typename T>
    typename std::enable_if<std::is_trivial<T>::value, boost::future<T>>::type static decode(
      boost::future<protocol::ClientMessage> f)
    {
        return f.then(boost::launch::deferred, [](boost::future<protocol::ClientMessage> f) {
            auto msg = f.get();
