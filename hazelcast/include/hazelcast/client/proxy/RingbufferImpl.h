...
namespace rb {
/**
 * Using this policy one can control the behavior what should to be done when an item is about to be
 * added to the ringbuffer, but there is 0 remaining capacity.
 *
 * Overflowing happens when a time-to-live is set and the oldest item in the ringbuffer (the head)
 * is not old enough to expire.
 *
...
enum struct overflow_policy
{
     ...
     *
     * Example: if there is a time-to-live of 30 seconds, the buffer is full and the oldest item in
     * the ring has been placed a second ago, then there are 29 seconds remaining for that item.
     * Using this policy you are going to overwrite no matter what.
     */
    OVERWRITE = 0,

    /**
     * Using this policy the call will fail immediately and the oldest item will not be overwritten
     * before it is old enough to retire. So this policy sacrificing the ability to write in favor
     * of time-to-live.
     *
     * The advantage of fail is that the caller can decide what to do since it doesn't trap the
     * thread due to backoff.
     *
     * Example: if there is a time-to-live of 30 seconds, the buffer is full and the oldest item in
     * the ring has been placed a second ago, then there are 29 seconds remaining for that item.
...
public:
    ...
    /**
     * The maximum number of items that can be retrieved in 1 go using the {@link
     * #readManyAsync(long, int, int, IFunction)} method.
     */
     ...
     *
     * If no ttl is set, the size will always be equal to capacity after the head completed the
     * first loop around the ring. This is because no items are getting retired.
     *
     * @return the size.
     */
    ...
    /**
     * Returns the sequence of the head. The head is the side of the ringbuffer where the oldest
...
protected:
    ...
    boost::future<int64_t> add_data(serialization::pimpl::data&& item_data)
    {
        auto request = protocol::codec::ringbuffer_add_encode(
          get_name(), static_cast<int32_t>(rb::overflow_policy::OVERWRITE), item_data);
        return invoke_and_get_future<int64_t>(request, partition_id_);
    }
    ...
    boost::future<int64_t> add_all_data(std::vector<serialization::pimpl::data>&& items,
...
protected:
    boost::future<protocol::ClientMessage> read_many_data(int64_t start_sequence,
                                                          int32_t min_count,
                                                          int32_t max_count,
                                                          serialization::pimpl::data* filter_data)
    {
        ...
        try {
            ...
            try {
                std::throw_with_nested(boost::enable_current_exception(exception::execution(
                  "ClientRingbufferProxy::readManyData", "capacity() method failed")));
            } catch (...) {
        ...
        util::Preconditions::check_true(
          max_count <= buffer_capacity_.get(),
