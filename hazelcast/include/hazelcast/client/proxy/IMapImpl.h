...
public:
     ...
     * <p/>
     * If a <tt>MapStore</tt> is defined for this map, deleteAll is <strong>not</strong> called by
     * this method. If you do want to deleteAll to be called use the #clear() method. <p/> The
     * EVICT_ALL event is fired for any registered listeners. See
     * EntryListener#map_evicted(MapEvent)}.
     *
     * @see #clear()
     */
...
protected:
    ...
    boost::future<protocol::ClientMessage> remove_all(
    ...
    boost::future<bool> try_remove(const serialization::pimpl::data& key,
                                   std::chrono::milliseconds timeout);

    boost::future<bool> try_put(const serialization::pimpl::data& key,
                                const serialization::pimpl::data& value,
                                std::chrono::milliseconds timeout);
    ...
    boost::future<protocol::ClientMessage> put_transient(const serialization::pimpl::data& key,
                                                         const serialization::pimpl::data& value,
                                                         std::chrono::milliseconds ttl);

    boost::future<boost::optional<serialization::pimpl::data>> put_if_absent_data(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& value,
      std::chrono::milliseconds ttl);

    boost::future<bool> replace(const serialization::pimpl::data& key,
    ...
    boost::future<protocol::ClientMessage> set(const serialization::pimpl::data& key,
                                               const serialization::pimpl::data& value,
                                               std::chrono::milliseconds ttl);
    ...
    boost::future<EntryVector> get_all_data(int partition_id,
    ...
    boost::future<protocol::ClientMessage> put_all_data(int partition_id,
                                                        const EntryVector& entries);

    boost::future<boost::optional<serialization::pimpl::data>> execute_on_key_data(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& processor);

    boost::future<boost::optional<serialization::pimpl::data>> submit_to_key_data(
      const serialization::pimpl::data& key,
      const serialization::pimpl::data& processor);

    boost::future<EntryVector> execute_on_keys_data(
      const std::vector<serialization::pimpl::data>& keys,
      const serialization::pimpl::data& processor);

    boost::future<EntryVector> execute_on_entries_data(
    ...
    template<typename K, typename V>
    static void set_anchor(std::vector<std::pair<K, boost::optional<V>>>& entries,
                           query::paging_predicate<K, V>& predicate,
                           int nearest_page)
    {
        if (0 == entries.size()) {
...
private:
    ...
    {
    public:
        MapEntryListenerWithPredicateMessageCodec(std::string name,
                                                  bool include_value,
                                                  int32_t listener_flags,
    ...
    {
    public:
        MapEntryListenerToKeyCodec(std::string name,
                                   bool include_value,
                                   int32_t listener_flags,
    ...
    std::shared_ptr<spi::impl::ListenerMessageCodec> create_map_entry_listener_codec(
      bool include_value,
      int32_t listener_flags);
    ...
    std::pair<ResultVector, query::anchor_data_list> get_paging_predicate_response(
      boost::future<protocol::ClientMessage> f) const
    {
        auto msg = f.get();
