...
private:
    ...
    /**
     * The returned connection could be different than the one passed to this method if there is
     * already an existing connection to the given member.
     */
    std::shared_ptr<Connection> on_authenticated(const std::shared_ptr<Connection>& connection,
    ...
    util::SynchronizedMap<boost::uuids::uuid, Connection, boost::hash<boost::uuids::uuid>>
      active_connections_;
