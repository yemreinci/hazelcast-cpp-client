...
/**
 * Provides a context to do transactional operations; so beginning/committing transactions, but also
 * retrieving transactional data-structures like the TransactionalMap.
 *
...
public:
     ...
     */
    template<typename T>
    std::shared_ptr<T> get_transactional_object(const std::string& service_name,
...
private:
    ...
    util::SynchronizedMap<std::pair<std::string, std::string>, proxy::TransactionalObject>
      txn_object_map_;
};
} // namespace client
} // namespace hazelcast
