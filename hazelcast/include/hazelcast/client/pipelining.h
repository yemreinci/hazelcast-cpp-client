...
public:
     ...
     * {@link up()} and {@link down()} methods of this class may be called from the executor thread,
     * and we need to make sure that the `Pipelining` instance is not destructed when these methods
     * are accessed.
     *
     ...
     *                                  every call is sync and you will not benefit from pipelining
     * at all.
     */
    ...
    /**
     * Adds a future to this Pipelining or blocks until there is capacity to add the future to the
     * Pipelining. <p> This call blocks until there is space in the Pipelining, but it doesn't mean
     * that the invocation that returned the ICompletableFuture got blocked.
     *
...
private:
    pipelining(int depth)
      : permits_(util::Preconditions::check_positive(depth, "depth must be positive"))
    {}
