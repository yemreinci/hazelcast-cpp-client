...
public:
     ...
     */
    boost::future<boost::uuids::uuid> add_item_listener(item_listener&& listener,
                                                        bool include_value)
    {
        std::unique_ptr<impl::item_event_handler<protocol::codec::queue_addlistener_handler>>
          itemEventHandler(new impl::item_event_handler<protocol::codec::queue_addlistener_handler>(
            get_name(),
    ...
    boost::future<size_t> drain_to(std::vector<E>& elements)
    {
        return proxy::IQueueImpl::drain_to_data().then(
          boost::launch::deferred, [&](boost::future<std::vector<serialization::pimpl::data>> f) {
     ...
     *
     * @return removes head of the queue and returns it to user . If not available returns empty
     ...
     * Returns immediately without waiting.
     *
     * @return head of queue without removing it. If not available returns empty constructed
     * shared_ptr.
     */
    template<typename E>
