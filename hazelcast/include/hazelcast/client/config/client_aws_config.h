 ...
 * <p/>
 * what happens behind the scenes is that data about the running AWS instances in a specific region
 * are downloaded using the accesskey/secretkey and are potential Hazelcast members. <p/>
 * <h1>Filtering</h1>
 * There are 2 mechanisms for filtering out AWS instances and these mechanisms can be combined
 * (AND). <ol> <li>If a securityGroup is configured, only instances within that security group are
 * selected.
 * </li>
 * <li>
...
public:
     ...
     *
     * @param region the region where the EC2 instances running the Hazelcast members will be
     * running
     * @return the updated client_aws_config
