...
public:
     ...
     *                          A zero value means wait until connection established or an error
     * occurs. The resolution of time is up to milliseconds.
     *
    ...
    /**
     * If {@code true}, client will route the key based operations to owner of the key on
     * best-effort basis. Note that it uses a cached version of PartitionService#getPartitions() and
     * doesn't guarantee that the operation will always be executed on the owner. The cached table
     * is updated every 10 seconds. <p> If {@code smartRouting == false}, all operations will be
     * routed to single member. Operations will need two hops if the chosen member is not owner of
     * the key. Client will have only single open connection. Useful, if there are many clients and
     * we want to avoid each of them connecting to each member. <p> Default value is {@code true}.
     *
     ...
     * client_network_config#addressList, all might be not available. Instead of giving up, throwing
     * Exception and stopping client, it will attempt to retry as much as
     * client_network_config#connectionAttemptLimit() times.
     *
     ...
     * Period for the next attempt to find a member to connect.
     *
     * @param period time to wait before another attempt. The resolution of time is up to
     * milliseconds.
     * @return configured \client_network_config for chaining
     */
    ...
    /**
     * Adds given addresses to candidate address list that client will use to establish initial
     * connection
     *
    ...
    /**
     * Adds given address to candidate address list that client will use to establish initial
     * connection
     *
