...
public:
    ...
    /**
     * Maximum value for prefetch count. The limit is ~10% of the time we allow the IDs to be from
     * the future (see {@link
     * com.hazelcast.flakeidgen.impl.FlakeIdGeneratorProxy#ALLOWED_FUTURE_MILLIS}). <p> The reason
     * to limit the prefetch count is that a single call to {@link FlakeIdGenerator#newId()} might
     * be blocked if the future allowance is exceeded: we want to avoid a single call for large
     * batch to block another call for small batch.
     */
     ...
     * will be fetched. Time unit resolution is milliseconds, default is 600,000msecs (10 minutes).
     * <p>
     * The IDs contain timestamp component, which ensures rough global ordering of IDs. If an ID
     * is assigned to an object that was created much later, it will be much out of order. If you
     * don't care about ordering, set this value to 0. <p> This setting pertains only to {@link
     * FlakeIdGenerator#newId newId} calls made on the member that configured it.
     *
