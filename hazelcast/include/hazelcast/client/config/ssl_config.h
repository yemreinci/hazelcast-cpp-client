...
public:
    ...
    /**
     * This API calls the OpenSSL SSL_CTX_load_verify_locations method underneath while starting the
     * client with this configuration. The validity of the files are checked only when the client
     * starts. Hence, this call will not do any error checking. Error checking is performed only
     * when the certificates are actually loaded during client start.
     *
     * @param filename the name of a file containing certification authority certificates in PEM
     * format.
     */
    ...
    /**
     * @param ciphers The list of ciphers to be used. During client start, if this API was set then
     * the SSL_CTX_set_cipher_list
     * (https://www.openssl.org/docs/man1.0.2/ssl/SSL_set_cipher_list.html) is called with the
     * provided ciphers string. The values and the format of the ciphers are described here:
     * https://www.openssl.org/docs/man1.0.2/apps/ciphers.html Some examples values for the string
     * are: "HIGH", "MEDIUM", "LOW", etc.
     *
