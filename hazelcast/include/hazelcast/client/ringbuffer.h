...
namespace client {
/**
 * A Ringbuffer is a data-structure where the content is stored in a ring like structure. A
 * ringbuffer has a capacity so it won't grow beyond that capacity and endanger the stability of the
 * system. If that capacity is exceeded, than the oldest item in the ringbuffer is overwritten.
 *
 * The ringbuffer has 2 always incrementing sequences:
 * <ol>
 * <li>
 * tailSequence: this is the side where the youngest item is found. So the tail is the side of the
 * ringbuffer where items are added to.
 * </li>
 * <li>
 * headSequence: this is the side where the oldest items are found. So the head is the side where
 * items gets discarded.
 * </li>
 * </ol>
 * The items in the ringbuffer can be found by a sequence that is in between (inclusive) the head
 * and tail sequence.
 *
 * If data is read from a ringbuffer with a sequence that is smaller than the headSequence, it means
 * that the data is not available anymore and a {@link stale_sequence} is thrown.
 *
 * A Ringbuffer currently is not a distributed data-structure. So all data is stored in a single
 * partition; comparable to the IQueue implementation. But we'll provide an option to partition the
 * data in the near future.
 *
 * A Ringbuffer can be used in a similar way as a queue, but one of the key differences is that a
 * queue.take is destructive, meaning that only 1 thread is able to take an item. A ringbuffer.read
 * is not destructive, so you can have multiple threads reading the same item multiple times.
 *
 * The Ringbuffer is the backing data-structure for the reliable {@link com.hazelcast.core.ITopic}
 * implementation. See
...
public:
    /**
     * Adds an item to the tail of the Ringbuffer. If there is no space in the Ringbuffer, the add
     * will overwrite the oldest item in the ringbuffer no matter what the ttl is. For more control
     * on this behavior.
     *
     * The returned value is the sequence of the added item. Using this sequence you can read the
     * added item.
     *
     * <h3>Using the sequence as id</h3>
     * This sequence will always be unique for this Ringbuffer instance so it can be used as a
     * unique id generator if you are publishing items on this Ringbuffer. However you need to take
     * care of correctly determining an initial id when any node uses the ringbuffer for the first
     * time. The most reliable way to do that is to write a dummy item into the ringbuffer and use
     * the returned sequence as initial id. On the reading side, this dummy item should be discard.
     ...
     * </code>
     *
     * This method is not destructive unlike e.g. a queue.take. So the same item can be read by
     * multiple readers or it can be read multiple times by the same reader.
     *
     ...
     * @throws stale_sequence             if the sequence is smaller then {@link #headSequence()}.
     * Because a Ringbuffer won't store all event indefinitely, it can be that the data for the
     *                                            given sequence doesn't exist anymore and the
     * {@link stale_sequence} is thrown. It is up to the caller to deal with this particular
     ...
     * @throws illegal_argument if sequence is smaller than 0 or larger than {@link
     * #tailSequence()}+1.
     ...
     * @return the future to synchronize on completion. The result of the future contains the
     * sequenceId of the last written item.
                                                 ...
                                                 const IFUNCTION* filter = nullptr)
    {
        ...
        return read_many_data(start_sequence, min_count, max_count, &filter_data)
          .then(
            [=](boost::future<protocol::ClientMessage> f) { return get_result_set(std::move(f)); });
    }
    ...
    rb::read_result_set get_result_set(boost::future<protocol::ClientMessage> f)
    {
        ...
        msg.rd_ptr(static_cast<int32_t>(initial_frame->frame_len) -
                   ClientMessage::RESPONSE_HEADER_LEN - ClientMessage::INT32_SIZE -
                   ClientMessage::INT64_SIZE);

        auto datas = msg.get<std::vector<data>>();
        auto item_seqs = msg.get_nullable<std::vector<int64_t>>();
        return rb::read_result_set(
          read_count, std::move(datas), get_serialization_service(), item_seqs, next_seq);
    }
