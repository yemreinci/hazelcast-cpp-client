...
private:
    ...
    struct listener_registration
    {
        ...
        std::shared_ptr<client::impl::BaseEventHandler> handler;
        util::SynchronizedMap<std::shared_ptr<connection::Connection>, connection_registration>
          registrations;
    };
    ...
    void remove_event_handler(int64_t call_id,
                              const std::shared_ptr<connection::Connection>& connection);

    boost::uuids::uuid register_listener_internal(
      std::shared_ptr<ListenerMessageCodec> listener_message_codec,
      std::shared_ptr<client::impl::BaseEventHandler> handler);

    bool deregister_listener_internal(boost::uuids::uuid user_registration_id);

    void connection_added_internal(const std::shared_ptr<connection::Connection>& connection);

    void connection_removed_internal(const std::shared_ptr<connection::Connection>& connection);

    void invoke(const std::shared_ptr<listener_registration>& listener_registration,
                const std::shared_ptr<connection::Connection>& connection);

    void invoke_from_internal_thread(
    ...
    util::
      SynchronizedMap<boost::uuids::uuid, listener_registration, boost::hash<boost::uuids::uuid>>
        registrations_;
