 ...
 */
class HAZELCAST_API ClientInvocation : public std::enable_shared_from_this<ClientInvocation>
{
public:
    ...
    void set_send_connection(const std::shared_ptr<connection::Connection>& send_connection);
    ...
    void set_event_handler(
      const std::shared_ptr<EventHandler<protocol::ClientMessage>>& event_handler);
...
private:
    ...
    /**
     * Contains the pending response from the primary. It is pending because it could be that
     * backups need to complete. Note that we do not need thread safety since these are only
     * read/write from the same io thread for the connection.
     */
