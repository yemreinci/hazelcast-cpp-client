 ...
 * <p>
 * When backpressure is enabled and there are too many concurrent invocations, calls of {@link
 * #next()} will block using a spin-loop with exponential backoff. <p/> Currently a single
 * CallIdSequence is used for all partitions, so there is contention. Also one partition can cause
 * problems in other partition if a lot of invocations are created for that partition. Then other
 ...
 * and do a mod based on the partition-id. The advantage is that you reduce contention and improve
 * isolation, at the expense of: <ol> <li>increased complexity</li> <li>not always being able to
 * fully utilize the number of invocations.</li>
 * </ol>
 */
...
public:
     ...
     * supports backpressure, it will not return unless the number of outstanding invocations is
     * within the configured limit. Instead it will block until the condition is met and eventually
     * throw hazelcast_overload_exception.
     *
     * @return the generated call ID
     * @throws hazelcast_overload if the outstanding invocation count hasn't dropped below the
     * configured limit
     */
