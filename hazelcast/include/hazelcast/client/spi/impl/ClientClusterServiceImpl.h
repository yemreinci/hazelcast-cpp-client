...
public:
    ...
    std::vector<member> get_members(const client::member_selector& selector) const;
...
private:
    static constexpr boost::chrono::milliseconds INITIAL_MEMBERS_TIMEOUT{ boost::chrono::seconds(
      120) };
    ...
    std::unordered_map<boost::uuids::uuid, membership_listener, boost::hash<boost::uuids::uuid>>
      listeners_;
    ...
    std::vector<membership_event> detect_membership_events(
