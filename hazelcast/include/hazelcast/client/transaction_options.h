...
public:
     ...
     *
     * A local transaction is less safe than a two phase transaction; when a member fails during the
     * commit of a local transaction, it could be that some of the changes are committed, while
     * others are not and this can leave your system in an inconsistent state.
     *
    ...
    /**
     *
     * The timeout determines the maximum lifespan of a transaction. So if a transaction is
     * configured with a timeout of 2 minutes, then it will automatically rollback if it hasn't
     * committed yet.
     *
     ...
     *
     * The durability is the number of machines that can take over if a member fails during a
     * transaction commit or rollback. This value only has meaning when TransactionType#TWO_PHASE is
     * selected.
     *
