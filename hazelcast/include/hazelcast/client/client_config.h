...
public:
    ...
    /**
     * Returns the configured cluster name. The name is sent as part of client authentication
     * message and may be verified on the member.
     *
    ...
    /**
     *  There are two types of credentials you can provide, \username_password_credentials and
     * \token_credentials
     *
    ...
    /**
     * If true, client will redo the operations that were executing on the server and client lost
     * the connection. This can be because of network, or simply because the member died. However it
     * is not clear whether the application is performed or not. For idempotent operations this is
     * harmless, but for non idempotent ones retrying can cause to undesirable effects. Note that
    ...
    /**
     * Returns a {\link ClientFlakeIdGeneratorConfig} configuration for the given flake ID generator
     * name. <p> The name is matched by pattern to the configuration and by stripping the partition
     * ID qualifier from the given {@code name}. If there is no config found by the name, it will
     * return the configuration with the name {@code "default"}.
     *
     ...
     */
    const config::client_flake_id_generator_config* find_flake_id_generator_config(
     ...
     * explicitly adding it by invoking {\link
     * #addFlakeIdGeneratorConfig(ClientFlakeIdGeneratorConfig)}. <p> Because it adds new
     * configurations if they are not already present, this method is intended to be used before
     * this config is used to create a hazelcast instance. Afterwards, newly added configurations
     * may be ignored.
     *
     * \param name name of the flake ID generator config
     ...
     * @see StringPartitioningStrategy#getBaseName(std::string)
     */
    const config::client_flake_id_generator_config* get_flake_id_generator_config(
      const std::string& name);
...
private:
    ...
    std::unordered_map<std::string, config::client_flake_id_generator_config>
      flake_id_generator_config_map_;
