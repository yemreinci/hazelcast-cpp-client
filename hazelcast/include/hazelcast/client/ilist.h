...
public:
    ...
    boost::future<boost::uuids::uuid> add_item_listener(item_listener&& listener,
                                                        bool include_value)
    {
        std::unique_ptr<impl::item_event_handler<protocol::codec::list_addlistener_handler>>
          itemEventHandler(new impl::item_event_handler<protocol::codec::list_addlistener_handler>(
            get_name(),
    ...
    /**
     * Replaced the element in the given index. And returns element if there were entry before
     * inserting.
     *
     * @param index insert position
     * @param element to be inserted.
     * @return oldElement in given index.
     * @throws Iclass_cast if the type of the specified element is incompatible with the server
     * side.
     * @throws index_out_of_bounds if the index is out of range.
     */
