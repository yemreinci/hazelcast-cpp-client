...
/**
 * This class represents the type of a Hazelcast serializable object. The fields can take the
 * following values:
 * 1. Primitive types: factoryId=-1, classId=-1, typeId is the type id for that primitive as listed
 * in
 * @link SerializationConstants
 * 2. Array of primitives: factoryId=-1, classId=-1, typeId is the type id for that array as listed
 * in
 * @link SerializationConstants
 * 3. identified_data_serializer: factory, class and type ids are non-negative values as registered
 * by the DataSerializableFactory.
...
/**
 * typed_data class is a wrapper class for the serialized binary data. It does late deserialization
 * of the data only when the get method is called.
 */
...
public:
     ...
     *
     * <b>CAUTION</b>: The type that you provide should be compatible with what object type is
     * returned with the get_type API, otherwise you will either get an exception of incorrectly try
...
template<>
struct HAZELCAST_API hz_serializer<hazelcast_json_value> : public builtin_serializer
{
public:
    static inline pimpl::serialization_constants get_type_id()
    {
...
/**
 * ClassDefinition defines a class schema for portable classes. It allows to query field names,
 * types, class id etc. It can be created manually using {@link ClassDefinitionBuilder} or on demand
 * during serialization phase.
 *
...
public:
    ...
    ClassDefinitionBuilder& add_portable_array_field(const std::string& field_name,
                                                     std::shared_ptr<ClassDefinition> def);
...
public:
    template<typename T>
    static inline typename std::
      enable_if<std::is_base_of<versioned_portable_serializer, hz_serializer<T>>::value, int>::type
      get_version(int)
    {
...
class HAZELCAST_API object_data_input : public pimpl::data_input<std::vector<byte>>
{
public:
    /**
     * Internal API. Constructor
     */
    object_data_input(const std::vector<byte>& buffer,
                      int offset,
    ...
    template<typename T>
    typename std::enable_if<std::is_base_of<portable_serializer, hz_serializer<T>>::value,
                            boost::optional<T>>::type inline read_object(int32_t type_id);
...
public:
    ...
    /* enable_if needed here since 'boost::optional<char [5]>' can not be composed this template
     * match */
    template<typename T>
    typename std::enable_if<!(std::is_array<T>::value &&
                              std::is_same<typename std::remove_all_extents<T>::type, char>::value),
...
public:
    ...
    std::shared_ptr<ClassDefinition> read_class_definition(object_data_input& input,
                                                           int id,
                                                           int class_id,
                                                           int version);
    ...
    template<typename T>
    typename std::enable_if<std::is_same<char, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<char16_t, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
        return field_type::TYPE_CHAR;
    }
    ...
    template<typename T>
    typename std::enable_if<std::is_same<bool, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<int32_t, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<int64_t, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<float, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    template<typename T>
    typename std::enable_if<std::is_same<double, typename std::remove_cv<T>::type>::value,
                            field_type>::type static get_type()
    {
    ...
    typename std::enable_if<
      ...
      field_type>::type static get_type()
    {
        return field_type::TYPE_UTF_ARRAY;
    }
...
public:
    ...
    void write(const std::string& field_name, T value)
    {
        typedef typename std::remove_pointer<
          typename std::remove_reference<typename std::remove_cv<T>::type>::type>::type value_type;
    ...
    void write_null_portable(const std::string& field_name)
    {
        ...
        std::shared_ptr<ClassDefinition> nestedClassDef =
          context_.lookup_class_definition(factoryId, classId, context_.get_version());
        if (!nestedClassDef) {
...
public:
    ...
    template<typename T>
    typename std::enable_if<
      std::is_same<boost::optional<std::string>, typename std::remove_cv<T>::type>::value,
      T>::type
    read(const std::string& field_name)
    {
        set_position(field_name, PortableContext::get_type<T>());
...
public:
    ...
    static boost::optional<T> read_object(object_data_input& in)
    {
        ...
        if (expectedFactoryId != factoryId || expectedClassId != classId) {
            BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
              "object_data_input::read_object<identified_data_serializer>",
              (boost::format("Factory id %1% and class id %2% of data do not match expected "
...
public:
    ...
    {
        object_data_output output(
          false, &portable_serializer_, serialization_config_.get_global_serializer());
                            ...
                            boost::optional<T>>::type inline to_object(const data& data)
    {
        ...
        object_data_input objectDataInput(data.to_byte_array(),
                                          8,
    ...
    template<typename T>
    typename std::enable_if<std::is_same<T, typed_data>::value,
...
public:
     ...
     *
     * Note that portable fields can not read after getRawDataInput() is called. In case this
     * happens, io will be thrown.
     *
     * @return rawDataInput
     * @throws io_exception
     */
...
public:
     ...
     * consecutively at the end of stream. User should not that after getting rawDataOutput trying
     * to write portable fields will result in io_exception
     *
...
template<typename T>
void
portable_writer::write_portable(const std::string& field_name, const T* portable)
{
    if (is_default_writer_)
...
template<typename T>
void
portable_writer::write_portable_array(const std::string& field_name, const std::vector<T>* values)
{
    if (is_default_writer_)
                                                                                          ...
                                                                                          type_id)
{
    if (type_id != hz_serializer<T>::get_type_id()) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          ...
          (boost::format("The associated serializer Serializer<T> type id %1% does not match "
...
PortableSerializer::read_object(object_data_input& in)
{
    ...
    if (factoryId != hz_serializer<T>::get_factory_id() ||
        ...
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          ...
          (boost::format("Received data (factory-class id)=(%1%, %2%) does not match expected "
                         "factory-class id (%3%, %4%)") %
...
template<typename T>
T
PortableSerializer::read(object_data_input& in, int32_t factory_id, int32_t class_id)
{
    int version = in.read<int32_t>();

    int portableVersion = find_portable_version<T>(factory_id, class_id);

    portable_reader reader = create_reader(in, factory_id, class_id, version, portableVersion);
...
{
    int portableVersion =
      PortableVersionHelper::get_version<T>(serialization_config_.get_portable_version());
    std::shared_ptr<ClassDefinition> cd = lookup_class_definition(
      hz_serializer<T>::get_factory_id(), hz_serializer<T>::get_class_id(), portableVersion);
    if (cd.get() == NULL) {
...
template<typename T>
void
DefaultPortableWriter::write_portable(const std::string& field_name, const T* portable)
{
                                            ...
                                            const std::vector<T>* values)
{
    ...
    if (len > 0) {
        std::shared_ptr<ClassDefinition> classDefinition =
          portable_serializer_.lookup_or_register_class_definition<T>((*values)[0]);
        ...
        for (int32_t i = 0; i < len; i++) {
            ...
            object_data_output_.write_at(currentOffset + i * util::Bits::INT_SIZE_IN_BYTES,
...
DefaultPortableWriter::check_portable_attributes(const FieldDefinition& fd)
{
    if (fd.get_factory_id() != hz_serializer<T>::get_factory_id()) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          ...
          (boost::format("Wrong Portable type! Expected factory-id: %1%, Actual factory-id: %2%") %
    ...
    if (fd.get_class_id() != hz_serializer<T>::get_class_id()) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          "DefaultPortableWriter::::checkPortableAttributes",
          (boost::format("Wrong Portable type! Expected class-id: %1%, Actual class-id: %2%") %
...
ClassDefinitionWriter::create_nested_class_def(const T& portable)
{
    ...
    ClassDefinitionBuilder definitionBuilder(
      hz_serializer<T>::get_factory_id(), hz_serializer<T>::get_class_id(), version);
