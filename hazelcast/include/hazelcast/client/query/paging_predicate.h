...
struct anchor_data_list
{
    ...
    std::vector<std::pair<serialization::pimpl::data, boost::optional<serialization::pimpl::data>>>
      data_list;
};
 ...
 * It can be constructed with a page-size, an inner predicate for filtering, and a comparator for
 * sorting. This class is not thread-safe and stateless. To be able to reuse for another query, one
 * should call
 ...
 *
 * // We are constructing our paging predicate with a predicate and page size. In this case query
 * results fetched two by two. paging_predicate predicate = new paging_predicate(lessEqualThanFour,
 * 2);
 *
 ...
 * System.out.println("anchor -> " + anchor); // will print 'anchor -> 1=1',  since the anchor is
 * the last entry of the previous page. predicate.previousPage(); // we are setting up paging
 * predicate to fetch previous page in the next call values = map.values(predicate);
 * System.out.println("values = " + values) // will print 'values = [0, 1]'
 * </pre>
 */
template<typename K, typename V>
class paging_predicate
  : public predicate
  , public paging_predicate_marker
{
    friend imap;
...
public:
    ...
    iteration_type get_iteration_type() const { return iteration_type_; }

    void set_iteration_type(iteration_type type) { iteration_type_ = type; }

    size_t get_page() const { return page_; }

    void set_page(size_t page_number) { page_ = page_number; }

    size_t get_page_size() const { return page_size_; }

    const query::entry_comparator<K, V>* get_comparator() const { return comparator_.get(); }
...
private:
    ...
    paging_predicate(serialization::pimpl::SerializationService& serialization_service,
      ...
      : out_stream_(serialization_service.new_output_stream())
      , page_size_(predicate_page_size)
      , page_(0)
      , iteration_type_(iteration_type::VALUE)
    {
     ...
     * results will be natural ordered
     * throws illegal_argument {@link illegal_argument} if pageSize is not greater than 0
     * throws illegal_argument {@link illegal_argument} if inner predicate is also {@link
     * paging_predicate}
     *
...
struct hz_serializer<query::paging_predicate<K, V>> : public identified_data_serializer
{
    ...
    static query::paging_predicate<K, V> read_data(object_data_input& in)
    {
        ...
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          "readData", "Client should not need to use readdata method!!!"));
    }
};
