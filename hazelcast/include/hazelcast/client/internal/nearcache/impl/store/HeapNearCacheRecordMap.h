...
public:
    ...
    HeapNearCacheRecordMap(serialization::pimpl::SerializationService& ss, int32_t initial_capacity)
    ...
    public:
        NearCacheEvictableSamplingEntry(const std::shared_ptr<KS>& key,
          ...
          : util::SampleableConcurrentHashMap<K, V, KS, R>::SamplingEntry(key, value)
          , serialization_service_(ss)
        {}
        ...
        {
            return std::shared_ptr<V>(serialization_service_.to_shared_object<V>(
            ...
            if (util::SynchronizedMap<std::shared_ptr<KS>, R>::remove(
                ...
                if (eviction_listener != NULL) {
                    eviction_listener->on_evict(
                      evictionCandidate->get_accessor(), evictionCandidate->get_evictable(), false);
                }
            }
        }
    ...
    {
        std::unique_ptr<util::Iterable<typename util::SampleableConcurrentHashMap<K, V, KS, R>::E>>
          samples =
            util::SampleableConcurrentHashMap<K, V, KS, R>::get_random_samples(sample_count);
        if (NULL == samples.get()) {
    ...
    class EvictionCandidateAdapter : public util::Iterable<eviction::EvictionCandidate<K, V, KS, R>>
    {
    public:
        EvictionCandidateAdapter(
          std::unique_ptr<
            util::Iterable<typename util::SampleableConcurrentHashMap<K, V, KS, R>::E>>&
            samples_iterable)
          : adapted_iterable_(std::move(samples_iterable))
        {
            adapted_iterator_ =
              std::unique_ptr<util::Iterator<eviction::EvictionCandidate<K, V, KS, R>>>(
        ...
        public:
            EvictionCandidateIterator(
              util::Iterator<typename util::SampleableConcurrentHashMap<K, V, KS, R>::E>&
                adapted_iterator)
            ...
            {
                std::shared_ptr<typename util::SampleableConcurrentHashMap<K, V, KS, R>::E> obj =
                  it_.next();
                std::shared_ptr<NearCacheEvictableSamplingEntry> heapObj =
                  std::static_pointer_cast<NearCacheEvictableSamplingEntry>(obj);
                return std::static_pointer_cast<eviction::EvictionCandidate<K, V, KS, R>>(heapObj);
            }
...
protected:
    std::shared_ptr<typename util::SampleableConcurrentHashMap<K, V, KS, R>::SamplingEntry>
    create_sampling_entry(std::shared_ptr<KS>& key, std::shared_ptr<R>& value) const override
    {
