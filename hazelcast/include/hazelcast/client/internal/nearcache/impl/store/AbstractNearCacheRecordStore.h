...
public:
    ...
    {
        auto& evictionConfig =
          const_cast<client::config::near_cache_config&>(cache_config).get_eviction_config();
    ...
    {
        auto& evictionConfig =
          const_cast<client::config::near_cache_config&>(near_cache_config_).get_eviction_config();
        this->records_ = create_near_cache_record_map(near_cache_config_);
        this->max_size_checker_ =
          create_near_cache_max_size_checker(evictionConfig, near_cache_config_);
    ...
    */
    ...
    void do_eviction_if_required() override
    {
        ...
        if (is_eviction_enabled()) {
            eviction_strategy_->evict(
              records_.get(), eviction_policy_evaluator_.get(), eviction_checker_.get(), this);
        }
    }
    ...
    void do_eviction() override
    {
        ...
        if (is_eviction_enabled()) {
            eviction_strategy_->evict(records_.get(), eviction_policy_evaluator_.get(), NULL, this);
        }
    }
...
protected:
    ...
    */
    ...
    virtual void put_to_record(std::shared_ptr<R>& record, const std::shared_ptr<V>& value)
    {
        assert(0);
    }
    ...
    void check_available() const
    {
        if (!is_available()) {
            BOOST_THROW_EXCEPTION(exception::illegal_state(
              near_cache_config_.get_name() + " named Near Cache record store is not available"));
        }
    }

    std::unique_ptr<eviction::EvictionPolicyEvaluator<K, V, KS, R>>
    create_eviction_policy_evaluator(const client::config::eviction_config& eviction_config)
    {
    ...
    std::shared_ptr<serialization::pimpl::data> value_to_data(const std::shared_ptr<V>& value)
    {
        if (value.get() != NULL) {
            return std::shared_ptr<serialization::pimpl::data>(
              new serialization::pimpl::data(serialization_service_.to_data<V>(value.get())));
        } else {
            return std::shared_ptr<serialization::pimpl::data>();
        }
    }
    ...
    */
    ...
    std::shared_ptr<V> data_to_value(const std::shared_ptr<serialization::pimpl::data>& data,
                                     ...
                                     void* dummy)
    {
        ...
        /*
                                        if (data.get() != NULL) {
                                            auto value =
           serializationService.to_object<V>(data.get()); return std::shared_ptr<V>(new
           V(std::move(value).value())); } else { return std::shared_ptr<V>();
                                        }
        */
    }
    ...
    */
    ...
    const std::shared_ptr<serialization::pimpl::data> to_data(const std::shared_ptr<V>& obj)
    {
        if (obj.get() == NULL) {
            return std::shared_ptr<serialization::pimpl::data>();
        } else {
    ...
    /*
                                std::shared_ptr<V>
       toValue(std::shared_ptr<serialization::pimpl::data> &obj) { if (obj.get() == NULL) { return
       std::shared_ptr<V>(); } else { return dataToValue(obj, (V *)NULL);
                                    }
                                }
    */
    ...
    /*
                                void onPut(const std::shared_ptr<KS> &key,
                                           const std::shared_ptr<serialization::pimpl::data> &value,
                                           const std::shared_ptr<R> &record, const
    ...
    */
    ...
    /*
                                void onPutError(const std::shared_ptr<KS> &key,
                                                const std::shared_ptr<serialization::pimpl::data>
       &value, const std::shared_ptr<R> &record, const std::shared_ptr<R> &oldRecord, const
       exception::IException &error) {
                                }
    */

    void on_remove(const std::shared_ptr<KS>& key, const std::shared_ptr<R>& record, bool removed)
    {}
    ...
    {
        return eviction_strategy_.get() != NULL && eviction_policy_evaluator_.get() != NULL &&
               eviction_policy_ != hazelcast::client::config::eviction_policy::NONE;
    }
    ...
    /*
static const int REFERENCE_SIZE = MEM_AVAILABLE ? MEM.arrayIndexScale(Object[].class) :
...
private:
    ...
    void put_internal(const std::shared_ptr<KS>& key, const std::shared_ptr<VALUE>& value)
    {
        ...
        if (!is_eviction_enabled() && eviction_checker_->is_eviction_required() &&
