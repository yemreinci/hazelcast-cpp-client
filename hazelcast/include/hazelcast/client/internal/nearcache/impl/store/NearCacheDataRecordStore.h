...
public:
    ...
    NearCacheDataRecordStore(const std::string& name,
      ...
      : BaseHeapNearCacheRecordStore<K, V, serialization::pimpl::data, record::NearCacheDataRecord>(
          name,
          config,
          ss)
    {}
...
protected:
    ...
    int64_t get_record_storage_memory_cost(record::NearCacheDataRecord* record) const override
    {
        if (record == NULL) {
            return 0L;
        }
    ...
    /*
       ...
       ANCRS::to_data(value); return valueToRecordInternal(data);
                                }
    */
...
private:
      ...
      const std::shared_ptr<serialization::pimpl::data>& data)
    {
        ...
        if (ANCRS::time_to_live_millis_ > 0) {
            return std::unique_ptr<record::NearCacheDataRecord>(new record::NearCacheDataRecord(
              data, creationTime, creationTime + ANCRS::time_to_live_millis_));
        } else {
            return std::unique_ptr<record::NearCacheDataRecord>(new record::NearCacheDataRecord(
