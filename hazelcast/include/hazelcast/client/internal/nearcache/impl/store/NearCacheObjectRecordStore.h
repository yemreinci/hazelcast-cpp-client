...
public:
    ...
    NearCacheObjectRecordStore(const std::string& name,
      ...
      : BaseHeapNearCacheRecordStore<K,
                                     ...
                                     record::NearCacheObjectRecord<V>>(name, config, ss)
    {}
...
protected:
    ...
    /*
       ...
       std::const_pointer_cast<serialization::pimpl::data>( value_data); const std::shared_ptr<V>
       value = ANCRS::toValue(data); return valueToRecordInternal(value);
                                }
    */
        ...
        if (ANCRS::time_to_live_millis_ > 0) {
            return std::unique_ptr<record::NearCacheObjectRecord<V>>(
              new record::NearCacheObjectRecord<V>(
                value, creationTime, creationTime + ANCRS::time_to_live_millis_));
        } else {
            return std::unique_ptr<record::NearCacheObjectRecord<V>>(
              new record::NearCacheObjectRecord<V>(
