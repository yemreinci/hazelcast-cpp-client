...
public:
    ...
    {
        ANCRS::on_evict(key, record, was_expired);
    ...
    {
        std::vector<std::pair<std::shared_ptr<KS>, std::shared_ptr<R>>> entries =
          ANCRS::records_->entry_set();
        for (typename std::vector<
               std::pair<std::shared_ptr<KS>, std::shared_ptr<R>>>::const_iterator it =
               entries.begin();
             it != entries.end();
             ++it) {
...
protected:
    ...
    {
        typename client::config::eviction_config::max_size_policy maxSizePolicy =
          eviction_config.get_maximum_size_policy();
        if (maxSizePolicy == client::config::eviction_config::ENTRY_COUNT) {
            return std::unique_ptr<eviction::MaxSizeChecker>(
              new maxsize::EntryCountNearCacheMaxSizeChecker<K, V, KS, R>(
                eviction_config.get_size(), *ANCRS::records_));
        }
