...
namespace nearcache {
/**
 * {@link NearCacheManager} is the contract point to manage all existing {@link NearCache}
 * instances.
 */
...
public:
     ...
     * @param nearCacheConfig      the {@link NearCacheConfig} of the {@link NearCache} to be
     * created
...
protected:
    ...
    std::unique_ptr<NearCache<KS, V>> create_near_cache(
      ...
      const client::config::near_cache_config& near_cache_config)
    {
        return std::unique_ptr<NearCache<KS, V>>(new impl::DefaultNearCache<K, V, KS>(
