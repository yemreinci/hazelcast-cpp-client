...
public:
    ...
    void connect(const std::shared_ptr<connection::Connection> connection) override
    {
        ...
        try {
            ...
            boost::asio::async_connect(socket_.lowest_layer(), addresses, boost::asio::use_future)
              .get();
            post_connect();
            connectTimer.cancel();
                     ...
                     const std::shared_ptr<spi::impl::ClientInvocation> invocation) override
    {
        ...
        boost::asio::post(socket_.get_executor(), [=]() {
            ...
            auto handler = [=](const boost::system::error_code& ec, std::size_t bytes_written) {
                if (ec) {
                    ...
                    auto message =
                      (boost::format{
                       ...
                       ec % *invocation % *connection)
                        .str();
                    invocationIt->second->notify_exception(boost::enable_current_exception(
                      std::make_exception_ptr(exception::io("Connection::write", message))));
    ...
    const address& get_remote_endpoint() const override { return remote_endpoint_; }

    boost::asio::executor get_executor() noexcept override { return socket_.get_executor(); }
...
protected:
    ...
    void do_read(const std::shared_ptr<connection::Connection> connection)
    {
        ...
        socket_.async_read_some(
          buffer(connection->read_handler.byte_buffer.ix(),
          ...
          [=](const boost::system::error_code& ec, std::size_t bytes_read) {
              if (ec) {
                  ...
                  util::IOUtil::close_resource(
                    connection.get(),
                    (boost::format("Socket read error. %1% for %2%") % ec % (*connection))
                      .str()
                      .c_str());
                  return;
              }

              connection->read_handler.byte_buffer.safe_increment_position(bytes_read);
