...
class SamplingBasedEvictionStrategy : public AbstractEvictionStrategy<MAPKEY, MAPVALUE, A, E, S>
{
protected:
    /**
     * Processes sampling based eviction logic on {@link SampleableEvictableStore}.
     *
     * @param sampleableEvictableStore  {@link SampleableEvictableStore} that holds {@link
     * Evictable} entries
    ...
    {
        std::unique_ptr<util::Iterable<EvictionCandidate<MAPKEY, MAPVALUE, A, E>>> samples =
          sampleable_evictable_store->sample(SAMPLE_COUNT);
        std::unique_ptr<
