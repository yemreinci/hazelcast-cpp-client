...
public:
     ...
     *
     * @param evictionConfig {@link eviction_configuration} for the requested {@link
      ...
      const client::config::eviction_config& eviction_config)
    {
        ...
        return EvictionStrategyProvider<MAPKEY, MAPVALUE, A, E, S>::EVICTION_STRATEGY_MAP
    ...
    static std::unordered_map<client::config::eviction_strategy_type,
    ...
    init()
    {
        std::unordered_map<client::config::eviction_strategy_type,
                           std::shared_ptr<EvictionStrategy<MAPKEY, MAPVALUE, A, E, S>>>
          map;
        map[client::config::eviction_strategy_type::SAMPLING_BASED_EVICTION] = std::shared_ptr<
          EvictionStrategy<MAPKEY, MAPVALUE, A, E, S>>(
          new impl::strategy::sampling::SamplingBasedEvictionStrategy<MAPKEY, MAPVALUE, A, E, S>());
        return map;
    }
