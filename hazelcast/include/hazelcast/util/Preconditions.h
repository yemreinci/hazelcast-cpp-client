...
public:
    ...
    {
        if (value <= 0) {
            throw client::exception::illegal_argument("Preconditions::checkPositive",
                                                      error_message);
        }
        return value;
    }
    ...
    {
        if (value < 0) {
            throw client::exception::illegal_argument("Preconditions::checkNotNegative",
                                                      error_message);
        }
        return value;
    }
     ...
     */
    template<typename T>
    static void check_max(const T& actual_size,
                          const T& expected_maximum,
                          const std::string& variable_name)
    {
        if (actual_size > expected_maximum) {
            throw(client::exception::exception_builder<client::exception::illegal_argument>(
                    "Preconditions::checkNotNegative")
                  << variable_name << " can't be larger than " << expected_maximum)
              .build();
        }
    }
     ...
     */
    static const std::string& check_has_text(const std::string& argument,
                                             const std::string& error_message);

    /**
     * @throws client::exception::invalid_configuration if the user does not compile with
     * HZ_BUILD_WITH_SSL flag but is trying to use a feature (e.g. TLS, AWS Cloud Discovery) that
     * needs this flag.
     */
