...
ClientConnectionManagerImpl::ClientConnectionManagerImpl(
  ...
  , translator_(address_translator)
  ...
  , reconnect_mode_(
      client.get_client_config().get_connection_strategy_config().get_reconnect_mode())
  ...
  , load_balancer_(client.get_client_config().get_load_balancer())
{
    ...
    if (connAttemptLimit < 0) {
        this->connection_attempt_limit_ =
          isAsync ? DEFAULT_CONNECTION_ATTEMPT_LIMIT_ASYNC : DEFAULT_CONNECTION_ATTEMPT_LIMIT_SYNC;
    } else {
...
ClientConnectionManagerImpl::get_or_connect(const address& address)
{
    ...
    connection = std::make_shared<Connection>(
      target, client_, ++connection_id_gen_, *socket_factory_, *this, connection_timeout_millis_);
    connection->connect();
    ...
    try {
        if (f.wait_for(authentication_timeout_) != boost::future_status::ready) {
            BOOST_THROW_EXCEPTION(
              exception::timeout("ClientConnectionManagerImpl::authenticate",
        ...
        response.rd_ptr(static_cast<int32_t>(initial_frame->frame_len) -
                        ClientMessage::RESPONSE_HEADER_LEN - 2 * ClientMessage::UINT8_SIZE -
                        2 * (sizeof(boost::uuids::uuid) + ClientMessage::UINT8_SIZE) -
    ...
    switch (authentication_status) {
        ...
        case protocol::CREDENTIALS_FAILED: {
            auto e = std::make_exception_ptr(exception::authentication(
              ...
              "ClientConfig::setClusterName()) does not match the one configured in the cluster or "
  ...
  serialization::pimpl::SerializationService& ss)
{
    ...
    if (!credential) {
        return protocol::codec::client_authentication_encode(cluster_name,
                                                             nullptr,
                                                             nullptr,
                                                             client_uuid_,
  ...
  const std::weak_ptr<client::impl::hazelcast_client_instance_impl>& client_impl)
{
    ...
    std::thread([client_impl] {
        std::shared_ptr<client::impl::hazelcast_client_instance_impl> clientInstance =
          client_impl.lock();
    ...
    boost::asio::post(executor_->get_executor(), [=]() {
        try {
            ...
            if (active_connections_.empty()) {
                HZ_LOG(logger_,
                       finest,
                       boost::str(boost::format("No connection to cluster: %1%") % cluster_id_));
...
ClientConnectionManagerImpl::connect_to_all_members()
{
    ...
    auto connecting_addresses =
      std::make_shared<util::sync_associative_container<std::unordered_set<address>>>();
...
ClientConnectionManagerImpl::do_connect_to_cluster()
{
    ...
    while (attempt < connection_attempt_limit_) {
        ...
        // If the address providers load no addresses (which seems to be possible), then the above
        // loop is not entered and the lifecycle check is missing, hence we need to repeat the same
        // check at this point.
        check_client_active();

        if (attempt < connection_attempt_limit_) {
            auto remainingTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                                   nextTryTime - std::chrono::steady_clock::now())
                                   .count();
            HZ_LOG(logger_,
                   warning,
                   boost::str(boost::format("Unable to get alive cluster connection, try in "
                                            "%1% ms later, attempt %2% of %3%.") %
                              (remainingTime > 0 ? remainingTime : 0) % attempt %
...
void
ClientConnectionManagerImpl::on_connection_close(Connection& connection, std::exception_ptr cause)
{
...
std::shared_ptr<Connection>
ClientConnectionManagerImpl::connect(const address& address)
{
    try {
        HZ_LOG(logger_, info, boost::str(boost::format("Trying to connect to %1%") % address));
...
void
ClientConnectionManagerImpl::handle_successful_auth(const std::shared_ptr<Connection>& connection,
                                                    auth_response response)
{
    ...
    auto changedCluster =
      initial_connection && !current_cluster_id.is_nil() && !(new_cluster_id == current_cluster_id);
    if (changedCluster) {
        HZ_LOG(
          logger_,
          warning,
    ...
    if (local_address) {
        HZ_LOG(logger_,
               ...
               boost::str(boost::format("Authenticated with server %1%:%2%, server version: %3%, "
                                        "local address: %4%") %
                          response.server_address % response.member_uuid % response.server_version %
                          *local_address));
    } else {
        HZ_LOG(logger_,
               info,
...
void
ClientConnectionManagerImpl::fire_life_cycle_event(lifecycle_event::lifecycle_state state)
{
...
ClientConnectionManagerImpl::check_partition_count(int32_t new_partition_count)
{
    auto& partition_service =
      static_cast<spi::impl::ClientPartitionServiceImpl&>(client_.get_partition_service());
...
ClientConnectionManagerImpl::check_invocation_allowed()
{
    ...
    if (async_start_) {
        ...
        BOOST_THROW_EXCEPTION(exception::io("ClientConnectionManagerImpl::check_invocation_allowed",
...
ReadHandler::handle()
{
    ...
    // it is important to check the ondata return value since there may be left data less than a
    // message header size, and this may cause an infinite loop.
...
Connection::connect()
{
    ...
    auto backupTimeout = static_cast<spi::impl::ClientInvocationServiceImpl&>(invocation_service_)
                           .get_backup_timeout();
...
void
Connection::schedule_periodic_backup_cleanup(std::chrono::milliseconds backup_timeout,
...
Connection::close(const std::string& reason, std::exception_ptr cause)
{
    ...
    try {
        ...
        HZ_LOG(
          ...
          warning,
          boost::str(boost::format("Exception while closing connection %1%") % e.get_message()));
    }
...
void
Connection::write(const std::shared_ptr<spi::impl::ClientInvocation>& client_invocation)
{
...
void
Connection::handle_client_message(const std::shared_ptr<protocol::ClientMessage>& message)
{
    ...
    if (client_context_.get_lifecycle_service().is_running()) {
        if (!close_cause_) {
            ...
            try {
                ...
                HZ_LOG(logger_, warning, boost::str(boost::format("%1%%2%") % message.str() % ie));
            }
        }
    } else {
        HZ_LOG(
...
{
    os << "ClientConnection{"
       << "alive=" << connection.is_alive() << ", connectionId=" << connection.get_connection_id()
       << ", remoteEndpoint=";
    ...
    os << ", lastReadTime=" << util::StringUtil::time_to_string(connection.last_read_time())
       << ", closedTime="
       << util::StringUtil::time_to_string(std::chrono::steady_clock::time_point(
            std::chrono::duration_cast<std::chrono::steady_clock::duration>(
  ...
  , logger_(client.get_logger())
{
    ...
    heartbeat_timeout_ = std::chrono::milliseconds(
      timeout_millis > 0
        ? timeout_millis
        : util::IOUtil::to_value<int64_t>(client_properties::PROP_HEARTBEAT_TIMEOUT_DEFAULT));
...
void
HeartbeatManager::check_connection(const std::shared_ptr<Connection>& connection)
{
                                       ...
                                       const std::string& reason)
{
    connection->close(
      reason,
      std::make_exception_ptr((exception::exception_builder<exception::target_disconnected>(
...
SocketFactory::start()
{
    ...
    const client::config::ssl_config& sslConfig =
...
SocketFactory::create(const address& address, std::chrono::milliseconds& connect_timeout_in_millis)
{
#ifdef HZ_BUILD_WITH_SSL
    if (ssl_context_.get()) {
        return std::unique_ptr<hz_socket>(new internal::socket::SSLSocket(
          io_,
          *ssl_context_,
          address,
    ...
    return std::unique_ptr<hz_socket>(new internal::socket::TcpSocket(
      io_,
      address,
      client_context_.get_client_config().get_network_config().get_socket_options(),
      connect_timeout_in_millis,
...
SSLSocket::SSLSocket(boost::asio::io_context& io_service,
                     ...
                     std::chrono::milliseconds& connect_timeout_in_millis,
                     boost::asio::ip::tcp::resolver& resolver)
  : BaseSocket<boost::asio::ssl::stream<boost::asio::ip::tcp::socket>>(resolver,
                                                                       address,
                                                                       socket_options,
                                                                       io_service,
                                                                       connect_timeout_in_millis,
...
operator<<(std::ostream& out, const SSLSocket::CipherInfo& info)
{
    out << "Cipher{"
           "Name: "
        << info.name << ", Bits:" << info.number_of_bits << ", Version:" << info.version
