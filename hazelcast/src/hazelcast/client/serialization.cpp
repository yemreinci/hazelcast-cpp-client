                                           ...
                                           std::shared_ptr<ClassDefinition> def)
{
    ...
    if (def->get_class_id() == 0) {
        BOOST_THROW_EXCEPTION(exception::illegal_argument(
          "ClassDefinitionBuilder::addPortableField", "Portable class id cannot be zero!"));
    }
...
ClassDefinitionBuilder::check()
{
    if (done_) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          "ClassDefinitionBuilder::check",
          "ClassDefinition is already built for " + util::IOUtil::to_string(class_id_)));
    }
}

void
ClassDefinitionBuilder::add_field(const std::string& field_name, field_type const& field_type)
{
    check();
...
{
    return field_name_ == rhs.field_name_ && type_ == rhs.type_ && class_id_ == rhs.class_id_ &&
           factory_id_ == rhs.factory_id_ && version_ == rhs.version_;
}
...
{
    os << "FieldDefinition{"
       << "index: " << definition.index_ << " fieldName: " << definition.field_name_
       << " type: " << static_cast<int32_t>(definition.type_)
       << " classId: " << definition.class_id_ << " factoryId: " << definition.factory_id_
...
object_data_input::object_data_input(
  ...
  : pimpl::data_input<std::vector<byte>>(buffer, offset)
  ...
  : is_default_reader_(is_default_reader)
{
    if (is_default_reader) {
        default_portable_reader_ =
          boost::make_optional(pimpl::DefaultPortableReader(portable_ser, input, cd));
    } else {
        morphing_portable_reader_ =
          boost::make_optional(pimpl::MorphingPortableReader(portable_ser, input, cd));
    }
}
...
ClassDefinition::operator==(const ClassDefinition& rhs) const
{
    return factory_id_ == rhs.factory_id_ && class_id_ == rhs.class_id_ &&
           version_ == rhs.version_ && field_definitions_map_ == rhs.field_definitions_map_;
}
...
{
    os << "ClassDefinition{"
       << "factoryId: " << definition.factory_id_ << " classId: " << definition.class_id_
...
data_output::data_output(bool dont_write)
...
data_output::write(const std::string& str)
{
    ...
    if (len < 0) {
        BOOST_THROW_EXCEPTION(
          (exception::exception_builder<exception::utf_data_format>("DataOutput::write")
           << "String \"" << str << "\" is not UTF-8 formatted !!!")
            .build());
    }
...
{
    os << "typeId: " << static_cast<int32_t>(type.type_id) << " factoryId: " << type.factory_id
...
PortableContext::get_class_definition_context(int factory_id)
{
    ...
    if (value == NULL) {
        value =
          std::shared_ptr<ClassDefinitionContext>(new ClassDefinitionContext(factory_id, this));
...
DefaultPortableWriter::set_position(const std::string& field_name, field_type field_type)
{
    if (raw_) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
    ...
    try {
        ...
        if (written_fields_.find(field_name) != written_fields_.end()) {
            BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
              "PortableWriter::setPosition",
              "Field '" + std::string(field_name) + "' has already been written!"));
        }

        written_fields_.insert(field_name);
        size_t pos = object_data_output_.position();
        int32_t index = fd.get_index();
        object_data_output_.write_at(offset_ + index * util::Bits::INT_SIZE_IN_BYTES,
...
SerializationService::get_object_type(const data* data)
{
    ...
    if (serialization_constants::CONSTANT_TYPE_DATA == type.type_id ||
        ...
        if (serialization_constants::CONSTANT_TYPE_DATA == type.type_id) {
            ...
            if (!identified) {
                BOOST_THROW_EXCEPTION(
                  exception::hazelcast_serialization("SerializationService::getObjectType",
...
ClassDefinitionContext::set_class_version(int class_id, int version)
{
    std::shared_ptr<int> current =
      current_class_versions_.put_if_absent(class_id, std::shared_ptr<int>(new int(version)));
    if (current != NULL && *current != version) {
        std::stringstream error;
        error << "Class-id: " << class_id << " is already registered!";
        BOOST_THROW_EXCEPTION(
          exception::illegal_argument("ClassDefinitionContext::setClassVersion", error.str()));
    }
}
  ...
  , raw_(false)
{
    ...
    try {
        ...
        BOOST_THROW_EXCEPTION(
          exception::hazelcast_serialization("[PortableReaderBase::PortableReaderBase]", e.what()));
    }
    if (fieldCount != cd->get_field_count()) {
        ...
        util::hz_snprintf(msg,
                          ...
                          cd->get_field_count());
        BOOST_THROW_EXCEPTION(
          exception::illegal_state("[PortableReaderBase::PortableReaderBase]", msg));
    }
    this->offset_ = input.position();
}

void
PortableReaderBase::set_position(const std::string& field_name, field_type const& field_type)
{
    if (raw_) {
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
    ...
    if (!cd_->has_field(field_name)) {
        ...
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          "PortableReader::getPosition ", "Don't have a field named " + std::string(field_name)));
    }
    ...
    data_input_->position(offset_ +
                          cd_->get_field(field_name).get_index() * util::Bits::INT_SIZE_IN_BYTES);
                                            ...
                                            int32_t class_id) const
{
    ...
    if (class_id != fd.get_class_id()) {
        char msg[100];
        util::hz_snprintf(
          msg, 100, "Invalid classId! Expected: %d, Current: %d", fd.get_class_id(), class_id);
        BOOST_THROW_EXCEPTION(exception::hazelcast_serialization(
          "DefaultPortableReader::checkFactoryAndClass ", std::string(msg)));
    }
}

MorphingPortableReader::MorphingPortableReader(PortableSerializer& portable_ser,
                                               object_data_input& input,
                                               std::shared_ptr<ClassDefinition> cd)
  : PortableReaderBase(portable_ser, input, cd)
{}
...
std::size_t
hash<std::shared_ptr<hazelcast::client::serialization::pimpl::data>>::operator()(
...
bool
equal_to<std::shared_ptr<hazelcast::client::serialization::pimpl::data>>::operator()(
...
bool
less<std::shared_ptr<hazelcast::client::serialization::pimpl::data>>::operator()(
