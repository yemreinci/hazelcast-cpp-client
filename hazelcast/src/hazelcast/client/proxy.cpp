...
{
    // TODO: remove this line once the client instance get_distributed_object works as expected in
    // Java for this proxy type
    lock_reference_id_generator_ = get_context().get_lock_reference_id_generator();
}

boost::future<bool>
MultiMapImpl::put(const serialization::pimpl::data& key, const serialization::pimpl::data& value)
{
    auto request =
      protocol::codec::multimap_put_encode(get_name(), key, value, util::get_current_thread_id());
...
{
    auto request =
      protocol::codec::multimap_get_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<std::vector<serialization::pimpl::data>>(request, key);
}

boost::future<bool>
MultiMapImpl::remove(const serialization::pimpl::data& key, const serialization::pimpl::data& value)
{
...
{
    auto request =
      protocol::codec::multimap_remove_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<std::vector<serialization::pimpl::data>>(request, key);
}
...
{
    auto request =
      protocol::codec::multimap_containskey_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<bool>(request, key);
}
...
{
    auto request =
      protocol::codec::multimap_valuecount_encode(get_name(), key, util::get_current_thread_id());
...
MultiMapImpl::add_entry_listener(std::shared_ptr<impl::BaseEventHandler> entry_event_handler,
                                 bool include_value)
{
    return register_listener(create_multi_map_entry_listener_codec(include_value),
...
boost::future<void>
MultiMapImpl::lock(const serialization::pimpl::data& key, std::chrono::milliseconds lease_time)
{
...
boost::future<bool>
MultiMapImpl::try_lock(const serialization::pimpl::data& key, std::chrono::milliseconds timeout)
{
...
MultiMapImpl::MultiMapEntryListenerMessageCodec::MultiMapEntryListenerMessageCodec(
  std::string name,
  bool include_value)
  : name_(std::move(name))
...
{
    ringbuffer_ =
      context->get_hazelcast_client_implementation()->get_distributed_object<ringbuffer>(
        std::string(TOPIC_RB_PREFIX) + name_);
}

boost::future<void>
ReliableTopicImpl::publish(serialization::pimpl::data&& data)
{
...
PNCounterImpl::get()
{
    ...
    if (!target) {
        BOOST_THROW_EXCEPTION(
          exception::no_data_member_in_cluster("ClientPNCounterProxy::get",
                                               "Cannot invoke operations on a CRDT because the "
                                               "cluster does not contain any data members"));
    }
...
PNCounterImpl::get_and_add(int64_t delta)
{
    ...
    if (!target) {
        BOOST_THROW_EXCEPTION(
          exception::no_data_member_in_cluster("ClientPNCounterProxy::getAndAdd",
                                               "Cannot invoke operations on a CRDT because the "
                                               "cluster does not contain any data members"));
    }
...
PNCounterImpl::add_and_get(int64_t delta)
{
    ...
    if (!target) {
        BOOST_THROW_EXCEPTION(
          exception::no_data_member_in_cluster("ClientPNCounterProxy::addAndGet",
                                               "Cannot invoke operations on a CRDT because the "
                                               "cluster does not contain any data members"));
    }
...
std::vector<member>
PNCounterImpl::get_replica_addresses(const std::unordered_set<member>& excluded_members)
{
...
PNCounterImpl::get_max_configured_replica_count()
{
    if (max_configured_replica_count_ > 0) {
        ...
        auto request = protocol::codec::pncounter_getconfiguredreplicacount_encode(get_name());
                                   ...
                                   const boost::shared_ptr<member>& target)
{
    ...
    try {
        ...
        auto request =
          protocol::codec::pncounter_get_encode(get_name(), timestamps, target->get_uuid());
        return invoke_on_member(request, target->get_uuid())
          .then(boost::launch::deferred, [=](boost::future<protocol::ClientMessage> f) {
              try {
...
boost::future<int64_t>
PNCounterImpl::invoke_add_internal(int64_t delta,
                                   ...
                                   const boost::shared_ptr<member>& target)
{
    ...
    try {
        auto request = protocol::codec::pncounter_add_encode(get_name(),
                                                             delta,
                                                             getBeforeUpdate,
...
int64_t
PNCounterImpl::get_and_update_timestamps(boost::future<protocol::ClientMessage> f)
{
    auto msg = f.get();
...
{
    return std::shared_ptr<impl::vector_clock>(new impl::vector_clock(replica_logical_timestamps));
}
  ...
  : ProxyImpl("hz:impl:listService", instance_name, context)
{
    serialization::pimpl::data key_data =
      get_context().get_serialization_service().to_data<std::string>(&instance_name);
...
boost::future<bool>
IListImpl::contains_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
boost::future<bool>
IListImpl::add_all_data(int index, const std::vector<serialization::pimpl::data>& elements)
{
    auto request = protocol::codec::list_addallwithindex_encode(get_name(), index, elements);
    return invoke_and_get_future<bool>(request, partition_id_);
}

boost::future<bool>
IListImpl::remove_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
boost::future<bool>
IListImpl::retain_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
IListImpl::ListListenerMessageCodec::ListListenerMessageCodec(std::string name, bool include_value)
  : name_(std::move(name))
  , include_value_(include_value)
{}
...
flake_id_generator_impl::IdBatch::IdIterator::IdIterator(int64_t base2,
...
flake_id_generator_impl::flake_id_generator_impl(const std::string& service_name,
                                                 const std::string& object_name,
                                                 spi::ClientContext* context)
  : ProxyImpl(service_name, object_name, context)
  , block_(nullptr)
...
flake_id_generator_impl::new_id()
{
    try {
        ...
        return new_id_batch(batch_size_)
          .then(boost::launch::deferred, [=](boost::future<flake_id_generator_impl::IdBatch> f) {
...
{
    auto request = protocol::codec::flakeidgenerator_newidbatch_encode(get_name(), size);
...
boost::future<bool>
IQueueImpl::offer(const serialization::pimpl::data& element, std::chrono::milliseconds timeout)
{
    auto request = protocol::codec::queue_offer_encode(
      get_name(), element, std::chrono::duration_cast<std::chrono::milliseconds>(timeout).count());
    return invoke_and_get_future<bool>(request, partition_id_);
}
...
IQueueImpl::poll_data(std::chrono::milliseconds timeout)
{
    auto request = protocol::codec::queue_poll_encode(
      get_name(), std::chrono::duration_cast<std::chrono::milliseconds>(timeout).count());
    return invoke_and_get_future<boost::optional<serialization::pimpl::data>>(request,
                                                                              partition_id_);
}
...
boost::future<bool>
IQueueImpl::contains_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
boost::future<bool>
IQueueImpl::add_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
boost::future<bool>
IQueueImpl::remove_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
boost::future<bool>
IQueueImpl::retain_all_data(const std::vector<serialization::pimpl::data>& elements)
{
...
SerializingProxy::invoke_on_partition(protocol::ClientMessage& request, int partition_id)
{
    try {
...
boost::future<protocol::ClientMessage>
SerializingProxy::invoke_on_connection(protocol::ClientMessage& request,
...
{
    return decode_optional_var_sized<map::data_entry_view>(invoke_on_key_owner(request, key));
}
...
PartitionSpecificClientProxy::on_initialize()
{
    std::string partitionKey =
      internal::partition::strategy::StringPartitioningStrategy::get_partition_key(name_);
    partition_id_ =
      get_context().get_partition_service().get_partition_id(to_data<std::string>(partitionKey));
}
...
{
    auto request =
      protocol::codec::map_containskey_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<bool>(request, key);
}
...
{
    auto request =
      protocol::codec::map_remove_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<boost::optional<serialization::pimpl::data>>(request, key);
}

boost::future<bool>
IMapImpl::remove(const serialization::pimpl::data& key, const serialization::pimpl::data& value)
{
...
{
    auto request =
      protocol::codec::map_delete_encode(get_name(), key, util::get_current_thread_id());
...
boost::future<bool>
IMapImpl::try_remove(const serialization::pimpl::data& key, std::chrono::milliseconds timeout)
{
...
{
    auto request =
      protocol::codec::map_replace_encode(get_name(), key, value, util::get_current_thread_id());
...
boost::future<protocol::ClientMessage>
IMapImpl::lock(const serialization::pimpl::data& key, std::chrono::milliseconds lease_time)
{
...
{
    auto request =
      protocol::codec::map_unlock_encode(get_name(),
                                         key,
                                         util::get_current_thread_id(),
                                         lock_reference_id_generator_->get_next_reference_id());
    return invoke_on_partition(request, get_partition_id(key));
...
// TODO: We can use generic template Listener instead of impl::BaseEventHandler to prevent the
...
{
    return register_listener(create_map_entry_listener_codec(include_value, listener_flags),
                             std::move(entry_event_handler));
}
...
{
    return register_listener(
      create_map_entry_listener_codec(include_value, listener_flags, std::move(key)),
      std::move(entry_event_handler));
}
...
{
    auto request =
      protocol::codec::map_getentryview_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<boost::optional<map::data_entry_view>>(request, key);
}
...
{
    auto request =
      protocol::codec::map_evict_encode(get_name(), key, util::get_current_thread_id());
    return invoke_and_get_future<bool>(request, key);
}
  ...
  protocol::codec::holder::paging_predicate_holder const& predicate)
{
    ...
    return invoke(request).then(
      boost::launch::deferred, [=](boost::future<protocol::ClientMessage> f) {
          return get_paging_predicate_response<std::vector<serialization::pimpl::data>>(
            std::move(f));
      });
}
  ...
  protocol::codec::holder::paging_predicate_holder const& predicate)
{
    ...
    return invoke(request).then(boost::launch::deferred,
                             ...
                             const serialization::pimpl::data& processor)
{
    auto request = protocol::codec::map_submittokey_encode(
      get_name(), processor, key, util::get_current_thread_id());
    return invoke_on_partition(request, get_partition_id(key))
      .then([](boost::future<protocol::ClientMessage> f) {
          auto msg = f.get();
          msg.skip_frame();
...
boost::future<EntryVector>
IMapImpl::execute_on_keys_data(const std::vector<serialization::pimpl::data>& keys,
                               const serialization::pimpl::data& processor)
{
...
boost::future<EntryVector>
IMapImpl::execute_on_entries_data(const serialization::pimpl::data& entry_processor)
{
    auto request = protocol::codec::map_executeonallkeys_encode(get_name(), entry_processor);
    return invoke_and_get_future<EntryVector>(request);
}
...
{
    auto request =
      protocol::codec::map_executewithpredicate_encode(get_name(), entry_processor, predicate);
    return invoke_and_get_future<EntryVector>(request);
}
...
IMapImpl::MapEntryListenerToKeyCodec::encode_add_request(bool local_only) const
{
    return protocol::codec::map_addentrylistenertokey_encode(
      name_, key_, include_value_, static_cast<int32_t>(listener_flags_), local_only);
}
...
IMapImpl::MapEntryListenerToKeyCodec::MapEntryListenerToKeyCodec(std::string name,
                                                                 bool include_value,
                                                                 int32_t listener_flags,
                                                                 serialization::pimpl::data key)
  : name_(std::move(name))
...
IMapImpl::MapEntryListenerWithPredicateMessageCodec::encode_add_request(bool local_only) const
{
    return protocol::codec::map_addentrylistenerwithpredicate_encode(
      name_, predicate_, include_value_, static_cast<int32_t>(listener_flags_), local_only);
}
...
boost::future<boost::uuids::uuid>
ITopicImpl::add_message_listener(std::shared_ptr<impl::BaseEventHandler> topic_event_handler)
{
...
ITopicImpl::TopicListenerMessageCodec::TopicListenerMessageCodec(std::string name)
  : name_(std::move(name))
{}
...
data_entry_view::data_entry_view(data&& key,
                                 data&& value,
                                 int64_t cost,
...
namespace impl {
namespace reliable {
ReliableTopicExecutor::ReliableTopicExecutor(std::shared_ptr<ringbuffer> rb, logger& lg)
  : ringbuffer_(std::move(rb))
  , q_(10)
  , shutdown_(false)
{
...
void
ReliableTopicExecutor::start()
{}

bool
ReliableTopicExecutor::stop()
{
    bool expected = false;
...
void
ReliableTopicExecutor::execute(Message m)
{
    q_.push(m);
}

void
ReliableTopicExecutor::Task::run()
{
    while (!shutdown_) {
        Message m = q_.pop();
        if (CANCEL == m.type) {
        ...
        try {
            ...
            while (!shutdown_ &&
                   f.wait_for(boost::chrono::seconds(1)) != boost::future_status::ready) {
            }
            if (f.is_ready()) {
...
std::string
ReliableTopicExecutor::Task::get_name() const
{
...
{
    out.write<int64_t>(
      std::chrono::duration_cast<std::chrono::milliseconds>(object.publish_time_.time_since_epoch())
        .count());
...
hz_serializer<topic::impl::reliable::ReliableTopicMessage>::read_data(object_data_input& in)
{
    ...
    message.publish_time_ =
      now + std::chrono::milliseconds(in.read<int64_t>()) - now.time_since_epoch();
...
operator<<(std::ostream& os, const entry_event& event)
{
    os << "name: " << event.name_ << " member: " << event.member_
       << " eventType: " << static_cast<int>(event.event_type_) << " key: " << event.key_.get_type()
       << " value: " << event.value_.get_type() << " oldValue: " << event.old_value_.get_type()
...
operator<<(std::ostream& os, const map_event& event)
{
    os << "MapEvent{member: " << event.member_
       << " eventType: " << static_cast<int>(event.event_type_) << " name: " << event.name_
