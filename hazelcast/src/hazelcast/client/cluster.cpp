...
const cluster&
membership_event::get_cluster() const
{
    return cluster_;
}
...
void
RoundRobinLB::init(cluster& cluster)
{
...
RoundRobinLB::RoundRobinLB(const RoundRobinLB& rhs)
  : index_(rhs.index_.load())
{}

void
RoundRobinLB::operator=(const RoundRobinLB& rhs)
{
...
void
AbstractLoadBalancer::operator=(const AbstractLoadBalancer& rhs)
{
...
AbstractLoadBalancer::init(cluster& cluster)
{
    ...
    cluster.add_membership_listener(
      membership_listener()
        .on_init([this](const initial_membership_event&) { set_members_ref(); })
        .on_joined([this](const membership_event&) { set_members_ref(); })
        .on_left([this](const membership_event&) { set_members_ref(); }));
}

void
AbstractLoadBalancer::set_members_ref()
{
...
std::vector<member>
AbstractLoadBalancer::get_members()
{
    std::lock_guard<std::mutex> lg(members_lock_);
    return members_ref_;
}
...
vector_clock::is_after(vector_clock& other)
{
    ...
    for (const vector_clock::timestamp_map::value_type& otherEntry : other.replica_timestamps_) {
        ...
        if (!localReplicaTimestamp.first || localReplicaTimestamp.second < otherReplicaTimestamp) {
            return false;
...
namespace std {
std::size_t
hash<hazelcast::client::member>::operator()(const hazelcast::client::member& k) const noexcept
{
