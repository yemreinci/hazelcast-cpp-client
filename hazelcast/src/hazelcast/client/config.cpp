...
int32_t
client_network_config::get_connection_attempt_limit() const
{
...
std::chrono::milliseconds
client_network_config::get_connection_attempt_period() const
{
...
client_network_config::set_connection_attempt_period(const std::chrono::milliseconds& interval)
{
    util::Preconditions::check_not_negative(
      interval.count(),
      (boost::format("Provided connectionAttemptPeriod(%1% msecs) cannot be negative") %
       interval.count())
        .str());
...
reliable_topic_config::reliable_topic_config(const char* topic_name)
...
{
    this->access_key_ =
      util::Preconditions::check_has_text(access_key, "accessKey must contain text");
    return *this;
}
...
{
    this->secret_key_ =
      util::Preconditions::check_has_text(secret_key, "secretKey must contain text");
    return *this;
}
...
{
    this->host_header_ =
      util::Preconditions::check_has_text(host_header, "hostHeader must contain text");
    return *this;
}
...
operator<<(std::ostream& out, const client_aws_config& config)
{
    return out << "client_aws_config{"
               << "enabled=" << config.is_enabled() << ", region='" << config.get_region() << '\''
               << ", securityGroupName='" << config.get_security_group_name() << '\''
               << ", tagKey='" << config.get_tag_key() << '\'' << ", tagValue='"
               << config.get_tag_value() << '\'' << ", hostHeader='" << config.get_host_header()
                                               ...
                                               const std::string& item_name) const
{
    ...
    if (duplicate.get() != NULL) {
        throw(exception::exception_builder<exception::invalid_configuration>(
              ...
              << "Configuration " << item_name << " has duplicate configuration. Candidate:"
...
int
matching_point_config_pattern_matcher::get_matching_point(const std::string& pattern,
                                                          const std::string& item_name) const
{
...
eviction_config&
eviction_config::set_maximum_size_policy(const eviction_config::max_size_policy& max_size_policy)
{
...
{
    out << "EvictionConfig{"
        << "size=" << config.get_size() << ", maxSizePolicy=" << config.get_maximum_size_policy()
...
near_cache_config&
near_cache_config::set_in_memory_format(const in_memory_format& in_memory_format)
{
...
near_cache_config::calculate_max_size(int32_t max_size)
{
    return (max_size == 0)
             ? INT32_MAX
             : util::Preconditions::check_not_negative(max_size, "Max-size cannot be negative!");
}
...
operator<<(std::ostream& out, const near_cache_config& config)
{
    out << "NearCacheConfig{"
        ...
        << ", localUpdatePolicy=" << config.local_update_policy_ << config.eviction_config_;
    out << '}';
...
{
    std::string baseName =
      internal::partition::strategy::StringPartitioningStrategy::get_base_name(name);
    auto config =
      internal::config::ConfigUtils::lookup_by_pattern<config::client_flake_id_generator_config>(
...
client_config::get_flake_id_generator_config(const std::string& name)
{
    ...
    if (defConfig == flake_id_generator_config_map_.end()) {
        flake_id_generator_config_map_.emplace("default",
                                               config::client_flake_id_generator_config("default"));
    }
