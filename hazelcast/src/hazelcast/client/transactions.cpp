...
namespace client {
namespace txn {
TransactionProxy::TransactionProxy(transaction_options& txn_options,
    ...
    try {
        if (state_ != TxnState::ACTIVE) {
            BOOST_THROW_EXCEPTION(
              exception::illegal_state("TransactionProxy::commit()", "Transaction is not active"));
        }
...
TransactionProxy::check_thread()
{
    if (thread_id_ != util::get_current_thread_id()) {
        BOOST_THROW_EXCEPTION(exception::illegal_state(
          "TransactionProxy::checkThread()", "Transaction cannot span multiple threads!"));
    }
}
...
TransactionProxy::check_timeout()
{
    if (start_time_ + options_.get_timeout() < std::chrono::steady_clock::now()) {
        BOOST_THROW_EXCEPTION(
          exception::transaction("TransactionProxy::checkTimeout()", "Transaction is timed-out!"));
    }
}
...
TransactionProxy::invoke(protocol::ClientMessage& request)
{
    return client_transaction_util::invoke(
      request, boost::uuids::to_string(get_txn_id()), client_context_, connection_);
}
...
const std::shared_ptr<util::exception_util::runtime_exception_factory>
  client_transaction_util::exceptionFactory(new class transaction_exception_factory());
                                ...
                                const std::shared_ptr<connection::Connection>& connection)
{
    try {
        std::shared_ptr<spi::impl::ClientInvocation> clientInvocation =
          spi::impl::ClientInvocation::create(client, request, object_name, connection);
...
namespace proxy {
TransactionalMapImpl::TransactionalMapImpl(const std::string& name,
...
TransactionalSetImpl::TransactionalSetImpl(const std::string& name,
                                           txn::TransactionProxy& transaction_proxy)
...
transaction_options::set_timeout(std::chrono::milliseconds duration)
{
    if (duration.count() <= 0) {
        BOOST_THROW_EXCEPTION(
          exception::illegal_state("TransactionOptions::setTimeout", "Timeout must be positive!"));
    }
    timeout_ = duration;
    return *this;
}
