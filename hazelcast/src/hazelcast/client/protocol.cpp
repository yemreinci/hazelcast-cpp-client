  ...
  : retryable_(false)
{
    ...
    initial_frame->flags = is_fingle_frame
...
template<>
void
ClientMessage::set(const std::vector<std::pair<boost::uuids::uuid, int64_t>>& values, bool is_final)
{
...
void
ClientMessage::set(const codec::holder::paging_predicate_holder& p, bool is_final)
{
    add_begin_frame();
                                 ...
                                 size_t& remaining_bytes_in_frame)
{
    ...
    // If there were bytes of a frame (remaining_bytes_in_frame) to read from the previous call, it
    // is read.
    ...
    while (remaining_bytes_in_frame == 0 && !is_final &&
           (remaining = byte_buff.remaining()) >= ClientMessage::SIZE_OF_FRAME_LENGTH_AND_FLAGS) {
...
{
    os << "ClientMessage{length=" << msg.size() << ", operation=" << msg.get_operation_name()
    ...
    if (unfragmented) {
        os << ", correlationId = " << msg.get_correlation_id() << ", messageType = 0x" << std::hex
           << msg.get_message_type() << std::dec << ", flags = 0x" << std::hex
           << msg.get_header_flags() << std::dec << ", is backup aware = "
           << ClientMessage::is_flag_set(msg.get_header_flags(), ClientMessage::BACKUP_AWARE_FLAG)
           << ", is backup event = "
           << ClientMessage::is_flag_set(msg.get_header_flags(), ClientMessage::BACKUP_EVENT_FLAG)
           << ", isEvent = "
           << ClientMessage::is_flag_set(msg.get_header_flags(), ClientMessage::IS_EVENT_FLAG)
           << "}";
    } else if (begin_fragment) {
        os << ", fragmentationId = "
           << boost::endian::load_little_s64(
                &msg.data_buffer_[0][ClientMessage::FRAGMENTATION_ID_OFFSET])
           << ", correlationId = " << msg.get_correlation_id() << ", messageType = 0x" << std::hex
           << msg.get_message_type() << std::dec << ", flags = 0x" << std::hex
           << msg.get_header_flags() << std::dec << ", is backup aware = "
           << ClientMessage::is_flag_set(msg.get_header_flags(), ClientMessage::BACKUP_AWARE_FLAG)
           << ", is backup event = "
...
{
    register_exception(UNDEFINED, new ExceptionFactoryImpl<exception::undefined_error_code>());
    ...
    register_exception(ILLEGAL_ACCESS_ERROR,
    ...
    register_exception(OUT_OF_MEMORY_ERROR,
                       new ExceptionFactoryImpl<exception::out_of_memory_error>());
    register_exception(STACK_OVERFLOW_ERROR,
    ...
    register_exception(LOCAL_MEMBER_RESET,
...
ClientExceptionFactory::~ClientExceptionFactory()
{
    ...
    for (std::unordered_map<int, hazelcast::client::protocol::ExceptionFactory*>::const_iterator
           it = error_code_to_factory_.begin();
...
void
ClientExceptionFactory::register_exception(int32_t error_code, ExceptionFactory* factory)
{
...
std::exception_ptr
ClientExceptionFactory::create_exception(std::vector<codec::ErrorHolder>::const_iterator begin,
...
std::exception_ptr
ClientExceptionFactory::create_exception(const std::vector<codec::ErrorHolder>& errors) const
{
