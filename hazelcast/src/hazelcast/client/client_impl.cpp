...
boost::uuids::uuid
hazelcast_client::add_lifecycle_listener(lifecycle_listener&& lifecycle_listener)
{
    return client_impl_->add_lifecycle_listener(std::move(lifecycle_listener));
}

bool
hazelcast_client::remove_lifecycle_listener(const boost::uuids::uuid& registration_id)
{
  ...
  , proxy_session_manager_(client_context_)
{
    ...
    int32_t maxAllowedConcurrentInvocations =
      client_properties_.get_integer(client_properties_.get_max_concurrent_invocations());
    int64_t backofftimeoutMs =
      client_properties_.get_long(client_properties_.get_backpressure_backoff_timeout_millis());
    bool isBackPressureEnabled = maxAllowedConcurrentInvocations != INT32_MAX;
    call_id_sequence_ = spi::impl::sequence::CallIdFactory::new_call_id_sequence(
      isBackPressureEnabled, maxAllowedConcurrentInvocations, backofftimeoutMs);

    std::vector<std::shared_ptr<connection::AddressProvider>> addressProviders =
      create_address_providers();
...
bool
hazelcast_client_instance_impl::remove_lifecycle_listener(const boost::uuids::uuid& registration_id)
{
...
{
    auto eventThreadCount =
      client_properties_.get_integer(client_properties_.get_event_thread_count());
    return std::make_shared<spi::impl::listener::listener_service_impl>(client_context_,
                                                                        eventThreadCount);
}
...
{
    HZ_LOG(*logger_, info, "Clearing local state of the client, because of a cluster restart");
...
hazelcast_client_instance_impl::create_address_providers()
{
    ...
    if (awsConfig.is_enabled()) {
        int awsMemberPort =
          client_properties_.get_integer(client_properties_.get_aws_member_port());
        if (awsMemberPort < 0 || awsMemberPort > 65535) {
            throw(exception::exception_builder<exception::invalid_configuration>(
                    "hazelcast_client_instance_impl::createAddressProviders")
                  << "Configured aws member port " << awsMemberPort
                  << " is not a valid port number. It should be between 0-65535 inclusive.")
              .build();
        }
...
hazelcast_client_instance_impl::get_distributed_object(const std::string& name)
{
    ...
    if (nearCacheConfig) {
        return proxy_manager_.get_or_create_proxy<
          map::NearCachedClientMapProxy<serialization::pimpl::data, serialization::pimpl::data>>(
...
address::address(const std::string& hostname, int port, unsigned long scope_id)
  : host_(hostname)
  , port_(port)
  , type_(IPV6)
  , scope_id_(scope_id)
...
void
hz_serializer<address>::write_data(const address& object, object_data_output& out)
{
...
iexecutor_service::iexecutor_service(const std::string& name, spi::ClientContext* context)
...
iexecutor_service::select_members(const member_selector& member_selector)
{
    ...
    if (selected.empty()) {
        BOOST_THROW_EXCEPTION(exception::rejected_execution(
          "IExecutorService::selectMembers", "No member could be selected with member selector"));
    }
    return selected;
}
...
iexecutor_service::invoke_on_target(protocol::ClientMessage&& request, boost::uuids::uuid target)
{
    try {
        std::shared_ptr<spi::impl::ClientInvocation> clientInvocation =
          spi::impl::ClientInvocation::create(get_context(), request, get_name(), target);
...
iexecutor_service::invoke_on_partition_owner(protocol::ClientMessage&& request, int partition_id)
{
    try {
        std::shared_ptr<spi::impl::ClientInvocation> clientInvocation =
          spi::impl::ClientInvocation::create(get_context(), request, get_name(), partition_id);
        return std::make_pair(clientInvocation->invoke(), clientInvocation);
    } catch (exception::iexception&) {
        util::exception_util::rethrow(std::current_exception());
    }
...
{
    auto request = protocol::codec::executorservice_isshutdown_encode(get_name());
...
const std::string client_properties::CLEAN_RESOURCES_PERIOD_MILLIS =
...
const std::string client_properties::RESPONSE_EXECUTOR_THREAD_COUNT =
  "hazelcast.client.response.executor.thread.count";
...
client_properties::client_properties(const std::unordered_map<std::string, std::string>& properties)
  ...
  , clean_resources_period_(CLEAN_RESOURCES_PERIOD_MILLIS, CLEAN_RESOURCES_PERIOD_MILLIS_DEFAULT)
  , invocation_retry_pause_millis_(INVOCATION_RETRY_PAUSE_MILLIS,
                                   INVOCATION_RETRY_PAUSE_MILLIS_DEFAULT)
  , invocation_timeout_seconds_(INVOCATION_TIMEOUT_SECONDS, INVOCATION_TIMEOUT_SECONDS_DEFAULT)
  , event_thread_count_(EVENT_THREAD_COUNT, EVENT_THREAD_COUNT_DEFAULT)
  , internal_executor_pool_size_(INTERNAL_EXECUTOR_POOL_SIZE, INTERNAL_EXECUTOR_POOL_SIZE_DEFAULT)
  , shuffle_member_list_(SHUFFLE_MEMBER_LIST, SHUFFLE_MEMBER_LIST_DEFAULT)
  , max_concurrent_invocations_(MAX_CONCURRENT_INVOCATIONS, MAX_CONCURRENT_INVOCATIONS_DEFAULT)
  ...
  , response_executor_thread_count_(RESPONSE_EXECUTOR_THREAD_COUNT,
                                    RESPONSE_EXECUTOR_THREAD_COUNT_DEFAULT)
...
const client_property&
client_properties::get_clean_resources_period_millis() const
{
...
const client_property&
client_properties::get_response_executor_thread_count() const
{
...
iexception::iexception(const std::string& exception_name,
  ...
  , report_((boost::format("%1% {%2%. Error code:%3%, Details:%4%.} at %5%.") % exception_name %
             message % error_no % details % source)
              .str())
{}
...
retryable_hazelcast::retryable_hazelcast(const std::string& error_name,
                                         ...
                                         bool runtime,
                                         bool retryable)
  : hazelcast_(error_name, error_code, source, message, details, cause, runtime, retryable)
{}
...
member_left::member_left(const std::string& source,
                         ...
                         const std::string& details,
                         std::exception_ptr cause)
  : execution("member_left", protocol::MEMBER_LEFT, source, message, details, cause, false, true)
{}
