...
initial_membership_event::initial_membership_event(cluster& cluster,
...
ProxyManager::initialize(const std::shared_ptr<ClientProxy>& client_proxy)
{
    ...
    spi::impl::ClientInvocation::create(client_, clientMessage, client_proxy->get_service_name())
      ->invoke()
      .get();
...
lifecycle_service::start()
{
    ...
    if (!connectionStrategyConfig.is_async_start()) {
        // The client needs to open connections to all members before any services requiring
        // internal listeners start
        ...
        case lifecycle_event::STARTING: {
            ...
            HZ_LOG(lg,
                   ...
                   (boost::format(
                    ...
                    date % commitId % client_context_.get_connection_manager().get_client_uuid())
                     .str());
            char msg[100];
...
{
    auto clientMessage =
      protocol::codec::client_destroyproxy_encode(get_name(), get_service_name());
    return spi::impl::ClientInvocation::create(
             get_context(),
             std::make_shared<protocol::ClientMessage>(std::move(clientMessage)),
             get_name())
      ->invoke()
      .then(boost::launch::deferred, [](boost::future<protocol::ClientMessage> f) { f.get(); });
}
...
ClientInvocationServiceImpl::ClientInvocationServiceImpl(ClientContext& client)
  ...
  , fail_on_indeterminate_operation_state_(client.get_client_properties().get_boolean(
      client.get_client_properties().fail_on_indeterminate_state()))
  , backup_timeout_(std::chrono::milliseconds(client.get_client_properties().get_integer(
...
bool
ClientInvocationServiceImpl::send(const std::shared_ptr<impl::ClientInvocation>& invocation,
...
bool
ClientInvocationServiceImpl::invoke(std::shared_ptr<ClientInvocation> invocation)
{
...
ClientInvocationServiceImpl::ResponseProcessor::ResponseProcessor(
  ...
  : logger_(lg)
  , client_(client_context)
{}

void
ClientInvocationServiceImpl::ResponseProcessor::process_internal(
  const std::shared_ptr<ClientInvocation>& invocation,
  const std::shared_ptr<protocol::ClientMessage>& response)
{
    try {
        ...
        HZ_LOG(logger_,
               severe,
               boost::str(boost::format("Failed to process response for %1%. %2%") % *invocation %
                          e.what()));
    }
}

void
ClientInvocationServiceImpl::ResponseProcessor::shutdown()
{
    ClientExecutionServiceImpl::shutdown_thread_pool(pool_.get());
}

void
ClientInvocationServiceImpl::ResponseProcessor::start()
{
    client_properties& clientProperties = client_.get_client_properties();
    auto threadCount =
      clientProperties.get_integer(clientProperties.get_response_executor_thread_count());
    if (threadCount > 0) {
...
void
ClientInvocationServiceImpl::ResponseProcessor::process(
  const std::shared_ptr<ClientInvocation>& invocation,
  const std::shared_ptr<ClientMessage>& response)
{
    if (!pool_) {
...
DefaultAddressProvider::DefaultAddressProvider(config::client_network_config& network_config,
  ...
  , no_other_address_provider_exist_(no_other_address_provider_exist)
{}
...
const boost::shared_ptr<ClientClusterServiceImpl::member_list_snapshot>
  ClientClusterServiceImpl::EMPTY_SNAPSHOT(new ClientClusterServiceImpl::member_list_snapshot{
    -1 });
...
boost::uuids::uuid
ClientClusterServiceImpl::add_membership_listener_without_init(membership_listener&& listener)
{
...
void
ClientClusterServiceImpl::fire_initial_membership_event(const initial_membership_event& event)
{
    std::lock_guard<std::mutex> g(listeners_lock_);
...
bool
ClientClusterServiceImpl::remove_membership_listener(boost::uuids::uuid registration_id)
{
    std::lock_guard<std::mutex> g(listeners_lock_);
    return listeners_.erase(registration_id) == 1;
}
...
ClientClusterServiceImpl::clear_member_list_version()
{
    ...
    // This check is necessary so that `clear_member_list_version` when handling auth response will
    // not intervene with client failover logic
...
ClientClusterServiceImpl::handle_event(int32_t version, const std::vector<member>& member_infos)
{
    ...
    if (version >= cluster_view_snapshot->version) {
        ...
        if (version >= cluster_view_snapshot->version) {
            ...
            auto snapshot =
              boost::make_shared<member_list_snapshot>(create_snapshot(version, member_infos));
...
void
ClientClusterServiceImpl::apply_initial_state(int32_t version,
...
ClientClusterServiceImpl::detect_membership_events(
  ...
  const std::unordered_map<boost::uuids::uuid, member, boost::hash<boost::uuids::uuid>>&
    current_members)
{
    ...
    for (auto const& e : previous_members) {
        events.emplace_back(client_.get_cluster(),
                            e.second,
        ...
        if (connection) {
            connection->close(
              ...
              std::make_exception_ptr(exception::target_disconnected(
                "ClientClusterServiceImpl::detect_membership_events",
                (boost::format("The client has closed the connection to this member, after "
...
ClientClusterServiceImpl::wait_initial_member_list_fetched() const
{
    ...
    if ((const_cast<boost::latch&>(initial_list_fetched_latch_))
        ...
        BOOST_THROW_EXCEPTION(
          exception::illegal_state("ClientClusterServiceImpl::wait_initial_member_list_fetched",
...
bool
ClientInvocationServiceImpl::invoke_on_target(const std::shared_ptr<ClientInvocation>& invocation,
                                              boost::uuids::uuid uuid)
{
    ...
    if (!connection) {
        HZ_LOG(logger_,
               finest,
               boost::str(boost::format("Client is not connected to target : %1%") % uuid));
        return false;
    }
    return send(invocation, connection);
}
...
ClientExecutionServiceImpl::ClientExecutionServiceImpl(const std::string& name,
  ...
  , user_executor_pool_size_(pool_size)
{}

void
ClientExecutionServiceImpl::start()
{
    int internalPoolSize =
      client_properties_.get_integer(client_properties_.get_internal_executor_pool_size());
    if (internalPoolSize <= 0) {
        internalPoolSize =
          util::IOUtil::to_value<int>(client_properties::INTERNAL_EXECUTOR_POOL_SIZE_DEFAULT);
    }
    ...
    if (user_executor_pool_size_ <= 0) {
        user_executor_pool_size_ =
          4; // hard coded thread pool count in case we could not get the processor count
    }
...
ClientExecutionServiceImpl::get_user_executor() const
{
    return user_executor_->get_executor();
}

void
ClientExecutionServiceImpl::shutdown_thread_pool(hazelcast::util::hz_thread_pool* pool)
{
    if (!pool) {
        return;
    }
...
ClientInvocation::invoke_on_selection()
{
    try {
        ...
        if (is_bind_to_single_connection()) {
            auto invoked =
              invocation_service_.invoke_on_connection(shared_from_this(), connection_);
            if (!invoked) {
                notify_exception(std::make_exception_ptr(exception::io(
                  "", (boost::format("Could not invoke on connection %1%") % *connection_).str())));
            }
            return;
        }
        ...
        if (smart_routing_) {
            if (partition_id_ != -1) {
                invoked =
                  invocation_service_.invoke_on_partition_owner(shared_from_this(), partition_id_);
        ...
        if (!invoked) {
            notify_exception(
              std::make_exception_ptr(exception::io("No connection found to invoke")));
        }
    } catch (exception::iexception&) {
...
void
ClientInvocation::set_exception(const exception::iexception& e, boost::exception_ptr exception_ptr)
{
    try {
        ...
        if (!lifecycle_service_.is_running()) {
            try {
                std::throw_with_nested(
                  boost::enable_current_exception(exception::hazelcast_client_not_active(
        ...
        if (timePassed > invocation_service_.get_invocation_timeout()) {
            ...
            auto timeoutException =
              (exception::exception_builder<exception::operation_timeout>(
               ...
               << *this << " timed out because exception occurred after client invocation timeout "
        ...
        if (sent_connection) {
            ...
            boost::asio::post(sent_connection->get_socket().get_executor(), [=]() {
                sent_connection->invocations.erase(
                  this_invocation->get_client_message()->get_correlation_id());
            });
        }
    }
}
...
ClientInvocation::should_retry(exception::iexception& exception)
{
    ...
    if (is_bind_to_single_connection() &&
        (errorCode == protocol::IO || errorCode == protocol::TARGET_DISCONNECTED)) {
        return false;
    }
...
operator<<(std::ostream& os, const ClientInvocation& invocation)
{
    ...
    os << "ClientInvocation{"
       ...
       << ", objectName = " << invocation.object_name_ << ", target = " << target.str()
       << ", sendConnection = ";
                         ...
                         const std::shared_ptr<connection::Connection>& connection)
{
    return std::shared_ptr<ClientInvocation>(new ClientInvocation(
      client_context, std::move(client_message), object_name, UNASSIGNED_PARTITION, connection));
}
...
{
    return std::shared_ptr<ClientInvocation>(new ClientInvocation(
      client_context, std::move(client_message), object_name, UNASSIGNED_PARTITION, nullptr, uuid));
}
...
std::shared_ptr<connection::Connection>
ClientInvocation::get_send_connection_or_wait() const
{
...
ClientInvocation::notify(const std::shared_ptr<protocol::ClientMessage>& msg)
{
    ...
    // when the backups complete, the response will be send by the last backup or
    // backup-timeout-handle mechanism kicks on
    if (expected_backups > backup_acks_received_) {
        ...
        // it is very important that the response is set after the backupsAcksExpected is set, else
        // the system can assume the invocation is complete because there is a response and no
        // backups need to respond
        pending_response_ = msg;

        // we are done since not all backups have completed. Therefore we should not notify the
        // future
        return;
    }
...
ClientInvocation::execute()
{
    ...
    // first we force a new invocation slot because we are going to return our old invocation slot
    // immediately after It is important that we first 'force' taking a new slot; otherwise it could
    // be that a sneaky invocation gets through that takes our slot!
    ...
    if (invoke_count_ < MAX_FAST_INVOCATION_COUNT) {
        ...
        int64_t delayMillis = util::min<int64_t>(
          static_cast<int64_t>(1) << (invoke_count_ - MAX_FAST_INVOCATION_COUNT),
...
ClientInvocation::notify_backup()
{
    ...
    if (!pending_response_) {
        // no pendingResponse has been set, so we are done since the invocation on the primary needs
        // to complete first
        return;
    }

    // if a pendingResponse is set, then the backupsAcksExpected has been set (so we can now safely
    // read backupsAcksExpected)
    if (backup_acks_expected_ != backup_acks_received_) {
        // we managed to complete a backup, but we were not the one completing the last backup, so
        // we are done
        return;
    }

    // we are the lucky one since we just managed to complete the last backup for this invocation
    // and since the pendingResponse is set, we can set it on the future
...
{
    // if the backups have completed, we are done; this also filters out all non backup-aware
    // operations since the backupsAcksExpected will always be equal to the backupsAcksReceived
    if (backup_acks_expected_ == backup_acks_received_) {
        return;
    }

    // if no response has yet been received, we we are done; we are only going to re-invoke an
    // operation if the response of the primary has been received, but the backups have not replied
    if (!pending_response_) {
        return;
    }

    // if this has not yet expired (so has not been in the system for a too long period) we ignore
    // it
    ...
    if (invocation_service_.fail_on_indeterminate_state()) {
        auto exception = boost::enable_current_exception(
          (exception::exception_builder<exception::indeterminate_operation_state>(
    ...
    // the backups have not yet completed, but we are going to release the future anyway if a
    // pendingResponse has been set
    complete_with_pending_response();
}
  ...
  std::chrono::steady_clock::time_point start_time)
{
    ...
    auto now = std::chrono::steady_clock::now();
    sb << "Creating transaction context timed out because exception occurred after client "
          "invocation timeout "
       << std::chrono::duration_cast<std::chrono::milliseconds>(invocation_timeout).count()
       << " ms. "
       << "Current time: " << util::StringUtil::time_to_string(std::chrono::steady_clock::now())
       << ". "
       << "Start time: " << util::StringUtil::time_to_string(start_time) << ". Total elapsed time: "
...
ClientTransactionManagerServiceImpl::throw_exception(bool smart_routing)
{
    ...
    if (smart_routing) {
        ...
        if (members.empty()) {
            msg << "No address was return by the LoadBalancer since there are no members in the "
                   "cluster";
        } else {
    ...
    BOOST_THROW_EXCEPTION(exception::illegal_state(
      "ClientTransactionManagerServiceImpl::throw_exception", "No active connection is found"));
}

AwsAddressProvider::AwsAddressProvider(config::client_aws_config& aws_config,
                                       int aws_member_port,
                                       logger& lg)
  : aws_member_port_(util::IOUtil::to_string<int>(aws_member_port))
  , logger_(lg)
  , aws_client_(aws_config, lg)
{}

std::vector<address>
AwsAddressProvider::load_addresses()
{
    update_lookup_table();
...
void
AwsAddressProvider::update_lookup_table()
{
    try {
...
AwsAddressProvider::get_lookup_table()
{
...
address
DefaultAddressTranslator::translate(const address& address)
{
    return address;
}

void
DefaultAddressTranslator::refresh()
{}
  ...
  const std::vector<std::pair<boost::uuids::uuid, std::vector<int>>>& partitions)
{
    HZ_LOG(
      logger_,
      finest,
      boost::str(boost::format("Handling new partition table with partitionStateVersion: %1%") %
                 version));
    ...
    while (true) {
        ...
        if (partition_table_.compare_exchange_strong(
              current,
              boost::shared_ptr<partition_table>(
                new partition_table{ connection, version, convert_to_map(partitions) }))) {
            HZ_LOG(
              logger_,
              finest,
              boost::str(boost::format("Applied partition table with partitionStateVersion : %1%") %
                         version));
            return;
        }
    }
...
int32_t
ClientPartitionServiceImpl::get_partition_id(const serialization::pimpl::data& key)
{
...
bool
ClientPartitionServiceImpl::check_and_set_partition_count(int32_t new_partition_count)
{
    int32_t expected = 0;
  ...
  const partition_table& current)
{
    ...
    if (!current.connection || *connection != *current.connection) {
        HZ_LOG(lg, finest, ([&current, &connection]() {
                   auto frmt =
                     boost::format("Event coming from a new connection. Old connection: %1%, "
                                   "new connection %2%");
...
void
ClientPartitionServiceImpl::log_failure(const std::shared_ptr<connection::Connection>& connection,
                                        int32_t version,
                                        ...
                                        const std::string& cause)
{
    HZ_LOG(logger_, finest, [&]() {
        ...
        if (current.connection) {
            return boost::str(frmt % cause % *connection % *current.connection % version %
                              current.version);
        } else {
...
AbstractCallIdSequence::AbstractCallIdSequence(int32_t max_concurrent_invocations)
{
    ...
    this->max_concurrent_invocations_ =
      util::Preconditions::check_positive(max_concurrent_invocations, out.str());
...
const std::unique_ptr<util::concurrent::IdleStrategy> CallIdSequenceWithBackpressure::IDLER(
  new util::concurrent::BackoffIdleStrategy(
    0,
    0,
    std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::microseconds(1000)).count(),
    std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::microseconds(MAX_DELAY_MS *
                                                                                   1000))
      .count()));
    ...
    for (int64_t idleCount = 0;; idleCount++) {
        ...
        if (elapsedNanos > backoff_timeout_nanos_) {
            throw(exception::exception_builder<exception::hazelcast_overload>(
                  ...
                  << " msecs, elapsed:" << std::chrono::microseconds(elapsedNanos / 1000).count()
                  << " msecs")
              .build();
        }
                                    ...
                                    int64_t backoff_timeout_ms)
{
    if (!is_back_pressure_enabled) {
        ...
        return std::unique_ptr<CallIdSequence>(new CallIdSequenceWithBackpressure(
          max_allowed_concurrent_invocations, backoff_timeout_ms));
    }
}
} // namespace sequence
...
void
listener_service_impl::connection_added(const std::shared_ptr<connection::Connection> connection)
{
...
void
listener_service_impl::connection_removed(const std::shared_ptr<connection::Connection> connection)
{
    boost::asio::post(registration_executor_->get_executor(),
...
listener_service_impl::remove_event_handler(
  ...
  const std::shared_ptr<connection::Connection>& connection)
{
    boost::asio::post(connection->get_socket().get_executor(), std::packaged_task<void()>([=]() {
    ...
    try {
        ...
        if (client_context_.get_lifecycle_service().is_running()) {
            HZ_LOG(
              ...
              warning,
              boost::str(
                boost::format("Delivery of event message to event handler failed. %1%, %2%, %3%") %
...
listener_service_impl::deregister_listener_internal(boost::uuids::uuid user_registration_id)
{
    ...
    for (auto it = listener_registrations.begin(); it != listener_registrations.end();) {
        ...
        try {
            ...
            auto invocation = ClientInvocation::create(client_context_, request, "", subscriber);
            ...
            if (subscriber->is_alive()) {
                ...
                HZ_LOG(logger_,
                       ...
                       boost::str(boost::format("ClientListenerService::deregisterListenerInternal "
                                  ...
                                  user_registration_id % subscriber->get_remote_address() % e));
            }
        }
    }
    if (successful) {
                              ...
                              const std::shared_ptr<connection::Connection>& connection)
{
    ...
    auto invocation =
      ClientInvocation::create(client_context_,
...
void
cluster_view_listener::connection_added(const std::shared_ptr<connection::Connection> connection)
{
    try_register(connection);
}

void
cluster_view_listener::connection_removed(const std::shared_ptr<connection::Connection> connection)
{
...
cluster_view_listener::cluster_view_listener(ClientContext& client_context)
  : client_context_(client_context)
  , listener_added_connection_(nullptr)
{}

void
cluster_view_listener::try_register(std::shared_ptr<connection::Connection> connection)
{
...
cluster_view_listener::event_handler::on_listener_register()
{
    ...
    HZ_LOG(lg,
           finest,
           boost::str(boost::format("Registered ClusterViewListenerHandler to %1%") % *connection));
}
