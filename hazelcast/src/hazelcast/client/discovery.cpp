...
// openssl include should be after the other so that winsock.h and winsock2.h conflict does not
// occur at windows
...
ec2_request_signer::ec2_request_signer(const config::client_aws_config& aws_config,
...
std::string
ec2_request_signer::sign(const std::unordered_map<std::string, std::string>& attributes)
{
  ...
  const std::unordered_map<std::string, std::string>& attributes) const
{
    ...
    out << impl::Constants::GET << NEW_LINE << '/' << NEW_LINE
        << get_canonicalized_query_string(attributes) << NEW_LINE << get_canonical_headers()
        << NEW_LINE << "host" << NEW_LINE << sha256_hashhex("");
    return out.str();
}
...
std::string
ec2_request_signer::get_canonicalized_query_string(const std::vector<std::string>& list) const
{
...
std::string
ec2_request_signer::format_attribute(const std::string& key, const std::string& value)
{
    std::ostringstream out;
...
/* Task 2 */
std::string
ec2_request_signer::create_string_to_sign(const std::string& canonical_request) const
{
    std::ostringstream out;
    out << impl::Constants::SIGNATURE_METHOD_V4 << NEW_LINE << timestamp_ << NEW_LINE
        << get_credential_scope() << NEW_LINE << sha256_hashhex(canonical_request);
    return out.str();
}
...
std::string
ec2_request_signer::create_signature(const std::string& string_to_sign,
...
std::string
ec2_request_signer::convert_to_hex_string(const unsigned char* buffer, unsigned int len) const
{
    std::stringstream ss;
...
{
    return hmac_sh_a256_bytes(key, key_len, (unsigned char*)&msg[0], msg.length(), hash);
}
...
address
AwsAddressTranslator::translate(const address& addr)
{
...
bool
AwsAddressTranslator::find_from_cache(const address& addr, address& translated_address)
{
    ...
    std::unordered_map<std::string, std::string>::const_iterator publicAddressIt =
      mapping->find(addr.get_host());
...
DescribeInstances::DescribeInstances(config::client_aws_config& aws_config,
                                     const std::string& endpoint,
                                     logger& lg)
  : aws_config_(aws_config)
  , endpoint_(endpoint)
...
DescribeInstances::try_get_default_iam_role()
{
    ...
    try {
        ...
        BOOST_THROW_EXCEPTION(exception::invalid_configuration(
          "tryGetDefaultIamRole", std::string("Invalid Aws Configuration. ") + e.what()));
    }
}
...
#pragma warning(pop)
    ...
    if (!uri) {
        BOOST_THROW_EXCEPTION(exception::illegal_argument(
          ...
          "Could not acquire credentials! Did not find declared AWS access key or IAM Role, and "
          "could not discover IAM Task Role or default role."));
    }
...
DescribeInstances::get_keys_from_iam_role()
{
    ...
    try {
        ...
        out << "Unable to retrieve credentials from IAM Task Role. URI: " << query << ". \n "
            << e.what();
        BOOST_THROW_EXCEPTION(exception::invalid_configuration("getKeysFromIamRole", out.str()));
    }
}
...
{
    utility::cloud_utility::unmarshal_json_response(in, aws_config_, attributes_);
}
            ...
            if (privateIp) {
                ...
                HZ_LOG(lg,
                       ...
                       boost::str(boost::format("Accepting EC2 instance [%1%][%2%]") %
                                  instanceItem.second.get_optional<std::string>("tagset.item.value")
                                    .value_or("") %
                                  prIp));
            }
        }
...
void
cloud_utility::unmarshal_json_response(std::istream& stream,
...
aws_client::aws_client(config::client_aws_config& aws_config, logger& lg)
  : aws_config_(aws_config)
  , logger_(lg)
{
    ...
    if (!aws_config.get_region().empty() && aws_config.get_region().length() > 0) {
        if (aws_config.get_host_header().find("ec2.") != 0) {
            BOOST_THROW_EXCEPTION(exception::invalid_configuration(
              "AWSClient::AWSClient", "HostHeader should start with \"ec2.\" prefix"));
        }
        boost::replace_all(
          this->endpoint_, "ec2.", std::string("ec2.") + aws_config.get_region() + ".");
    }
}
