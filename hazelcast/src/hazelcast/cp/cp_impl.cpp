...
proxy_session_manager::proxy_session_manager(hazelcast::client::spi::ClientContext& client)
  : client_(client)
{}
...
int64_t
proxy_session_manager::acquire_session(const raft_group_id& group_id, int32_t count)
{
...
proxy_session_manager::get_or_create_session(const raft_group_id& group_id)
{
    ...
    if (!running_) {
        BOOST_THROW_EXCEPTION(client::exception::hazelcast_instance_not_active(
          "proxy_session_manager::get_or_create_session", "Session manager is already shut down!"));
    }
...
proxy_session_manager::create_new_session(const raft_group_id& group_id)
{
    ...
    auto session =
      sessions_.emplace(group_id, session_state{ response.id, response.ttl_millis }).first;
...
{
    auto request =
      client::protocol::codec::cpsession_createsession_encode(group_id, client_.get_name());
    auto response =
      spi::impl::ClientInvocation::create(client_, request, "sessionManager")->invoke().get();
...
void
proxy_session_manager::invalidate_session(const raft_group_id& group_id, int64_t session_id)
{
    {
        boost::upgrade_lock<boost::shared_mutex> read_lock(lock_);
        auto session = sessions_.find(group_id);
...
void
proxy_session_manager::release_session(const raft_group_id& group_id, int64_t session_id)
{
...
int64_t
proxy_session_manager::get_or_create_unique_thread_id(const raft_group_id& group_id)
{
    boost::upgrade_lock<boost::shared_mutex> read_lock(lock_);
...
int64_t
proxy_session_manager::get_session_acquire_count(const raft_group_id& group_id, int64_t session_id)
{
    boost::upgrade_lock<boost::shared_mutex> read_lock(lock_);
    auto session = sessions_.find(group_id);
...
proxy_session_manager::generate_thread_id(const raft_group_id& group_id)
{
    ...
    return spi::impl::ClientInvocation::create(client_, request, "sessionManager")
      ->invoke()
      .get()
