                                     ...
                                     const std::string& object_name)
{
    ...
    auto is_sessionless = spi::impl::ClientInvocation::create(context_, request, object_name)
                            ->invoke()
                            .get()
                            .get_first_fixed_sized_field<bool>();
    if (is_sessionless) {
        return std::shared_ptr<counting_semaphore>(new sessionless_semaphore(
          proxy_name, &context_, group_id, object_name, context_.get_proxy_session_manager()));
    } else {
        return std::shared_ptr<counting_semaphore>(new session_semaphore(
          proxy_name, &context_, group_id, object_name, context_.get_proxy_session_manager()));
    }
}
...
raft_proxy_factory::object_name_for_proxy(const std::string& name)
{
    ...
    Preconditions::check_true(index < (name.size() - 1), "Object name cannot be empty string");
    Preconditions::check_true(name.find('@', index + 1) == std::string::npos,
                              "Custom CP group name must be specified at most once");

    auto object_name = name.substr(0, index);
    boost::trim(object_name);
    Preconditions::check_true(object_name.size() > 0, "Object name cannot be empty string");
    return object_name;
}

raft_group_id
raft_proxy_factory::get_group_id(const std::string& proxy_name, const std::string& object_name)
{
    auto request = cpgroup_createcpgroup_encode(proxy_name);
    return *spi::impl::ClientInvocation::create(context_, request, object_name)
              ->invoke()
              .get()
              .get_first_var_sized_field<raft_group_id>();
}
...
boost::future<int64_t>
atomic_long::alter_data(data& function_data, alter_result_type result_type)
{
    auto request = atomiclong_alter_encode(
      group_id_, object_name_, function_data, static_cast<int32_t>(result_type));
    return invoke_and_get_future<int64_t>(request);
}

boost::future<boost::optional<data>>
atomic_long::apply_data(data& function_data)
{
...
boost::future<boost::optional<data>>
atomic_reference::set_data(const data& new_value_data)
{
...
boost::future<boost::optional<data>>
atomic_reference::get_and_set_data(const data& new_value_data)
{
...
boost::future<bool>
atomic_reference::compare_and_set_data(const data& expect_data, const data& update_data)
{
    auto request =
      atomicref_compareandset_encode(group_id_, object_name_, &expect_data, &update_data);
    return invoke_and_get_future<bool>(request);
}

boost::future<bool>
atomic_reference::contains_data(const data& value_data)
{
...
boost::future<void>
atomic_reference::alter_data(const data& function_data)
{
    return to_void_future(invoke_apply(function_data, return_value_type::NO_VALUE, true));
}

boost::future<boost::optional<data>>
atomic_reference::alter_and_get_data(const data& function_data)
{
    return invoke_apply(function_data, return_value_type::NEW, true);
}

boost::future<boost::optional<data>>
atomic_reference::get_and_alter_data(const data& function_data)
{
    return invoke_apply(function_data, return_value_type::OLD, true);
}

boost::future<boost::optional<data>>
atomic_reference::apply_data(const data& function_data)
{
...
{
    auto request = atomicref_apply_encode(
      group_id_, object_name_, function_data, static_cast<int32_t>(return_type), alter);
    return invoke_and_get_future<boost::optional<data>>(request);
}
...
latch::try_wait()
{
    return get_count().then(boost::launch::deferred,
                            [](boost::future<int32_t> f) { return f.get() == 0; });
}
...
latch::wait_for(std::chrono::milliseconds timeout)
{
    ...
    auto request =
      countdownlatch_await_encode(group_id_, object_name_, invoation_uid, timeout_millis);
...
fenced_lock::lock_and_get_fence()
{
    ...
    auto do_lock_once = [=]() {
        ...
        return do_lock(session_id, thread_id, invocation_uid)
          .then(boost::launch::deferred, [=](boost::future<int64_t> f) {
              try {
                  auto fence = f.get();
                  ...
                  BOOST_THROW_EXCEPTION(exception::lock_acquire_limit_reached(
                    "fenced_lock::lock_and_get_fence",
                    (boost::format("Lock [%1%] reentrant lock limit is already reached!") %
                     object_name_)
                      .str()));
                  ...
                  BOOST_THROW_EXCEPTION(exception::lock_acquire_limit_reached(
                    "fenced_lock::lock_and_get_fence",
                    (boost::format("Lock [%1%] not acquired because the lock call on the CP group "
...
fenced_lock::throw_lock_ownership_lost(int64_t session_id) const
{
    BOOST_THROW_EXCEPTION(client::exception::lock_ownership_lost(
      "fenced_lock",
      (boost::format("Current thread is not owner of the Lock[%1%] because its Session[%2%] is "
                     "closed by server!") %
       get_name() % session_id)
        .str()));
}
...
fenced_lock::throw_illegal_monitor_state() const
{
    BOOST_THROW_EXCEPTION(client::exception::illegal_monitor_state(
      "fenced_lock",
      (boost::format("Current thread is not owner of the Lock[%1%]") % get_name()).str()));
}
...
{
    auto request =
      client::protocol::codec::fencedlock_getlockownership_encode(group_id_, object_name_);
...
fenced_lock::try_lock()
{
    return try_lock_and_get_fence().then(
      boost::launch::deferred, [](boost::future<int64_t> f) { return f.get() != INVALID_FENCE; });
}
...
fenced_lock::try_lock_and_get_fence(std::chrono::milliseconds timeout)
{
    ...
    return do_try_lock_once().then(boost::launch::deferred,
                                   [=](boost::future<std::pair<int64_t, bool>> f) {
                                       auto result = f.get();
                                       if (!result.second) {
                                           return result.first;
                                       }
...
fenced_lock::get_fence()
{
    ...
    return do_get_lock_ownership_state().then(
      boost::launch::deferred, [=](boost::future<lock_ownership_state> f) {
...
session_aware_proxy::session_aware_proxy(const std::string& service_name,
                                         const std::string& proxy_name,
                                         spi::ClientContext* context,
                                         const raft_group_id& group_id,
                                         const std::string& object_name,
...
counting_semaphore::counting_semaphore(const std::string& proxy_name,
                                       spi::ClientContext* context,
                                       const raft_group_id& group_id,
                                       const std::string& object_name,
                                       internal::session::proxy_session_manager& session_manager)
  : session_aware_proxy(SERVICE_NAME, proxy_name, context, group_id, object_name, session_manager)
{}
...
counting_semaphore::init(int32_t permits)
{
    ...
    return spi::impl::ClientInvocation::create(context_, request, object_name_)
      ->invoke()
...
boost::future<bool>
counting_semaphore::try_acquire_for(std::chrono::milliseconds rel_time, int32_t permits)
{
...
boost::future<void>
counting_semaphore::do_release(int32_t permits, int64_t thread_id, int64_t session_id)
{
    auto invocation_uid = get_context().get_hazelcast_client_implementation()->random_uuid();
    ...
    return to_void_future(
      spi::impl::ClientInvocation::create(context_, request, object_name_)->invoke());
}
      ...
      .then(boost::launch::deferred, [=](boost::future<protocol::ClientMessage> f) {
          try {
              ...
              throw exception::illegal_state(
                ...
                (boost::format("Semaphore[%1%] ] not acquired because the acquire call on the CP "
...
boost::future<bool>
sessionless_semaphore::try_acquire_for_millis(int32_t permits, std::chrono::milliseconds timeout)
{
    util::Preconditions::check_positive(permits, "Permits must be positive!");

    return do_try_acquire(
      permits,
      timeout > std::chrono::milliseconds::zero() ? timeout : std::chrono::milliseconds::zero());
}
...
session_semaphore::session_semaphore(const std::string& proxy_name,
                                     spi::ClientContext* context,
                                     const raft_group_id& group_id,
                                     const std::string& object_name,
                                     internal::session::proxy_session_manager& session_manager)
...
session_semaphore::try_acquire_for_millis(int32_t permits, std::chrono::milliseconds timeout)
{
    ...
    auto do_try_acquire_once = ([=]() {
        ...
        auto request = client::protocol::codec::semaphore_acquire_encode(
          group_id_, object_name_, session_id, thread_id, invocation_uid, permits, timeout.count());
        return spi::impl::ClientInvocation::create(context_, request, object_name_)
          ->invoke()
          .then(boost::launch::deferred, [=](boost::future<protocol::ClientMessage> f) {
              try {
                  ...
                  if (!use_timeout) {
                      BOOST_THROW_EXCEPTION(exception::illegal_state(
                        ...
                        (boost::format("Semaphore[%1%] not acquired because the acquire call on "
                                       "the CP group is cancelled, possibly because of another "
    ...
    return do_try_acquire_once().then(
      boost::launch::deferred, [=](boost::future<std::pair<bool, bool>> f) {
          auto result = f.get();
          if (!result.second) {
              return result.first;
          }
...
session_semaphore::throw_illegal_state_exception(std::exception_ptr e)
{
    auto ise = boost::enable_current_exception(
      exception::illegal_state("session_semaphore::illegal_state", "No valid session!"));
    if (!e) {
        throw ise;
    }
    try {
