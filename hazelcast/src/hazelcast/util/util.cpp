...
std::istream&
SyncHttpsClient::open_connection()
{
    ...
    try {
        ...
        size_t bytesRead;
        while ((bytesRead = boost::asio::read(
                  *socket_, response_.prepare(1024), boost::asio::transfer_at_least(1), error))) {
        ...
        std::ostringstream out;
        out << "Could not retrieve response from https://" << server_ << uri_path_
            << " Error:" << e.what();
        throw client::exception::io("SyncHttpsClient::openConnection", out.str());
    }
#endif // HZ_BUILD_WITH_SSL
...
namespace util {
int32_t
UTFUtil::is_valid_ut_f8(const std::string& str)
{
...
void
sleepmillis(uint64_t milliseconds)
{
...
int
strerror_s(int errnum, char* strerrbuf, size_t buflen, const char* msg_prefix)
{
    int numChars = 0;
...
StringUtil::time_to_string(std::chrono::steady_clock::time_point t)
{
    ...
    oss << '.' << std::setfill('0') << std::setw(3)
        << duration_cast<milliseconds>(systemDuration).count() % 1000;

    return oss.str();
}

std::vector<std::string>
StringUtil::tokenize_version_string(const std::string& version)
{
...
SyncHttpClient::open_connection()
{
    try {
        ...
        std::ostringstream out;
        out << "Could not retrieve response from http://" << server_ << uri_path_
            << " Error:" << e.what();
        throw client::exception::io("SyncHttpClient::openConnection", out.str());
    }
}
                                         ...
                                         int64_t max_park_period_ns)
{
    ...
    Preconditions::check_not_negative(min_park_period_ns,
                                      "minParkPeriodNs must be positive or zero");
...
namespace util {
const std::string&
Preconditions::check_has_text(const std::string& argument, const std::string& error_message)
{
    if (argument.empty()) {
...
Preconditions::check_ssl(const std::string& source_method)
{
    ...
    throw client::exception::invalid_configuration(
      ...
      "HZ_BUILD_WITH_SSL flag. You should also have the openssl installed on your machine and you "
      "need "
    ...
    if (!inetAddress.get()) {
        ...
        for (int i = 0; i < port_try_count; i++) {
            if (inetAddress->is_v4()) {
                ...
                addresses.push_back(client::address(
                  scoped_address, possiblePort + i, inetAddress->to_v6().scope_id()));
            }
        }
    }
...
AddressHolder::AddressHolder(const std::string& address, const std::string& scope_id, int port)
  : address_(address)
  , scope_id_(scope_id)
  , port_(port)
{}
...
const std::shared_ptr<exception_util::runtime_exception_factory>
  exception_util::hazelcastExceptionFactory(new class hazelcast_exception_factory());
...
void
exception_util::hazelcast_exception_factory::rethrow(std::exception_ptr throwable,
...
ByteBuffer::read_int()
{
    ...
    char c = read_byte();
    char d = read_byte();
    return (0xff000000 & (a << 24)) | (0x00ff0000 & (b << 16)) | (0x0000ff00 & (c << 8)) |
           (0x000000ff & d);
}
...
void
hz_thread_pool::shutdown_gracefully()
{
    pool_->join();
    pool_->stop();
