...
std::ostream&
operator<<(std::ostream& os, logger::level lvl)
{
    switch (lvl) {
...
logger::logger(std::string instance_name,
...
void
logger::log(const char* file_name, int line, level lvl, const std::string& msg) noexcept
{
...
std::tm
time_t_to_localtime(const std::time_t& t)
{
    std::tm lt;
                        ...
                        const std::string& msg) noexcept
{
    ...
    sstrm << time_buffer << std::setfill('0') << std::setw(3) << ms << ' ' << lvl << ": ["
          << std::this_thread::get_id() << "] " << instance_name << '[' << cluster_name << "] ["
          << client::version() << "] [" << file_name << ':' << line << "] " << msg << '\n';
